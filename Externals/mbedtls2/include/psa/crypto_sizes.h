 ...
 *
 * \return                    The tag length for the specified algorithm and
 * key. If the AEAD algorithm does not have an identified tag that can be
 * distinguished from the rest of the ciphertext, return 0. If the key type or
 * AEAD algorithm is not recognized, or the parameters are incompatible, return
 * 0.
 */
...
#define PSA_AEAD_NONCE_LENGTH(key_type, alg)                                   \
  (PSA_BLOCK_CIPHER_BLOCK_LENGTH(key_type) == 16                               \
       ? MBEDTLS2_PSA_ALG_AEAD_EQUAL(alg, PSA_ALG_CCM)   ? 13           \
         : MBEDTLS2_PSA_ALG_AEAD_EQUAL(alg, PSA_ALG_GCM) ? 12           \
                                                                : 0            \
   : (key_type) == PSA_KEY_TYPE_CHACHA20 &&                                    \
           MBEDTLS2_PSA_ALG_AEAD_EQUAL(alg, PSA_ALG_CHACHA20_POLY1305)  \
       ? 12                                                                    \
       : 0)
 ...
 *                          \p #PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(\p
 * key_type),
 *                          \p key_bits).
 */
 ...
 *          side effects.
 *
 * \param key_type  A symmetric key type that is compatible with algorithm \p
 * alg.
 *
 ...
 * \return              A sufficient output size for the specified key type and
 *                      algorithm. If the key type or cipher algorithm is not
 *                      recognized, or the parameters are incompatible, return
 * 0.
 */
