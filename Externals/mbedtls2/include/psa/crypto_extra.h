...
#if !defined(MBEDTLS2_PSA_KEY_SLOT_COUNT)
...
static inline void psa_set_key_slot_number(psa_key_attributes_t *attributes,
                                           psa_key_slot_number_t slot_number) {
  attributes->core.flags |= MBEDTLS2_PSA_KA_FLAG_HAS_SLOT_NUMBER;
 ...
 */
typedef struct mbedtls2_psa_stats_s {
 ...
 */
void mbedtls2_psa_get_stats(mbedtls2_psa_stats_t *stats);
 ...
 * * If the compile-time option MBEDTLS2_PSA_INJECT_ENTROPY is enabled.
 *   Note that you must provide compatible implementations of
 *   mbedtls2_nv_seed_read and mbedtls2_nv_seed_write.
 ...
 *                          or equal to both
 * #MBEDTLS2_ENTROPY_MIN_PLATFORM and
 * #MBEDTLS2_ENTROPY_BLOCK_SIZE. It must be less or equal to
 *                          #MBEDTLS2_ENTROPY_MAX_SEED_SIZE.
 *
 * \retval #PSA_SUCCESS
...
mbedtls2_ecc_group_to_psa(mbedtls2_ecp_group_id grpid,
                                 size_t *bits) {
  switch (grpid) {
 ...
 *                      (`MBEDTLS2_ECP_DP_xxx`).
 * \return              #MBEDTLS2_ECP_DP_NONE if \c curve is not
 * recognized.
 * \return              #MBEDTLS2_ECP_DP_NONE if \p bits is not
 *                      correct for \p curve.
 */
 ...
 *
 * When the compile-time option #MBEDTLS2_PSA_CRYPTO_EXTERNAL_RNG is
 * enabled, this function replaces Mbed TLS's entropy and DRBG modules for all
...
psa_status_t mbedtls2_psa_external_get_random(
    mbedtls2_psa_external_random_context_t *context, uint8_t *output,
 ...
 * implementation.
 *
 * The range of key identifiers from #MBEDTLS2_PSA_KEY_ID_BUILTIN_MIN
 * to #MBEDTLS2_PSA_KEY_ID_BUILTIN_MAX within the range from
 ...
 */
#define MBEDTLS2_PSA_KEY_ID_BUILTIN_MIN ((psa_key_id_t)0x7fff0000)

/** The maximum value for a key identifier that is built into the
 * implementation.
 *
 * See #MBEDTLS2_PSA_KEY_ID_BUILTIN_MIN for more information.
 */
#define MBEDTLS2_PSA_KEY_ID_BUILTIN_MAX ((psa_key_id_t)0x7fffefff)
...
static inline int psa_key_id_is_builtin(psa_key_id_t key_id) {
  return ((key_id >= MBEDTLS2_PSA_KEY_ID_BUILTIN_MIN) &&
          (key_id <= MBEDTLS2_PSA_KEY_ID_BUILTIN_MAX));
}

/** Platform function to obtain the location and slot number of a built-in key.
 *
 * An application-specific implementation of this function must be provided if
 * #MBEDTLS2_PSA_CRYPTO_BUILTIN_KEYS is enabled. This would typically be
 * provided as part of a platform's system image.
 *
 * #MBEDTLS2_SVC_KEY_ID_GET_KEY_ID(\p key_id) needs to be in the range
 * from #MBEDTLS2_PSA_KEY_ID_BUILTIN_MIN to
 * #MBEDTLS2_PSA_KEY_ID_BUILTIN_MAX.
 *
 * In a multi-application configuration
 * (\c MBEDTLS2_PSA_CRYPTO_KEY_ID_ENCODES_OWNER is defined),
 * this function should check that #MBEDTLS2_SVC_KEY_ID_GET_OWNER_ID(\p
 * key_id) is allowed to use the given key.
 *
 ...
 */
psa_status_t mbedtls2_psa_platform_get_builtin_key(
    mbedtls2_svc_key_id_t key_id, psa_key_lifetime_t *lifetime,
