...
Basic usage, to read the Mbed TLS or Mbed Crypto configuration:
    config = ConfigFile()
    if 'MBEDTLS2_RSA_C' in config: print('RSA is enabled')
"""
...
class Setting:
    ...
    Fields:
    * name: the symbol name ('MBEDTLS2_xxx').
...
EXCLUDE_FROM_FULL = frozenset(
    [
        ...
        "MBEDTLS2_CTR_DRBG_USE_128_BIT_KEY",  # interacts with ENTROPY_FORCE_SHA256
        "MBEDTLS2_DEPRECATED_REMOVED",  # conflicts with deprecated options
        "MBEDTLS2_DEPRECATED_WARNING",  # conflicts with deprecated options
        "MBEDTLS2_ECDH_VARIANT_EVEREST_ENABLED",  # influences the use of ECDH in TLS
        "MBEDTLS2_ECP_NO_FALLBACK",  # removes internal ECP implementation
        "MBEDTLS2_ECP_NO_INTERNAL_RNG",  # removes a feature
        "MBEDTLS2_ECP_RESTARTABLE",  # incompatible with USE_PSA_CRYPTO
        "MBEDTLS2_ENTROPY_FORCE_SHA256",  # interacts with CTR_DRBG_128_BIT_KEY
        "MBEDTLS2_HAVE_SSE2",  # hardware dependency
        "MBEDTLS2_MEMORY_BACKTRACE",  # depends on MEMORY_BUFFER_ALLOC_C
        "MBEDTLS2_MEMORY_BUFFER_ALLOC_C",  # makes sanitizers (e.g. ASan) less effective
        "MBEDTLS2_MEMORY_DEBUG",  # depends on MEMORY_BUFFER_ALLOC_C
        "MBEDTLS2_NO_64BIT_MULTIPLICATION",  # influences anything that uses bignum
        "MBEDTLS2_NO_DEFAULT_ENTROPY_SOURCES",  # removes a feature
        "MBEDTLS2_NO_PLATFORM_ENTROPY",  # removes a feature
        "MBEDTLS2_NO_UDBL_DIVISION",  # influences anything that uses bignum
        "MBEDTLS2_PKCS11_C",  # build dependency (libpkcs11-helper)
        "MBEDTLS2_PLATFORM_NO_STD_FUNCTIONS",  # removes a feature
        "MBEDTLS2_PSA_CRYPTO_CONFIG",  # toggles old/new style PSA config
        "MBEDTLS2_PSA_CRYPTO_EXTERNAL_RNG",  # behavior change + build dependency
        "MBEDTLS2_PSA_CRYPTO_KEY_ID_ENCODES_OWNER",  # incompatible with USE_PSA_CRYPTO
        "MBEDTLS2_PSA_CRYPTO_SPM",  # platform dependency (PSA SPM)
        "MBEDTLS2_PSA_INJECT_ENTROPY",  # build dependency (hook functions)
        "MBEDTLS2_REMOVE_3DES_CIPHERSUITES",  # removes a feature
        "MBEDTLS2_REMOVE_ARC4_CIPHERSUITES",  # removes a feature
        "MBEDTLS2_RSA_NO_CRT",  # influences the use of RSA in X.509 and TLS
        "MBEDTLS2_SHA512_NO_SHA384",  # removes a feature
        "MBEDTLS2_SSL_HW_RECORD_ACCEL",  # build dependency (hook functions)
        "MBEDTLS2_TEST_CONSTANT_FLOW_MEMSAN",  # build dependency (clang+memsan)
        "MBEDTLS2_TEST_CONSTANT_FLOW_VALGRIND",  # build dependency (valgrind headers)
        "MBEDTLS2_TEST_NULL_ENTROPY",  # removes a feature
        "MBEDTLS2_X509_ALLOW_UNSUPPORTED_CRITICAL_EXTENSION",  # influences the use of X.509 in TLS
        "MBEDTLS2_ZLIB_SUPPORT",  # build dependency (libz)
    ]
)
...
def is_seamless_alt(name):
    ...
    """
    if name == "MBEDTLS2_PLATFORM_SETUP_TEARDOWN_ALT":
        # Similar to non-platform xxx_ALT, requires platform_alt.h
        return False
    return name.startswith("MBEDTLS2_PLATFORM_")
...
EXCLUDE_FROM_BAREMETAL = frozenset(
    [
        # pylint: disable=line-too-long
        "MBEDTLS2_ENTROPY_NV_SEED",  # requires a filesystem and FS_IO or alternate NV seed hooks
        "MBEDTLS2_FS_IO",  # requires a filesystem
        "MBEDTLS2_HAVEGE_C",  # requires a clock
        "MBEDTLS2_HAVE_TIME",  # requires a clock
        "MBEDTLS2_HAVE_TIME_DATE",  # requires a clock
        "MBEDTLS2_NET_C",  # requires POSIX-like networking
        "MBEDTLS2_PLATFORM_FPRINTF_ALT",  # requires FILE* from stdio.h
        "MBEDTLS2_PLATFORM_NV_SEED_ALT",  # requires a filesystem and ENTROPY_NV_SEED
        "MBEDTLS2_PLATFORM_TIME_ALT",  # requires a clock and HAVE_TIME
        "MBEDTLS2_PSA_CRYPTO_SE_C",  # requires a filesystem and PSA_CRYPTO_STORAGE_C
        "MBEDTLS2_PSA_CRYPTO_STORAGE_C",  # requires a filesystem
        "MBEDTLS2_PSA_ITS_FILE_C",  # requires a filesystem
        "MBEDTLS2_THREADING_C",  # requires a threading interface
        "MBEDTLS2_THREADING_PTHREAD",  # requires pthread
        "MBEDTLS2_TIMING_C",  # requires a clock
    ]
)
...
def baremetal_adapter(name, active, section):
    ...
    if not is_full_section(section):
        return active
    if name == "MBEDTLS2_NO_PLATFORM_ENTROPY":
...
def include_in_crypto(name):
    ...
    if (
        name.startswith("MBEDTLS2_X509_")
        or name.startswith("MBEDTLS2_SSL_")
        or name.startswith("MBEDTLS2_KEY_EXCHANGE_")
    ):
        return False
    if name in [
        "MBEDTLS2_CERTS_C",  # part of libmbedx509
        "MBEDTLS2_DEBUG_C",  # part of libmbedtls
        "MBEDTLS2_NET_C",  # part of libmbedtls
        "MBEDTLS2_PKCS11_C",  # part of libmbedx509
    ]:
        return False
    return True
...
def no_deprecated_adapter(adapter):
    ...
    and sets ``MBEDTLS2_DEPRECATED_REMOVED``.
    """

    def continuation(name, active, section):
        if name == "MBEDTLS2_DEPRECATED_REMOVED":
            return True
...
class ConfigFile(Config):
    ...
    _path_in_tree = "include/mbedtls2/config.h"
    default_path = [
        _path_in_tree,
