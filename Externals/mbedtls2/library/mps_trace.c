...
void mbedtls2_mps_trace_print_msg( int id, int line, const char *format, ... )
{
    ...
    ret = mbedtls2_vsnprintf( str, MPS_TRACE_BUF_SIZE, format, argp );
    va_end( argp );

    if( ret >= 0 && ret < MPS_TRACE_BUF_SIZE )
    {
        str[ret] = '\0';
        mbedtls2_printf( "[%d|L%d]: %s\n", id, line, str );
    }
}

int mbedtls2_mps_trace_get_depth()
{
    return trace_depth;
}
void mbedtls2_mps_trace_dec_depth()
{
    trace_depth--;
}
void mbedtls2_mps_trace_inc_depth()
{
    trace_depth++;
}
...
void mbedtls2_mps_trace_indent( int level, mbedtls2_mps_trace_type ty )
{
    if( level > 0 )
    {
        while( --level )
    ...
    switch( ty )
    {
        case MBEDTLS2_MPS_TRACE_TYPE_COMMENT:
            mbedtls2_printf( "@ " );
            break;

        case MBEDTLS2_MPS_TRACE_TYPE_CALL:
            mbedtls2_printf( "+--> " );
            break;

        case MBEDTLS2_MPS_TRACE_TYPE_ERROR:
            mbedtls2_printf( "E " );
            break;

        case MBEDTLS2_MPS_TRACE_TYPE_RETURN:
            mbedtls2_printf( "< " );
            break;
