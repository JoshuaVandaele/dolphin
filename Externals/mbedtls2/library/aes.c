...
#define AES_VALIDATE_RET(cond)                                                 \
  MBEDTLS2_INTERNAL_VALIDATE_RET(                                       \
      cond, MBEDTLS2_ERR_AES_BAD_INPUT_DATA)
#define AES_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)

#if defined(MBEDTLS2_PADLOCK_C) &&                                      \
    (defined(MBEDTLS2_HAVE_X86) ||                                      \
     defined(MBEDTLS2_PADLOCK_ALIGN16))
...
static void aes_gen_tables(void) {
  ...
  for (i = 0; i < 256; i++) {
    x = FSb[i];
    y = MBEDTLS2_BYTE_0(XTIME(x));
    z = MBEDTLS2_BYTE_0(y ^ x);
...
#else /* MBEDTLS2_AES_FEWER_TABLES */
...
void mbedtls2_aes_init(mbedtls2_aes_context *ctx) {
  AES_VALIDATE(ctx != NULL);

  memset(ctx, 0, sizeof(mbedtls2_aes_context));
}

void mbedtls2_aes_free(mbedtls2_aes_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_aes_context));
}

#if defined(MBEDTLS2_CIPHER_MODE_XTS)
void mbedtls2_aes_xts_init(mbedtls2_aes_xts_context *ctx) {
  AES_VALIDATE(ctx != NULL);

  mbedtls2_aes_init(&ctx->crypt);
  mbedtls2_aes_init(&ctx->tweak);
}

void mbedtls2_aes_xts_free(mbedtls2_aes_xts_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_aes_free(&ctx->crypt);
  mbedtls2_aes_free(&ctx->tweak);
}
...
int mbedtls2_aes_setkey_enc(mbedtls2_aes_context *ctx,
  ...
  case 256:
    ...
    break;
  default:
    return (MBEDTLS2_ERR_AES_INVALID_KEY_LENGTH);
  }
  ...
  if (aes_padlock_ace == -1)
    aes_padlock_ace =
        mbedtls2_padlock_has_support(MBEDTLS2_PADLOCK_ACE);

  if (aes_padlock_ace)
    ctx->rk = RK = MBEDTLS2_PADLOCK_ALIGN16(ctx->buf);
  else
#endif
    ctx->rk = RK = ctx->buf;

#if defined(MBEDTLS2_AESNI_C) && defined(MBEDTLS2_HAVE_X86_64)
  if (mbedtls2_aesni_has_support(MBEDTLS2_AESNI_AES))
    return (mbedtls2_aesni_setkey_enc((unsigned char *)ctx->rk, key,
                                             keybits));
#endif

  for (i = 0; i < (keybits >> 5); i++) {
    RK[i] = MBEDTLS2_GET_UINT32_LE(key, i << 2);
  }
  ...
  case 10:
    ...
    for (i = 0; i < 10; i++, RK += 4) {
      RK[4] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS2_BYTE_1(RK[3])]) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_2(RK[3])] << 8) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_3(RK[3])] << 16) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_0(RK[3])] << 24);
  ...
  case 12:
    ...
    for (i = 0; i < 8; i++, RK += 6) {
      RK[6] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS2_BYTE_1(RK[5])]) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_2(RK[5])] << 8) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_3(RK[5])] << 16) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_0(RK[5])] << 24);
  ...
  case 14:
    ...
    for (i = 0; i < 7; i++, RK += 8) {
      RK[8] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS2_BYTE_1(RK[7])]) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_2(RK[7])] << 8) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_3(RK[7])] << 16) ^
              ((uint32_t)FSb[MBEDTLS2_BYTE_0(RK[7])] << 24);

      RK[9] = RK[1] ^ RK[8];
      RK[10] = RK[2] ^ RK[9];
      RK[11] = RK[3] ^ RK[10];

      RK[12] = RK[4] ^ ((uint32_t)FSb[MBEDTLS2_BYTE_0(RK[11])]) ^
               ((uint32_t)FSb[MBEDTLS2_BYTE_1(RK[11])] << 8) ^
               ((uint32_t)FSb[MBEDTLS2_BYTE_2(RK[11])] << 16) ^
               ((uint32_t)FSb[MBEDTLS2_BYTE_3(RK[11])] << 24);
...
int mbedtls2_aes_setkey_dec(mbedtls2_aes_context *ctx,
                                   const unsigned char *key,
                                   unsigned int keybits) {
  int i, j, ret;
  mbedtls2_aes_context cty;
  uint32_t *RK;
  uint32_t *SK;
  ...
  /* Also checks keybits */
  if ((ret = mbedtls2_aes_setkey_enc(&cty, key, keybits)) != 0)
    goto exit;
...
#if defined(MBEDTLS2_AESNI_C) && defined(MBEDTLS2_HAVE_X86_64)
  if (mbedtls2_aesni_has_support(MBEDTLS2_AESNI_AES)) {
    mbedtls2_aesni_inverse_key((unsigned char *)ctx->rk,
  ...
  *RK++ = *SK++;
  ...
  for (i = ctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8) {
    for (j = 0; j < 4; j++, SK++) {
      *RK++ = AES_RT0(FSb[MBEDTLS2_BYTE_0(*SK)]) ^
              AES_RT1(FSb[MBEDTLS2_BYTE_1(*SK)]) ^
              AES_RT2(FSb[MBEDTLS2_BYTE_2(*SK)]) ^
              AES_RT3(FSb[MBEDTLS2_BYTE_3(*SK)]);
    }
  }
...
#if defined(MBEDTLS2_CIPHER_MODE_XTS)
static int mbedtls2_aes_xts_decode_keys(const unsigned char *key,
                                               unsigned int keybits,
...
int mbedtls2_aes_xts_setkey_enc(mbedtls2_aes_xts_context *ctx,
                                       const unsigned char *key,
                                       unsigned int keybits) {
  ...
  ret = mbedtls2_aes_xts_decode_keys(key, keybits, &key1, &key1bits,
                                            &key2, &key2bits);
  if (ret != 0)
    return (ret);

  /* Set the tweak key. Always set tweak key for the encryption mode. */
  ret = mbedtls2_aes_setkey_enc(&ctx->tweak, key2, key2bits);
  if (ret != 0)
    return (ret);

  /* Set crypt key for encryption. */
  return mbedtls2_aes_setkey_enc(&ctx->crypt, key1, key1bits);
}

int mbedtls2_aes_xts_setkey_dec(mbedtls2_aes_xts_context *ctx,
                                       const unsigned char *key,
                                       unsigned int keybits) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  const unsigned char *key1, *key2;
  ...
  return mbedtls2_aes_setkey_dec(&ctx->crypt, key1, key1bits);
}
#endif /* MBEDTLS2_CIPHER_MODE_XTS */

#define AES_FROUND(X0, X1, X2, X3, Y0, Y1, Y2, Y3)                             \
  do {                                                                         \
    (X0) = *RK++ ^ AES_FT0(MBEDTLS2_BYTE_0(Y0)) ^                       \
           AES_FT1(MBEDTLS2_BYTE_1(Y1)) ^                               \
           AES_FT2(MBEDTLS2_BYTE_2(Y2)) ^                               \
           AES_FT3(MBEDTLS2_BYTE_3(Y3));                                \
                                                                               \
    (X1) = *RK++ ^ AES_FT0(MBEDTLS2_BYTE_0(Y1)) ^                       \
           AES_FT1(MBEDTLS2_BYTE_1(Y2)) ^                               \
           AES_FT2(MBEDTLS2_BYTE_2(Y3)) ^                               \
           AES_FT3(MBEDTLS2_BYTE_3(Y0));                                \
                                                                               \
    (X2) = *RK++ ^ AES_FT0(MBEDTLS2_BYTE_0(Y2)) ^                       \
           AES_FT1(MBEDTLS2_BYTE_1(Y3)) ^                               \
           AES_FT2(MBEDTLS2_BYTE_2(Y0)) ^                               \
           AES_FT3(MBEDTLS2_BYTE_3(Y1));                                \
                                                                               \
    (X3) = *RK++ ^ AES_FT0(MBEDTLS2_BYTE_0(Y3)) ^                       \
           AES_FT1(MBEDTLS2_BYTE_1(Y0)) ^                               \
           AES_FT2(MBEDTLS2_BYTE_2(Y1)) ^                               \
           AES_FT3(MBEDTLS2_BYTE_3(Y2));                                \
  } while (0)

#define AES_RROUND(X0, X1, X2, X3, Y0, Y1, Y2, Y3)                             \
  do {                                                                         \
    (X0) = *RK++ ^ AES_RT0(MBEDTLS2_BYTE_0(Y0)) ^                       \
           AES_RT1(MBEDTLS2_BYTE_1(Y3)) ^                               \
           AES_RT2(MBEDTLS2_BYTE_2(Y2)) ^                               \
           AES_RT3(MBEDTLS2_BYTE_3(Y1));                                \
                                                                               \
    (X1) = *RK++ ^ AES_RT0(MBEDTLS2_BYTE_0(Y1)) ^                       \
           AES_RT1(MBEDTLS2_BYTE_1(Y0)) ^                               \
           AES_RT2(MBEDTLS2_BYTE_2(Y3)) ^                               \
           AES_RT3(MBEDTLS2_BYTE_3(Y2));                                \
                                                                               \
    (X2) = *RK++ ^ AES_RT0(MBEDTLS2_BYTE_0(Y2)) ^                       \
           AES_RT1(MBEDTLS2_BYTE_1(Y1)) ^                               \
           AES_RT2(MBEDTLS2_BYTE_2(Y0)) ^                               \
           AES_RT3(MBEDTLS2_BYTE_3(Y3));                                \
                                                                               \
    (X3) = *RK++ ^ AES_RT0(MBEDTLS2_BYTE_0(Y3)) ^                       \
           AES_RT1(MBEDTLS2_BYTE_1(Y2)) ^                               \
           AES_RT2(MBEDTLS2_BYTE_2(Y1)) ^                               \
           AES_RT3(MBEDTLS2_BYTE_3(Y0));                                \
  } while (0)
...
int mbedtls2_internal_aes_encrypt(mbedtls2_aes_context *ctx,
  ...
  t.X[0] = MBEDTLS2_GET_UINT32_LE(input, 0);
  t.X[0] ^= *RK++;
  t.X[1] = MBEDTLS2_GET_UINT32_LE(input, 4);
  t.X[1] ^= *RK++;
  t.X[2] = MBEDTLS2_GET_UINT32_LE(input, 8);
  t.X[2] ^= *RK++;
  t.X[3] = MBEDTLS2_GET_UINT32_LE(input, 12);
  t.X[3] ^= *RK++;
  ...
  t.X[0] = *RK++ ^ ((uint32_t)FSb[MBEDTLS2_BYTE_0(t.Y[0])]) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_1(t.Y[1])] << 8) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_2(t.Y[2])] << 16) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_3(t.Y[3])] << 24);

  t.X[1] = *RK++ ^ ((uint32_t)FSb[MBEDTLS2_BYTE_0(t.Y[1])]) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_1(t.Y[2])] << 8) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_2(t.Y[3])] << 16) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_3(t.Y[0])] << 24);

  t.X[2] = *RK++ ^ ((uint32_t)FSb[MBEDTLS2_BYTE_0(t.Y[2])]) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_1(t.Y[3])] << 8) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_2(t.Y[0])] << 16) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_3(t.Y[1])] << 24);

  t.X[3] = *RK++ ^ ((uint32_t)FSb[MBEDTLS2_BYTE_0(t.Y[3])]) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_1(t.Y[0])] << 8) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_2(t.Y[1])] << 16) ^
           ((uint32_t)FSb[MBEDTLS2_BYTE_3(t.Y[2])] << 24);

  MBEDTLS2_PUT_UINT32_LE(t.X[0], output, 0);
  MBEDTLS2_PUT_UINT32_LE(t.X[1], output, 4);
  MBEDTLS2_PUT_UINT32_LE(t.X[2], output, 8);
  MBEDTLS2_PUT_UINT32_LE(t.X[3], output, 12);

  mbedtls2_platform_zeroize(&t, sizeof(t));
...
void mbedtls2_aes_encrypt(mbedtls2_aes_context *ctx,
                                 const unsigned char input[16],
                                 unsigned char output[16]) {
  MBEDTLS2_IGNORE_RETURN(
      mbedtls2_internal_aes_encrypt(ctx, input, output));
}
...
int mbedtls2_internal_aes_decrypt(mbedtls2_aes_context *ctx,
                                         const unsigned char input[16],
                                         unsigned char output[16]) {
  int i;
  uint32_t *RK = ctx->rk;
  ...
  t.X[0] = *RK++ ^ ((uint32_t)RSb[MBEDTLS2_BYTE_0(t.Y[0])]) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_1(t.Y[3])] << 8) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_2(t.Y[2])] << 16) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_3(t.Y[1])] << 24);

  t.X[1] = *RK++ ^ ((uint32_t)RSb[MBEDTLS2_BYTE_0(t.Y[1])]) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_1(t.Y[0])] << 8) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_2(t.Y[3])] << 16) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_3(t.Y[2])] << 24);

  t.X[2] = *RK++ ^ ((uint32_t)RSb[MBEDTLS2_BYTE_0(t.Y[2])]) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_1(t.Y[1])] << 8) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_2(t.Y[0])] << 16) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_3(t.Y[3])] << 24);

  t.X[3] = *RK++ ^ ((uint32_t)RSb[MBEDTLS2_BYTE_0(t.Y[3])]) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_1(t.Y[2])] << 8) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_2(t.Y[1])] << 16) ^
           ((uint32_t)RSb[MBEDTLS2_BYTE_3(t.Y[0])] << 24);
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_aes_decrypt(mbedtls2_aes_context *ctx,
                                 const unsigned char input[16],
                                 unsigned char output[16]) {
  MBEDTLS2_IGNORE_RETURN(
      mbedtls2_internal_aes_decrypt(ctx, input, output));
}
#endif /* !MBEDTLS2_DEPRECATED_REMOVED */
 ...
 */
int mbedtls2_aes_crypt_ecb(mbedtls2_aes_context *ctx, int mode,
                                  const unsigned char input[16],
                                  unsigned char output[16]) {
  AES_VALIDATE_RET(ctx != NULL);
  AES_VALIDATE_RET(input != NULL);
  AES_VALIDATE_RET(output != NULL);
  AES_VALIDATE_RET(mode == MBEDTLS2_AES_ENCRYPT ||
                   mode == MBEDTLS2_AES_DECRYPT);

#if defined(MBEDTLS2_AESNI_C) && defined(MBEDTLS2_HAVE_X86_64)
  if (mbedtls2_aesni_has_support(MBEDTLS2_AESNI_AES))
    return (mbedtls2_aesni_crypt_ecb(ctx, mode, input, output));
#endif

#if defined(MBEDTLS2_PADLOCK_C) && defined(MBEDTLS2_HAVE_X86)
  if (aes_padlock_ace) {
    if (mbedtls2_padlock_xcryptecb(ctx, mode, input, output) == 0)
      return (0);
  ...
  if (mode == MBEDTLS2_AES_ENCRYPT)
    return (mbedtls2_internal_aes_encrypt(ctx, input, output));
  else
    return (mbedtls2_internal_aes_decrypt(ctx, input, output));
}
 ...
 */
int mbedtls2_aes_crypt_cbc(mbedtls2_aes_context *ctx, int mode,
  ...
  if (length % 16)
    return (MBEDTLS2_ERR_AES_INVALID_INPUT_LENGTH);

#if defined(MBEDTLS2_PADLOCK_C) && defined(MBEDTLS2_HAVE_X86)
  if (aes_padlock_ace) {
    if (mbedtls2_padlock_xcryptcbc(ctx, mode, length, iv, input,
                                          output) == 0)
      return (0);
  ...
  if (mode == MBEDTLS2_AES_DECRYPT) {
    while (length > 0) {
      memcpy(temp, input, 16);
      ret = mbedtls2_aes_crypt_ecb(ctx, mode, input, output);
      if (ret != 0)
        goto exit;
    ...
    while (length > 0) {
      ...
      ret = mbedtls2_aes_crypt_ecb(ctx, mode, output, output);
      if (ret != 0)
        goto exit;
      memcpy(iv, output, 16);
...
typedef unsigned char mbedtls2_be128[16];
 ...
 * endian machines.
 */
static void mbedtls2_gf128mul_x_ble(unsigned char r[16],
                                           const unsigned char x[16]) {
  uint64_t a, b, ra, rb;

  a = MBEDTLS2_GET_UINT64_LE(x, 0);
  b = MBEDTLS2_GET_UINT64_LE(x, 8);

  ra = (a << 1) ^ 0x0087 >> (8 - ((b >> 63) << 3));
  rb = (a >> 63) | (b << 1);

  MBEDTLS2_PUT_UINT64_LE(ra, r, 0);
  MBEDTLS2_PUT_UINT64_LE(rb, r, 8);
}

/*
 * AES-XTS buffer encryption/decryption
 */
int mbedtls2_aes_crypt_xts(mbedtls2_aes_xts_context *ctx,
  ...
  /* Compute the tweak. */
  ret = mbedtls2_aes_crypt_ecb(&ctx->tweak, MBEDTLS2_AES_ENCRYPT,
                                      data_unit, tweak);
  if (ret != 0)
    return (ret);
  ...
  while (blocks--) {
    ...
    if (leftover && (mode == MBEDTLS2_AES_DECRYPT) && blocks == 0) {
      ...
      mbedtls2_gf128mul_x_ble(tweak, tweak);
    }

    for (i = 0; i < 16; i++)
      tmp[i] = input[i] ^ tweak[i];

    ret = mbedtls2_aes_crypt_ecb(&ctx->crypt, mode, tmp, tmp);
    if (ret != 0)
      return (ret);
  ...
  if (leftover) {
    ...
    unsigned char *t = mode == MBEDTLS2_AES_DECRYPT ? prev_tweak : tweak;
 ...
 */
int mbedtls2_aes_crypt_cfb128(mbedtls2_aes_context *ctx, int mode,
  ...
  if (n > 15)
    return (MBEDTLS2_ERR_AES_BAD_INPUT_DATA);

  if (mode == MBEDTLS2_AES_DECRYPT) {
    while (length--) {
      if (n == 0) {
        ret = mbedtls2_aes_crypt_ecb(ctx, MBEDTLS2_AES_ENCRYPT,
                                            iv, iv);
        if (ret != 0)
          goto exit;
      }
 ...
 */
int mbedtls2_aes_crypt_cfb8(mbedtls2_aes_context *ctx, int mode,
                                   size_t length, unsigned char iv[16],
                                   const unsigned char *input,
                                   unsigned char *output) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  while (length--) {
    memcpy(ov, iv, 16);
    ret =
        mbedtls2_aes_crypt_ecb(ctx, MBEDTLS2_AES_ENCRYPT, iv, iv);
    if (ret != 0)
      goto exit;
 ...
 */
int mbedtls2_aes_crypt_ofb(mbedtls2_aes_context *ctx,
                                  size_t length, size_t *iv_off,
                                  unsigned char iv[16],
                                  const unsigned char *input,
                                  unsigned char *output) {
  ...
  while (length--) {
    if (n == 0) {
      ret = mbedtls2_aes_crypt_ecb(ctx, MBEDTLS2_AES_ENCRYPT, iv,
                                          iv);
      if (ret != 0)
        goto exit;
    }
 ...
 */
int mbedtls2_aes_crypt_ctr(mbedtls2_aes_context *ctx,
 ...
 * Checkup routine
 */
int mbedtls2_aes_self_test(int verbose) {
...
#if defined(MBEDTLS2_CIPHER_MODE_CBC) ||                                \
    defined(MBEDTLS2_CIPHER_MODE_CFB)
  unsigned char iv[16];
#endif
#if defined(MBEDTLS2_CIPHER_MODE_CBC)
...
#if defined(MBEDTLS2_CIPHER_MODE_CTR) ||                                \
    defined(MBEDTLS2_CIPHER_MODE_CFB) ||                                \
    defined(MBEDTLS2_CIPHER_MODE_OFB)
  size_t offset;
#endif
#if defined(MBEDTLS2_CIPHER_MODE_CTR) ||                                \
    defined(MBEDTLS2_CIPHER_MODE_XTS)
  int len;
#endif
#if defined(MBEDTLS2_CIPHER_MODE_CTR)
  unsigned char nonce_counter[16];
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  AES-ECB-%3u (%s): ", keybits,
                             (mode == MBEDTLS2_AES_DECRYPT) ? "dec"
                                                                   : "enc");

    memset(buf, 0, 16);

    if (mode == MBEDTLS2_AES_DECRYPT) {
      ret = mbedtls2_aes_setkey_dec(&ctx, key, keybits);
      aes_tests = aes_test_ecb_dec[u];
    } else {
      ret = mbedtls2_aes_setkey_enc(&ctx, key, keybits);
     ...
     * MBEDTLS2_AES_ALT is defined.
     */
    if (ret == MBEDTLS2_ERR_PLATFORM_FEATURE_UNSUPPORTED &&
        keybits == 192) {
      mbedtls2_printf("skipped\n");
      continue;
    } else if (ret != 0) {
      goto exit;
    }
    ...
    for (j = 0; j < 10000; j++) {
      ret = mbedtls2_aes_crypt_ecb(&ctx, mode, buf, buf);
      if (ret != 0)
        goto exit;
    }
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  AES-CBC-%3u (%s): ", keybits,
                             (mode == MBEDTLS2_AES_DECRYPT) ? "dec"
                                                                   : "enc");
    ...
    for (j = 0; j < 10000; j++) {
      ...
      ret = mbedtls2_aes_crypt_cbc(&ctx, mode, 16, iv, buf, buf);
      if (ret != 0)
        goto exit;
    }
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  AES-CFB128-%3u (%s): ", keybits,
                             (mode == MBEDTLS2_AES_DECRYPT) ? "dec"
                                                                   : "enc");
    ...
    ret =
        mbedtls2_aes_crypt_cfb128(&ctx, mode, 64, &offset, iv, buf, buf);
    if (ret != 0)
      goto exit;
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  AES-OFB-%3u (%s): ", keybits,
                             (mode == MBEDTLS2_AES_DECRYPT) ? "dec"
                                                                   : "enc");
    ...
    ret = mbedtls2_aes_crypt_ofb(&ctx, 64, &offset, iv, buf, buf);
    if (ret != 0)
      goto exit;
  ...
  for (i = 0; i < 6; i++) {
    ...
    offset = 0;
    if ((ret = mbedtls2_aes_setkey_enc(&ctx, key, 128)) != 0)
      goto exit;
    ...
    ret = mbedtls2_aes_crypt_ctr(&ctx, len, &offset, nonce_counter,
...
#if defined(MBEDTLS2_CIPHER_MODE_XTS)
  {
    ...
    mbedtls2_aes_xts_context ctx_xts;

    /*
     * XTS mode
     */
    mbedtls2_aes_xts_init(&ctx_xts);
    ...
    for (i = 0; i < num_tests << 1; i++) {
      ...
      if (mode == MBEDTLS2_AES_DECRYPT) {
        ret = mbedtls2_aes_xts_setkey_dec(&ctx_xts, key, 256);
        if (ret != 0)
          goto exit;
        memcpy(buf, aes_test_xts_ct32[u], len);
        aes_tests = aes_test_xts_pt32[u];
      } else {
        ret = mbedtls2_aes_xts_setkey_enc(&ctx_xts, key, 256);
        if (ret != 0)
          goto exit;
      ...
      ret = mbedtls2_aes_crypt_xts(&ctx_xts, mode, len, data_unit, buf,
                                          buf);
      if (ret != 0)
        goto exit;
