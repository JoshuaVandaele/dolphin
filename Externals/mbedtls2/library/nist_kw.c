 ...
 * Initialize context
 */
void mbedtls2_nist_kw_init(mbedtls2_nist_kw_context *ctx) {
  memset(ctx, 0, sizeof(mbedtls2_nist_kw_context));
}

int mbedtls2_nist_kw_setkey(mbedtls2_nist_kw_context *ctx,
   ...
   */
  if (cipher != MBEDTLS2_CIPHER_ID_AES)
  ...
  if ((ret = mbedtls2_cipher_setkey(
           &ctx->cipher_ctx, key, keybits,
           is_wrap ? MBEDTLS2_ENCRYPT : MBEDTLS2_DECRYPT)) != 0) {
    return (ret);
  }
 ...
 * Free context
 */
void mbedtls2_nist_kw_free(mbedtls2_nist_kw_context *ctx) {
  mbedtls2_cipher_free(&ctx->cipher_ctx);
  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_nist_kw_context));
}
 ...
 */
int mbedtls2_nist_kw_wrap(mbedtls2_nist_kw_context *ctx,
  ...
  *out_len = 0;
  ...
  if (mode == MBEDTLS2_KW_MODE_KW) {
    ...
    MBEDTLS2_PUT_UINT32_BE((in_len & 0xffffffff), output,
  ...
  if (mode == MBEDTLS2_KW_MODE_KWP && in_len <= KW_SEMIBLOCK_LENGTH) {
    memcpy(inbuff, output, 16);
    ret = mbedtls2_cipher_update(&ctx->cipher_ctx, inbuff, 16, output,
                                        &olen);
    if (ret != 0)
      goto cleanup;
  } else {
    ...
    if (semiblocks < MIN_SEMIBLOCKS_COUNT) {
      ret = MBEDTLS2_ERR_CIPHER_BAD_INPUT_DATA;
      goto cleanup;
    }
    ...
    for (t = 1; t <= s; t++) {
      ...
      ret = mbedtls2_cipher_update(&ctx->cipher_ctx, inbuff, 16, outbuff,
                                          &olen);
      if (ret != 0)
        goto cleanup;
...
cleanup:
  ...
  mbedtls2_platform_zeroize(inbuff, KW_SEMIBLOCK_LENGTH * 2);
  mbedtls2_platform_zeroize(outbuff, KW_SEMIBLOCK_LENGTH * 2);
 ...
 */
static int unwrap(mbedtls2_nist_kw_context *ctx,
                  const unsigned char *input, size_t semiblocks,
                  unsigned char A[KW_SEMIBLOCK_LENGTH], unsigned char *output,
                  size_t *out_len) {
  int ret = 0;
...
cleanup:
  ...
  mbedtls2_platform_zeroize(inbuff, sizeof(inbuff));
  mbedtls2_platform_zeroize(outbuff, sizeof(outbuff));
 ...
 */
int mbedtls2_nist_kw_unwrap(mbedtls2_nist_kw_context *ctx,
                                   mbedtls2_nist_kw_mode_t mode,
                                   const unsigned char *input, size_t in_len,
                                   unsigned char *output, size_t *out_len,
                                   size_t out_size) {
  ...
  *out_len = 0;
  ...
  if (mode == MBEDTLS2_KW_MODE_KW) {
    ...
    diff = mbedtls2_ct_memcmp(NIST_KW_ICV1, A, KW_SEMIBLOCK_LENGTH);
  ...
  } else if (mode == MBEDTLS2_KW_MODE_KWP) {
    size_t padlen = 0;
    uint32_t Plen;
    /*
    ...
    if (in_len == KW_SEMIBLOCK_LENGTH * 2) {
      unsigned char outbuff[KW_SEMIBLOCK_LENGTH * 2];
      ret = mbedtls2_cipher_update(&ctx->cipher_ctx, input, 16, outbuff,
                                          &olen);
      if (ret != 0)
        goto cleanup;
    ...
    /* Check ICV in "constant-time" */
    diff = mbedtls2_ct_memcmp(NIST_KW_ICV2, A, KW_SEMIBLOCK_LENGTH / 2);

    if (diff != 0) {
      ret = MBEDTLS2_ERR_CIPHER_AUTH_FAILED;
    }

    Plen = MBEDTLS2_GET_UINT32_BE(A, KW_SEMIBLOCK_LENGTH / 2);
...
cleanup:
  ...
  mbedtls2_platform_zeroize(&bad_padding, sizeof(bad_padding));
  mbedtls2_platform_zeroize(&diff, sizeof(diff));
  mbedtls2_platform_zeroize(A, sizeof(A));
...
int mbedtls2_nist_kw_self_test(int verbose) {
  mbedtls2_nist_kw_context ctx;
  unsigned char out[48];
  size_t olen;
  int i;
  int ret = 0;
  mbedtls2_nist_kw_init(&ctx);

  for (i = 0; i < KW_TESTS; i++) {
    if (verbose != 0)
      mbedtls2_printf("  KW-AES-%u ", (unsigned int)key_len[i] * 8);

    ret = mbedtls2_nist_kw_setkey(&ctx, MBEDTLS2_CIPHER_ID_AES,
                                         kw_key[i], key_len[i] * 8, 1);
    if (ret != 0) {
      if (verbose != 0)
        mbedtls2_printf("  KW: setup failed ");

      goto end;
    }

    ret = mbedtls2_nist_kw_wrap(&ctx, MBEDTLS2_KW_MODE_KW,
                                       kw_msg[i], kw_msg_len[i], out, &olen,
                                       sizeof(out));
    ...
    if ((ret = mbedtls2_nist_kw_setkey(
             &ctx, MBEDTLS2_CIPHER_ID_AES, kw_key[i], key_len[i] * 8,
             0)) != 0) {
      if (verbose != 0)
        mbedtls2_printf("  KW: setup failed ");
    ...
    ret = mbedtls2_nist_kw_unwrap(&ctx, MBEDTLS2_KW_MODE_KW, out,
  ...
  for (i = 0; i < KW_TESTS; i++) {
    olen = sizeof(out);
    if (verbose != 0)
      mbedtls2_printf("  KWP-AES-%u ", (unsigned int)key_len[i] * 8);

    ret = mbedtls2_nist_kw_setkey(&ctx, MBEDTLS2_CIPHER_ID_AES,
                                         kwp_key[i], key_len[i] * 8, 1);
    if (ret != 0) {
      if (verbose != 0)
        mbedtls2_printf("  KWP: setup failed ");

      goto end;
    }
    ret = mbedtls2_nist_kw_wrap(&ctx, MBEDTLS2_KW_MODE_KWP,
                                       kwp_msg[i], kwp_msg_len[i], out, &olen,
                                       sizeof(out));
    ...
    if ((ret = mbedtls2_nist_kw_setkey(
             &ctx, MBEDTLS2_CIPHER_ID_AES, kwp_key[i], key_len[i] * 8,
             0)) != 0) {
      if (verbose != 0)
        mbedtls2_printf("  KWP: setup failed ");
    ...
    ret = mbedtls2_nist_kw_unwrap(&ctx, MBEDTLS2_KW_MODE_KWP, out,
                                         olen, out, &olen, sizeof(out));
