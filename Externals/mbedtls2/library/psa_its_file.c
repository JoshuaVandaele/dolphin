...
static void psa_its_fill_filename(psa_storage_uid_t uid, char *filename) {
  ...
  mbedtls2_snprintf(filename, PSA_ITS_STORAGE_FILENAME_LENGTH,
...
static psa_status_t psa_its_read_file(psa_storage_uid_t uid,
  ...
  *p_stream = fopen(filename, "rb");
  ...
  p_info->size = (header.size[0] | header.size[1] << 8 | header.size[2] << 16 |
                  header.size[3] << 24);
...
psa_status_t psa_its_set(psa_storage_uid_t uid, uint32_t data_length,
  ...
  MBEDTLS2_PUT_UINT32_LE(data_length, header.size, 0);
  MBEDTLS2_PUT_UINT32_LE(create_flags, header.flags, 0);
