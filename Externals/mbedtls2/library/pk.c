...
void mbedtls2_pk_free(mbedtls2_pk_context *ctx) {
  ...
  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_pk_context));
}
...
#endif
  /* MBEDTLS2_PK_RSA_ALT omitted on purpose */
  default:
    return (NULL);
  }
}
...
int mbedtls2_pk_setup_opaque(mbedtls2_pk_context *ctx,
  ...
  const mbedtls2_pk_info_t *const info = &mbedtls2_pk_opaque_info;
 ...
 */
static inline int pk_hashlen_helper(mbedtls2_md_type_t md_alg,
                                    size_t *hash_len) {
  const mbedtls2_md_info_t *md_info;

  if (*hash_len != 0 && md_alg == MBEDTLS2_MD_NONE)
    return (0);

  if ((md_info = mbedtls2_md_info_from_type(md_alg)) == NULL)
    return (-1);

  if (*hash_len != 0 && *hash_len != mbedtls2_md_get_size(md_info))
    return (-1);
 ...
 */
static int pk_restart_setup(mbedtls2_pk_restart_ctx *ctx,
                            const mbedtls2_pk_info_t *info) {
...
int mbedtls2_pk_verify_restartable(
  ...
  PK_VALIDATE_RET(ctx != NULL);
  PK_VALIDATE_RET((md_alg == MBEDTLS2_MD_NONE && hash_len == 0) ||
                  hash != NULL);
  ...
  if (rs_ctx != NULL && mbedtls2_ecp_restart_is_enabled() &&
    ...
    if (ret != MBEDTLS2_ERR_ECP_IN_PROGRESS)
      mbedtls2_pk_restart_free(rs_ctx);
  ...
  if (ctx->pk_info->verify_func == NULL)
    return (MBEDTLS2_ERR_PK_TYPE_MISMATCH);
...
int mbedtls2_pk_verify(mbedtls2_pk_context *ctx,
  ...
  return (mbedtls2_pk_verify_restartable(ctx, md_alg, hash, hash_len,
                                                sig, sig_len, NULL));
}
...
int mbedtls2_pk_verify_ext(mbedtls2_pk_type_t type,
  ...
  if (type == MBEDTLS2_PK_RSASSA_PSS) {
    ...
    ret = mbedtls2_rsa_rsassa_pss_verify_ext(
        mbedtls2_pk_rsa(*ctx), NULL, NULL, MBEDTLS2_RSA_PUBLIC,
        md_alg, (unsigned int)hash_len, hash, pss_opts->mgf1_hash_id,
...
#else
    return (MBEDTLS2_ERR_PK_FEATURE_UNAVAILABLE);
#endif /* MBEDTLS2_RSA_C && MBEDTLS2_PKCS1_V21 */
  }
...
int mbedtls2_pk_sign(mbedtls2_pk_context *ctx,
                            ...
                            void *p_rng) {
  return (mbedtls2_pk_sign_restartable(ctx, md_alg, hash, hash_len, sig,
                                              sig_len, f_rng, p_rng, NULL));
}
...
int mbedtls2_pk_check_pair(const mbedtls2_pk_context *pub,
  ...
  if (prv->pk_info->type == MBEDTLS2_PK_RSA_ALT) {
    if (pub->pk_info->type != MBEDTLS2_PK_RSA)
      return (MBEDTLS2_ERR_PK_TYPE_MISMATCH);
  } else {
...
int mbedtls2_pk_wrap_as_opaque(mbedtls2_pk_context *pk,
  ...
  return (MBEDTLS2_ERR_PK_TYPE_MISMATCH);
#else
  const mbedtls2_ecp_keypair *ec;
  unsigned char d[MBEDTLS2_ECP_MAX_BYTES];
  size_t d_len;
  ...
  if (mbedtls2_pk_get_type(pk) != MBEDTLS2_PK_ECKEY)
    return (MBEDTLS2_ERR_PK_TYPE_MISMATCH);

  ec = mbedtls2_pk_ec(*pk);
  d_len = (ec->grp.nbits + 7) / 8;
  if ((ret = mbedtls2_mpi_write_binary(&ec->d, d, d_len)) != 0)
    return (ret);

  curve_id = mbedtls2_ecc_group_to_psa(ec->grp.id, &bits);
  ...
  return (mbedtls2_pk_setup_opaque(pk, *key));
