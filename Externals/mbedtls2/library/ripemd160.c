 ...
 *  http://homes.esat.kuleuven.be/~bosselae/mbedtls2_ripemd160.html
...
void mbedtls2_ripemd160_init(mbedtls2_ripemd160_context *ctx) {
  memset(ctx, 0, sizeof(mbedtls2_ripemd160_context));
}

void mbedtls2_ripemd160_free(mbedtls2_ripemd160_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_ripemd160_context));
}

void mbedtls2_ripemd160_clone(
    mbedtls2_ripemd160_context *dst,
    const mbedtls2_ripemd160_context *src) {
  *dst = *src;
}
 ...
 */
int mbedtls2_ripemd160_starts_ret(
    mbedtls2_ripemd160_context *ctx) {
  ctx->total[0] = 0;
  ctx->total[1] = 0;
...
void mbedtls2_ripemd160_starts(mbedtls2_ripemd160_context *ctx) {
  mbedtls2_ripemd160_starts_ret(ctx);
}
#endif
 ...
 * Process one block
 */
int mbedtls2_internal_ripemd160_process(
    mbedtls2_ripemd160_context *ctx, const unsigned char data[64]) {
  struct {
    uint32_t A, B, C, D, E, Ap, Bp, Cp, Dp, Ep, X[16];
  } local;

  local.X[0] = MBEDTLS2_GET_UINT32_LE(data, 0);
  local.X[1] = MBEDTLS2_GET_UINT32_LE(data, 4);
  local.X[2] = MBEDTLS2_GET_UINT32_LE(data, 8);
  local.X[3] = MBEDTLS2_GET_UINT32_LE(data, 12);
  local.X[4] = MBEDTLS2_GET_UINT32_LE(data, 16);
  local.X[5] = MBEDTLS2_GET_UINT32_LE(data, 20);
  local.X[6] = MBEDTLS2_GET_UINT32_LE(data, 24);
  local.X[7] = MBEDTLS2_GET_UINT32_LE(data, 28);
  local.X[8] = MBEDTLS2_GET_UINT32_LE(data, 32);
  local.X[9] = MBEDTLS2_GET_UINT32_LE(data, 36);
  local.X[10] = MBEDTLS2_GET_UINT32_LE(data, 40);
  local.X[11] = MBEDTLS2_GET_UINT32_LE(data, 44);
  local.X[12] = MBEDTLS2_GET_UINT32_LE(data, 48);
  local.X[13] = MBEDTLS2_GET_UINT32_LE(data, 52);
  local.X[14] = MBEDTLS2_GET_UINT32_LE(data, 56);
  local.X[15] = MBEDTLS2_GET_UINT32_LE(data, 60);
...
#undef Kp
  ...
  mbedtls2_platform_zeroize(&local, sizeof(local));

  return (0);
}

#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_ripemd160_process(mbedtls2_ripemd160_context *ctx,
                                       const unsigned char data[64]) {
  mbedtls2_internal_ripemd160_process(ctx, data);
}
#endif
 ...
 */
int mbedtls2_ripemd160_update_ret(mbedtls2_ripemd160_context *ctx,
  ...
  if (left && ilen >= fill) {
    ...
    if ((ret = mbedtls2_internal_ripemd160_process(ctx, ctx->buffer)) !=
        0)
      return (ret);
  ...
  while (ilen >= 64) {
    if ((ret = mbedtls2_internal_ripemd160_process(ctx, input)) != 0)
      return (ret);
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_ripemd160_update(mbedtls2_ripemd160_context *ctx,
                                      const unsigned char *input, size_t ilen) {
  mbedtls2_ripemd160_update_ret(ctx, input, ilen);
}
#endif
 ...
 */
int mbedtls2_ripemd160_finish_ret(mbedtls2_ripemd160_context *ctx,
  ...
  MBEDTLS2_PUT_UINT32_LE(low, msglen, 0);
  MBEDTLS2_PUT_UINT32_LE(high, msglen, 4);

  last = ctx->total[0] & 0x3F;
  padn = (last < 56) ? (56 - last) : (120 - last);

  ret = mbedtls2_ripemd160_update_ret(ctx, ripemd160_padding, padn);
  if (ret != 0)
    return (ret);

  ret = mbedtls2_ripemd160_update_ret(ctx, msglen, 8);
  if (ret != 0)
    return (ret);

  MBEDTLS2_PUT_UINT32_LE(ctx->state[0], output, 0);
  MBEDTLS2_PUT_UINT32_LE(ctx->state[1], output, 4);
  MBEDTLS2_PUT_UINT32_LE(ctx->state[2], output, 8);
  MBEDTLS2_PUT_UINT32_LE(ctx->state[3], output, 12);
  MBEDTLS2_PUT_UINT32_LE(ctx->state[4], output, 16);

  return (0);
}

#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_ripemd160_finish(mbedtls2_ripemd160_context *ctx,
                                      unsigned char output[20]) {
  mbedtls2_ripemd160_finish_ret(ctx, output);
}
#endif
 ...
 */
int mbedtls2_ripemd160_ret(const unsigned char *input, size_t ilen,
                                  unsigned char output[20]) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ripemd160_context ctx;

  mbedtls2_ripemd160_init(&ctx);

  if ((ret = mbedtls2_ripemd160_starts_ret(&ctx)) != 0)
    goto exit;

  if ((ret = mbedtls2_ripemd160_update_ret(&ctx, input, ilen)) != 0)
    goto exit;

  if ((ret = mbedtls2_ripemd160_finish_ret(&ctx, output)) != 0)
    goto exit;

exit:
  mbedtls2_ripemd160_free(&ctx);

  return (ret);
}

#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_ripemd160(const unsigned char *input, size_t ilen,
                               unsigned char output[20]) {
  mbedtls2_ripemd160_ret(input, ilen, output);
}
#endif
 ...
 * http://homes.esat.kuleuven.be/~bosselae/mbedtls2_ripemd160.html#HMAC
 */
#define TESTS 8
 ...
 * Checkup routine
 */
int mbedtls2_ripemd160_self_test(int verbose) {
  int i, ret = 0;
  unsigned char output[20];
  ...
  for (i = 0; i < TESTS; i++) {
    if (verbose != 0)
      mbedtls2_printf("  RIPEMD-160 test #%d: ", i + 1);

    ret = mbedtls2_ripemd160_ret(ripemd160_test_str[i],
