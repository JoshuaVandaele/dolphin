...
#define POLY1305_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)
 ...
 */
static void poly1305_process(mbedtls2_poly1305_context *ctx,
                             size_t nblocks, const unsigned char *input,
  ...
  for (i = 0U; i < nblocks; i++) {
    ...
    d0 = MBEDTLS2_GET_UINT32_LE(input, offset + 0);
    d1 = MBEDTLS2_GET_UINT32_LE(input, offset + 4);
    d2 = MBEDTLS2_GET_UINT32_LE(input, offset + 8);
    d3 = MBEDTLS2_GET_UINT32_LE(input, offset + 12);
 ...
 */
static void poly1305_compute_mac(const mbedtls2_poly1305_context *ctx,
  ...
  MBEDTLS2_PUT_UINT32_LE(acc0, mac, 0);
  MBEDTLS2_PUT_UINT32_LE(acc1, mac, 4);
  MBEDTLS2_PUT_UINT32_LE(acc2, mac, 8);
  MBEDTLS2_PUT_UINT32_LE(acc3, mac, 12);
}

void mbedtls2_poly1305_init(mbedtls2_poly1305_context *ctx) {
  POLY1305_VALIDATE(ctx != NULL);

  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_poly1305_context));
}

void mbedtls2_poly1305_free(mbedtls2_poly1305_context *ctx) {
  if (ctx == NULL)
    return;
...
int mbedtls2_poly1305_starts(mbedtls2_poly1305_context *ctx,
                                    const unsigned char key[32]) {
  POLY1305_VALIDATE_RET(ctx != NULL);
  POLY1305_VALIDATE_RET(key != NULL);

  /* r &= 0x0ffffffc0ffffffc0ffffffc0fffffff */
  ctx->r[0] = MBEDTLS2_GET_UINT32_LE(key, 0) & 0x0FFFFFFFU;
  ctx->r[1] = MBEDTLS2_GET_UINT32_LE(key, 4) & 0x0FFFFFFCU;
  ctx->r[2] = MBEDTLS2_GET_UINT32_LE(key, 8) & 0x0FFFFFFCU;
  ctx->r[3] = MBEDTLS2_GET_UINT32_LE(key, 12) & 0x0FFFFFFCU;

  ctx->s[0] = MBEDTLS2_GET_UINT32_LE(key, 16);
  ctx->s[1] = MBEDTLS2_GET_UINT32_LE(key, 20);
  ctx->s[2] = MBEDTLS2_GET_UINT32_LE(key, 24);
  ctx->s[3] = MBEDTLS2_GET_UINT32_LE(key, 28);
  ...
  mbedtls2_platform_zeroize(ctx->queue, sizeof(ctx->queue));
  ctx->queue_len = 0U;

  return (0);
}

int mbedtls2_poly1305_update(mbedtls2_poly1305_context *ctx,
...
int mbedtls2_poly1305_finish(mbedtls2_poly1305_context *ctx,
                                    unsigned char mac[16]) {
  POLY1305_VALIDATE_RET(ctx != NULL);
...
int mbedtls2_poly1305_mac(const unsigned char key[32],
                                 const unsigned char *input, size_t ilen,
                                 unsigned char mac[16]) {
  mbedtls2_poly1305_context ctx;
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  POLY1305_VALIDATE_RET(key != NULL);
  POLY1305_VALIDATE_RET(mac != NULL);
  POLY1305_VALIDATE_RET(ilen == 0 || input != NULL);

  mbedtls2_poly1305_init(&ctx);

  ret = mbedtls2_poly1305_starts(&ctx, key);
  if (ret != 0)
    goto cleanup;

  ret = mbedtls2_poly1305_update(&ctx, input, ilen);
  if (ret != 0)
    goto cleanup;

  ret = mbedtls2_poly1305_finish(&ctx, mac);

cleanup:
  mbedtls2_poly1305_free(&ctx);
  return (ret);
}
...
int mbedtls2_poly1305_self_test(int verbose) {
  unsigned char mac[16];
  unsigned i;
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  for (i = 0U; i < 2U; i++) {
    if (verbose != 0)
      mbedtls2_printf("  Poly1305 test %u ", i);

    ret = mbedtls2_poly1305_mac(test_keys[i], test_data[i],
                                       test_data_len[i], mac);
