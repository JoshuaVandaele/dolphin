...
static int mbedtls2_mps_trace_id = MBEDTLS2_MPS_TRACE_BIT_READER;
#endif /* MBEDTLS2_MPS_ENABLE_TRACE */
...
static inline int mps_reader_is_accumulating(
    mbedtls2_mps_reader const *rd )
{
    mbedtls2_mps_size_t acc_remaining;
    if( rd->acc == NULL )
        return( 0 );
...
static inline mbedtls2_mps_size_t mps_reader_get_fragment_offset(
    mbedtls2_mps_reader const *rd )
{
    unsigned char *acc = rd->acc;
    mbedtls2_mps_size_t frag_offset;
...
static inline mbedtls2_mps_size_t mps_reader_serving_from_accumulator(
    mbedtls2_mps_reader const *rd )
{
    mbedtls2_mps_size_t frag_offset, end;
...
static inline void mps_reader_zero( mbedtls2_mps_reader *rd )
{
    ...
    static mbedtls2_mps_reader const zero =
        { .frag          = NULL,
          .frag_len      = 0,
          .commit        = 0,
          .end           = 0,
...
int mbedtls2_mps_reader_init( mbedtls2_mps_reader *rd,
                             unsigned char *acc,
                             mbedtls2_mps_size_t acc_len )
{
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_init" );
    MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_COMMENT,
                       "* Accumulator size: %u bytes", (unsigned) acc_len );
    mps_reader_zero( rd );
    rd->acc = acc;
    rd->acc_len = acc_len;
    MBEDTLS2_MPS_TRACE_RETURN( 0 );
}

int mbedtls2_mps_reader_free( mbedtls2_mps_reader *rd )
{
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_free" );
    mps_reader_zero( rd );
    MBEDTLS2_MPS_TRACE_RETURN( 0 );
}

int mbedtls2_mps_reader_feed( mbedtls2_mps_reader *rd,
                             unsigned char *new_frag,
                             mbedtls2_mps_size_t new_frag_len )
{
    mbedtls2_mps_size_t copy_to_acc;
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_feed" );
    MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_COMMENT,
                       "* Fragment length: %u bytes", (unsigned) new_frag_len );

    if( new_frag == NULL )
        MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_READER_INVALID_ARG );

    MBEDTLS2_MPS_STATE_VALIDATE_RAW( mps_reader_is_producing( rd ),
        "mbedtls2_mps_reader_feed() requires reader to be in producing mode" );

    if( mps_reader_is_accumulating( rd ) )
    {
        unsigned char *acc    = rd->acc;
        mbedtls2_mps_size_t acc_remaining = rd->acc_share.acc_remaining;
        mbedtls2_mps_size_t acc_available = rd->acc_available;
        ...
        if( acc_remaining > 0 )
        {
            ...
            MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_READER_NEED_MORE );
        }
...
int mbedtls2_mps_reader_get( mbedtls2_mps_reader *rd,
                            mbedtls2_mps_size_t desired,
                            unsigned char **buffer,
                            mbedtls2_mps_size_t *buflen )
{
    unsigned char *frag;
    mbedtls2_mps_size_t frag_len, frag_offset, end, frag_fetched, frag_remaining;
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_get" );
    MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_COMMENT,
                       "* Bytes requested: %u", (unsigned) desired );

    MBEDTLS2_MPS_STATE_VALIDATE_RAW( mps_reader_is_consuming( rd ),
          "mbedtls2_mps_reader_get() requires reader to be in consuming mode" );
    ...
    if( mps_reader_serving_from_accumulator( rd ) )
    {
         ...
         * `frag` starting from the next call to mbedtls2_mps_reader_get().
         */
        ...
        {
            mbedtls2_mps_size_t acc_available;
            ...
            if( acc_available - end != desired )
            {
                 ...
                 * validate this. */
                MBEDTLS2_MPS_TRACE_RETURN(
                    MBEDTLS2_ERR_MPS_READER_INCONSISTENT_REQUESTS );
            }
        }
    ...
    if( frag_remaining < desired )
    {
        ...
        if( buflen == NULL )
        {
            ...
            MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_READER_OUT_OF_DATA );
        }
...
int mbedtls2_mps_reader_commit( mbedtls2_mps_reader *rd )
{
    mbedtls2_mps_size_t end;
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_commit" );
    MBEDTLS2_MPS_STATE_VALIDATE_RAW( mps_reader_is_consuming( rd ),
       "mbedtls2_mps_reader_commit() requires reader to be in consuming mode" );
...
int mbedtls2_mps_reader_reclaim( mbedtls2_mps_reader *rd,
                                int *paused )
{
    unsigned char *frag, *acc;
    mbedtls2_mps_size_t pending, commit;
    mbedtls2_mps_size_t acc_len, frag_offset, frag_len;
    MBEDTLS2_MPS_TRACE_INIT( "mbedtls2_mps_reader_reclaim" );

    if( paused != NULL )
        *paused = 0;

    MBEDTLS2_MPS_STATE_VALIDATE_RAW( mps_reader_is_consuming( rd ),
       "mbedtls2_mps_reader_reclaim() requires reader to be in consuming mode" );
    ...
    if( pending == 0 )
    {
        ...
        if( commit < frag_offset || commit - frag_offset < frag_len )
        {
            ...
            rd->end = commit;
            MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_READER_DATA_LEFT );
        }
    ...
    else
    {
        ...
        mbedtls2_mps_size_t acc_backup_offset;
        mbedtls2_mps_size_t acc_backup_len;
        mbedtls2_mps_size_t frag_backup_offset;
        mbedtls2_mps_size_t frag_backup_len;

        mbedtls2_mps_size_t backup_len;
        mbedtls2_mps_size_t acc_len_needed;
        ...
        if( acc == NULL )
        {
            ...
            MBEDTLS2_MPS_TRACE_RETURN(
                MBEDTLS2_ERR_MPS_READER_NEED_ACCUMULATOR );
        }
        acc_len = rd->acc_len;
        ...
        if( overflow || acc_len < acc_len_needed )
        {
             ...
             * there hadn't been a call to mbedtls2_mps_reader_get()
             * since the last commit. */
            rd->end = commit;
            rd->pending = 0;
            MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_ERROR,
            ...
            MBEDTLS2_MPS_TRACE_RETURN(
                MBEDTLS2_ERR_MPS_READER_ACCUMULATOR_TOO_SMALL );
        }
