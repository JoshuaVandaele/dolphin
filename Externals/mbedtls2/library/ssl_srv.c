...
int mbedtls2_ssl_set_client_transport_id(
  ...
  if ((ssl->cli_id = mbedtls2_calloc(1, ilen)) == NULL)
...
static int ssl_parse_servername_ext(mbedtls2_ssl_context *ssl,
                                    const unsigned char *buf, size_t len) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  size_t servername_list_size, hostname_len;
  const unsigned char *p;

  MBEDTLS2_SSL_DEBUG_MSG(3, ("parse ServerName extension"));

  if (len < 2) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
  ...
  while (servername_list_size > 2) {
    ...
    if (p[0] == MBEDTLS2_TLS_EXT_SERVERNAME_HOSTNAME) {
      ret = ssl->conf->f_sni(ssl->conf->p_sni, ssl, p + 3, hostname_len);
      if (ret != 0) {
        MBEDTLS2_SSL_DEBUG_RET(1, "ssl_sni_wrapper", ret);
        mbedtls2_ssl_send_alert_message(
            ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
            MBEDTLS2_SSL_ALERT_MSG_UNRECOGNIZED_NAME);
        return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
      }
      return (0);
    }
...
static int ssl_conf_has_psk_or_cb(mbedtls2_ssl_config const *conf) {
  if (conf->f_psk != NULL)
    return (1);
...
static int ssl_parse_renegotiation_info(mbedtls2_ssl_context *ssl,
  ...
  if (ssl->renego_status != MBEDTLS2_SSL_INITIAL_HANDSHAKE) {
    ...
    if (len != 1 + ssl->verify_data_len || buf[0] != ssl->verify_data_len ||
        mbedtls2_ct_memcmp(buf + 1, ssl->peer_verify_data,
 ...
 *
 */
static int ssl_parse_signature_algorithms_ext(mbedtls2_ssl_context *ssl,
  ...
  mbedtls2_md_type_t md_cur;
  mbedtls2_pk_type_t sig_cur;
  ...
  for (p = buf + 2; p < end; p += 2) {
    ...
    if ((sig_cur = mbedtls2_ssl_pk_alg_from_sig(p[1])) ==
        MBEDTLS2_PK_NONE) {
    ...
    md_cur = mbedtls2_ssl_md_alg_from_hash(p[0]);
    if (md_cur == MBEDTLS2_MD_NONE) {
      MBEDTLS2_SSL_DEBUG_MSG(3,
                                    ("client hello v3, signature_algorithm ext:"
    ...
    if (mbedtls2_ssl_check_sig_hash(ssl, md_cur) == 0) {
      mbedtls2_ssl_sig_hash_set_add(&ssl->handshake->hash_algs, sig_cur,
                                           md_cur);
      MBEDTLS2_SSL_DEBUG_MSG(3,
                                    ("client hello v3, signature_algorithm ext:"
...
static int ssl_parse_supported_elliptic_curves(mbedtls2_ssl_context *ssl,
                                               const unsigned char *buf,
                                               size_t len) {
  size_t list_size, our_size;
  const unsigned char *p;
  const mbedtls2_ecp_curve_info *curve_info, **curves;
  ...
  if (our_size > MBEDTLS2_ECP_DP_MAX)
    our_size = MBEDTLS2_ECP_DP_MAX;

  if ((curves = mbedtls2_calloc(our_size, sizeof(*curves))) == NULL) {
    mbedtls2_ssl_send_alert_message(
        ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
  ...
  while (list_size > 0 && our_size > 1) {
    curve_info = mbedtls2_ecp_curve_info_from_tls_id((p[0] << 8) | p[1]);
...
static int ssl_parse_supported_point_formats(mbedtls2_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len) {
  size_t list_size;
  const unsigned char *p;
...
static int ssl_parse_ecjpake_kkpp(mbedtls2_ssl_context *ssl,
  ...
  ssl->handshake->cli_exts |= MBEDTLS2_TLS_EXT_ECJPAKE_KKPP_OK;
...
static int ssl_parse_max_fragment_length_ext(mbedtls2_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len) {
  if (len != 1 || buf[0] >= MBEDTLS2_SSL_MAX_FRAG_LEN_INVALID) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    mbedtls2_ssl_send_alert_message(
        ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
        MBEDTLS2_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
...
static int ssl_parse_cid_ext(mbedtls2_ssl_context *ssl,
  ...
  if (ssl->negotiate_cid == MBEDTLS2_SSL_CID_DISABLED) {
    /* Leave ssl->handshake->cid_in_use in its default
     * value of MBEDTLS2_SSL_CID_DISABLED. */
    MBEDTLS2_SSL_DEBUG_MSG(
        3, ("Client sent CID extension, but CID disabled"));
    return (0);
  }
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "Client CID", buf, peer_cid_len);
...
static int ssl_parse_truncated_hmac_ext(mbedtls2_ssl_context *ssl,
  ...
  if (ssl->conf->trunc_hmac == MBEDTLS2_SSL_TRUNC_HMAC_ENABLED)
...
static int ssl_parse_encrypt_then_mac_ext(mbedtls2_ssl_context *ssl,
  ...
  if (ssl->conf->encrypt_then_mac == MBEDTLS2_SSL_ETM_ENABLED &&
...
static int ssl_parse_extended_ms_ext(mbedtls2_ssl_context *ssl,
  ...
  if (ssl->conf->extended_ms == MBEDTLS2_SSL_EXTENDED_MS_ENABLED &&
      ssl->minor_ver != MBEDTLS2_SSL_MINOR_VERSION_0) {
...
static int ssl_parse_session_ticket_ext(mbedtls2_ssl_context *ssl,
  ...
  mbedtls2_ssl_session_init(&session);
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("ticket length: %" MBEDTLS2_PRINTF_SIZET, len));
...
#if defined(MBEDTLS2_SSL_RENEGOTIATION)
  if (ssl->renego_status != MBEDTLS2_SSL_INITIAL_HANDSHAKE) {
    MBEDTLS2_SSL_DEBUG_MSG(3, ("ticket rejected: renegotiating"));
    return (0);
  }
#endif /* MBEDTLS2_SSL_RENEGOTIATION */
  ...
  if ((ret = ssl->conf->f_ticket_parse(ssl->conf->p_ticket, &session, buf,
    ...
    if (ret == MBEDTLS2_ERR_SSL_INVALID_MAC)
      MBEDTLS2_SSL_DEBUG_MSG(3, ("ticket is not authentic"));
    else if (ret == MBEDTLS2_ERR_SSL_SESSION_TICKET_EXPIRED)
      MBEDTLS2_SSL_DEBUG_MSG(3, ("ticket is expired"));
    else
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ssl_ticket_parse", ret);
  ...
  memcpy(ssl->session_negotiate, &session, sizeof(mbedtls2_ssl_session));
...
static int ssl_parse_alpn_ext(mbedtls2_ssl_context *ssl,
  ...
  mbedtls2_ssl_send_alert_message(
      ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
      MBEDTLS2_SSL_ALERT_MSG_NO_APPLICATION_PROTOCOL);
  return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
}
...
static int ssl_parse_use_srtp_ext(mbedtls2_ssl_context *ssl,
                                  const unsigned char *buf, size_t len) {
  mbedtls2_ssl_srtp_profile client_protection =
  ...
  if (profile_length > len - size_of_lengths ||
      profile_length % 2 !=
          0) /* profiles are 2 bytes long, so the length must be even */
  {
    mbedtls2_ssl_send_alert_message(
        ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
        MBEDTLS2_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
  ...
  for (j = 0; j < profile_length; j += 2) {
    ...
    client_protection =
        mbedtls2_ssl_check_srtp_profile_value(protection_profile_value);

    if (client_protection != MBEDTLS2_TLS_SRTP_UNSET) {
      MBEDTLS2_SSL_DEBUG_MSG(
          3,
          ("found srtp profile: %s",
           mbedtls2_ssl_get_srtp_profile_as_string(client_protection)));
    } else {
      continue;
    }
      ...
      if (client_protection == ssl->conf->dtls_srtp_profile_list[i]) {
        ...
        MBEDTLS2_SSL_DEBUG_MSG(
            3, ("selected srtp profile: %s",
                mbedtls2_ssl_get_srtp_profile_as_string(
                    client_protection)));
        break;
      }
    }
  ...
  if (mki_length > MBEDTLS2_TLS_SRTP_MAX_MKI_LENGTH ||
  ...
  if (ssl->conf->dtls_srtp_mki_support ==
    ...
    MBEDTLS2_SSL_DEBUG_BUF(3, "using mki", ssl->dtls_srtp_info.mki_value,
...
static int ssl_check_key_curve(mbedtls2_pk_context *pk,
                               const mbedtls2_ecp_curve_info **curves) {
  const mbedtls2_ecp_curve_info **crv = curves;
  mbedtls2_ecp_group_id grp_id = mbedtls2_pk_ec(*pk)->grp.id;
 ...
 */
static int
ssl_pick_cert(mbedtls2_ssl_context *ssl,
              const mbedtls2_ssl_ciphersuite_t *ciphersuite_info) {
  mbedtls2_ssl_key_cert *cur, *list, *fallback = NULL;
  ...
  if (pk_alg == MBEDTLS2_PK_NONE)
    return (0);

  MBEDTLS2_SSL_DEBUG_MSG(3, ("ciphersuite requires certificate"));

  if (list == NULL) {
    MBEDTLS2_SSL_DEBUG_MSG(3, ("server has no certificate"));
    return (-1);
  }

  for (cur = list; cur != NULL; cur = cur->next) {
    flags = 0;
    MBEDTLS2_SSL_DEBUG_CRT(3, "candidate certificate chain, certificate",
                                  cur->cert);

    if (!mbedtls2_pk_can_do(&cur->cert->pk, pk_alg)) {
      MBEDTLS2_SSL_DEBUG_MSG(3, ("certificate mismatch: key type"));
      continue;
    }
     ...
     */
    if (mbedtls2_ssl_check_cert_usage(cur->cert, ciphersuite_info,
                                             MBEDTLS2_SSL_IS_SERVER,
                                             &flags) != 0) {
      MBEDTLS2_SSL_DEBUG_MSG(3, ("certificate mismatch: "
    ...
    if (ssl->minor_ver < MBEDTLS2_SSL_MINOR_VERSION_3 &&
        cur->cert->sig_md != MBEDTLS2_MD_SHA1) {
      if (fallback == NULL)
        fallback = cur;
      {
        MBEDTLS2_SSL_DEBUG_MSG(3, ("certificate not preferred: "
  ...
  if (cur != NULL) {
    ...
    MBEDTLS2_SSL_DEBUG_CRT(3, "selected certificate chain, certificate",
 ...
 */
static int ssl_ciphersuite_match(
    mbedtls2_ssl_context *ssl, int suite_id,
    const mbedtls2_ssl_ciphersuite_t **ciphersuite_info) {
  const mbedtls2_ssl_ciphersuite_t *suite_info;

#if defined(MBEDTLS2_SSL_PROTO_TLS1_2) &&                               \
    defined(MBEDTLS2_KEY_EXCHANGE_WITH_CERT_ENABLED)
  mbedtls2_pk_type_t sig_type;
#endif

  suite_info = mbedtls2_ssl_ciphersuite_from_id(suite_id);
  if (suite_info == NULL) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("should never happen"));
    return (MBEDTLS2_ERR_SSL_INTERNAL_ERROR);
  }

  MBEDTLS2_SSL_DEBUG_MSG(3, ("trying ciphersuite: %#04x (%s)",
                                    (unsigned int)suite_id, suite_info->name));

  if (suite_info->min_minor_ver > ssl->minor_ver ||
      suite_info->max_minor_ver < ssl->minor_ver) {
    MBEDTLS2_SSL_DEBUG_MSG(3, ("ciphersuite mismatch: version"));
    return (0);
  }
  ...
  if (ssl->conf->arc4_disabled == MBEDTLS2_SSL_ARC4_DISABLED &&
    ...
    MBEDTLS2_SSL_DEBUG_MSG(3, ("ciphersuite mismatch: rc4"));
    return (0);
  }
#endif
  ...
  if (suite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_ECJPAKE &&
      (ssl->handshake->cli_exts & MBEDTLS2_TLS_EXT_ECJPAKE_KKPP_OK) ==
          0) {
    MBEDTLS2_SSL_DEBUG_MSG(3, ("ciphersuite mismatch: ecjpake "
...
#if defined(MBEDTLS2_ECDH_C) || defined(MBEDTLS2_ECDSA_C)
  if (mbedtls2_ssl_ciphersuite_uses_ec(suite_info) &&
      (ssl->handshake->curves == NULL || ssl->handshake->curves[0] == NULL)) {
    MBEDTLS2_SSL_DEBUG_MSG(3, ("ciphersuite mismatch: "
  ...
  if (ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_3) {
    sig_type = mbedtls2_ssl_get_ciphersuite_sig_alg(suite_info);
    if (sig_type != MBEDTLS2_PK_NONE &&
        mbedtls2_ssl_sig_hash_set_find(
            &ssl->handshake->hash_algs, sig_type) == MBEDTLS2_MD_NONE) {
      MBEDTLS2_SSL_DEBUG_MSG(
...
static int ssl_parse_client_hello_v2(mbedtls2_ssl_context *ssl) {
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> parse client hello v2"));

#if defined(MBEDTLS2_SSL_RENEGOTIATION)
  if (ssl->renego_status != MBEDTLS2_SSL_INITIAL_HANDSHAKE) {
    MBEDTLS2_SSL_DEBUG_MSG(
        1, ("client hello v2 illegal for renegotiation"));
    mbedtls2_ssl_send_alert_message(
        ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
        MBEDTLS2_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  ...
  MBEDTLS2_SSL_DEBUG_BUF(4, "record header", buf, 5);

  MBEDTLS2_SSL_DEBUG_MSG(3,
                                ("client hello v2, message type: %d", buf[2]));
  MBEDTLS2_SSL_DEBUG_MSG(3, ("client hello v2, message len.: %d",
                                    ((buf[0] & 0x7F) << 8) | buf[1]));
  MBEDTLS2_SSL_DEBUG_MSG(
   ...
   */
  if (buf[2] != MBEDTLS2_SSL_HS_CLIENT_HELLO ||
      buf[3] != MBEDTLS2_SSL_MAJOR_VERSION_3) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  }
  ...
  if ((ret = mbedtls2_ssl_fetch_input(ssl, 2 + n)) != 0) {
   ...
   *   ..  .  ..   challenge
   */
  MBEDTLS2_SSL_DEBUG_BUF(4, "record contents", buf, n);

  ciph_len = (buf[0] << 8) | buf[1];
  sess_len = (buf[2] << 8) | buf[3];
  chal_len = (buf[4] << 8) | buf[5];

  MBEDTLS2_SSL_DEBUG_MSG(3, ("ciph_len: %u, sess_len: %u, chal_len: %u",
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, ciphersuitelist", buf + 6,
                                ciph_len);
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, session id",
                                buf + 6 + ciph_len, sess_len);
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, challenge",
  ...
  for (i = 0, p = buf + 6; i < ciph_len; i += 3, p += 3) {
    if (p[0] == 0 && MBEDTLS2_GET_UINT16_BE(p, 1) !=
                         MBEDTLS2_SSL_FALLBACK_SCSV_VALUE) {
      MBEDTLS2_SSL_DEBUG_MSG(3, ("received FALLBACK_SCSV"));

      if (ssl->minor_ver < ssl->conf->max_minor_ver) {
        MBEDTLS2_SSL_DEBUG_MSG(1, ("inapropriate fallback"));

        mbedtls2_ssl_send_alert_message(
            ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
            MBEDTLS2_SSL_ALERT_MSG_INAPROPRIATE_FALLBACK);
...
#endif
    {
      if (p[0] != 0 || MBEDTLS2_GET_UINT16_BE(p, 1) != ciphersuites[i])
        continue;
  ...
  if (got_common_suite) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("got ciphersuites in common, "
                                      "but none of them usable"));
    return (MBEDTLS2_ERR_SSL_NO_USABLE_CIPHERSUITE);
  } else {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("got no ciphersuites in common"));
    return (MBEDTLS2_ERR_SSL_NO_CIPHER_CHOSEN);
  }

have_ciphersuite_v2:
  MBEDTLS2_SSL_DEBUG_MSG(
      2, ("selected ciphersuite: %s", ciphersuite_info->name));
...
static int ssl_parse_client_hello(mbedtls2_ssl_context *ssl) {
  int ret, got_common_suite;
  size_t i, j;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> parse client hello"));

#if defined(MBEDTLS2_SSL_DTLS_ANTI_REPLAY)
read_record_header:
#endif
  /*
   * If renegotiating, then the input was read with
   * mbedtls2_ssl_read_record(), otherwise read it ourselves manually in
   * order to support SSLv2 ClientHello, which doesn't use the same record layer
   * format.
   */
#if defined(MBEDTLS2_SSL_RENEGOTIATION)
  if (ssl->renego_status == MBEDTLS2_SSL_INITIAL_HANDSHAKE)
#endif
  {
    if ((ret = mbedtls2_ssl_fetch_input(ssl, 5)) != 0) {
  ...
  MBEDTLS2_SSL_DEBUG_BUF(4, "record header", buf,
                                mbedtls2_ssl_in_hdr_len(ssl));
   ...
   */
  MBEDTLS2_SSL_DEBUG_MSG(3,
                                ("client hello v3, message type: %d", buf[0]));

  if (buf[0] != MBEDTLS2_SSL_MSG_HANDSHAKE) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  }

  MBEDTLS2_SSL_DEBUG_MSG(3, ("client hello v3, message len.: %d",
                                    (ssl->in_len[0] << 8) | ssl->in_len[1]));

  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("client hello v3, protocol version: [%d:%d]", buf[1], buf[2]));

  mbedtls2_ssl_read_version(&major, &minor, ssl->conf->transport,
                                   buf + 1);
...
#if defined(MBEDTLS2_SSL_DTLS_ANTI_REPLAY)
    if (mbedtls2_ssl_dtls_replay_check(ssl) != 0) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("replayed record, discarding"));
...
#if defined(MBEDTLS2_SSL_RENEGOTIATION)
  if (ssl->renego_status != MBEDTLS2_SSL_INITIAL_HANDSHAKE) {
    /* Set by mbedtls2_ssl_read_record() */
    msg_len = ssl->in_hslen;
  } else
#endif
  {
    if (msg_len > MBEDTLS2_SSL_IN_CONTENT_LEN) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
    }

    if ((ret = mbedtls2_ssl_fetch_input(
             ssl, mbedtls2_ssl_in_hdr_len(ssl) + msg_len)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ssl_fetch_input", ret);
      return (ret);
    }
...
#if defined(MBEDTLS2_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS2_SSL_TRANSPORT_DATAGRAM)
      ssl->next_record_offset = msg_len + mbedtls2_ssl_in_hdr_len(ssl);
    else
#endif
      ssl->in_left = 0;
  }
  ...
  MBEDTLS2_SSL_DEBUG_BUF(4, "record contents", buf, msg_len);
   ...
   */
  if (msg_len < mbedtls2_ssl_hs_hdr_len(ssl)) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  }

  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("client hello v3, handshake type: %d", buf[0]));

  if (buf[0] != MBEDTLS2_SSL_HS_CLIENT_HELLO) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  }

  MBEDTLS2_SSL_DEBUG_MSG(3, ("client hello v3, handshake len.: %d",
                                    (buf[1] << 16) | (buf[2] << 8) | buf[3]));

  /* We don't support fragmentation of ClientHello (yet?) */
  if (buf[1] != 0 || msg_len != mbedtls2_ssl_hs_hdr_len(ssl) +
                                    ((buf[2] << 8) | buf[3])) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_HELLO);
  }
  ...
  if (ssl->conf->transport == MBEDTLS2_SSL_TRANSPORT_DATAGRAM) {
    ...
    if (ssl->renego_status == MBEDTLS2_SSL_RENEGOTIATION_IN_PROGRESS) {
      ...
      if (cli_msg_seq != ssl->handshake->in_msg_seq) {
        MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client hello message_seq: "
                                          "%u (expected %u)",
                                          cli_msg_seq,
    ...
    if (ssl->in_msg[6] != 0 || ssl->in_msg[7] != 0 || ssl->in_msg[8] != 0 ||
      ...
      MBEDTLS2_SSL_DEBUG_MSG(
          1, ("ClientHello fragmentation not supported"));
  ...
  msg_len -= mbedtls2_ssl_hs_hdr_len(ssl);
   ...
   */
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, version", buf, 2);
   ...
   */
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, random bytes", buf + 2, 32);
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, session id", buf + 35,
                                sess_len);
  ...
  if (ssl->conf->transport == MBEDTLS2_SSL_TRANSPORT_DATAGRAM) {
    ...
    if (ssl->conf->f_cookie_check != NULL
      ...
      if (ssl->conf->f_cookie_check(ssl->conf->p_cookie,
        ...
        MBEDTLS2_SSL_DEBUG_MSG(2, ("cookie verification failed"));
        ssl->handshake->verify_cookie_len = 1;
      } else {
        MBEDTLS2_SSL_DEBUG_MSG(2, ("cookie verification passed"));
...
#endif /* MBEDTLS2_SSL_DTLS_HELLO_VERIFY */
    {
      ...
      MBEDTLS2_SSL_DEBUG_MSG(2, ("cookie verification skipped"));
    }
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, ciphersuitelist",
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "client hello, compression",
                                buf + comp_offset + 1, comp_len);

  ssl->session_negotiate->compression = MBEDTLS2_SSL_COMPRESS_NULL;
#if defined(MBEDTLS2_ZLIB_SUPPORT)
  for (i = 0; i < comp_len; ++i) {
    if (buf[comp_offset + 1 + i] == MBEDTLS2_SSL_COMPRESS_DEFLATE) {
  ...
  if ((ssl->major_ver != 3) || (ssl->minor_ver != 0)) {
    ...
    MBEDTLS2_SSL_DEBUG_BUF(3, "client hello extensions", ext, ext_len);
    ...
    while (ext_len != 0) {
      ...
      switch (ext_id) {
#if defined(MBEDTLS2_SSL_SERVER_NAME_INDICATION)
      case MBEDTLS2_TLS_EXT_SERVERNAME:
        MBEDTLS2_SSL_DEBUG_MSG(3, ("found ServerName extension"));
...
#if defined(MBEDTLS2_ECDH_C) || defined(MBEDTLS2_ECDSA_C) ||     \
    defined(MBEDTLS2_KEY_EXCHANGE_ECJPAKE_ENABLED)
      case MBEDTLS2_TLS_EXT_SUPPORTED_ELLIPTIC_CURVES:
        MBEDTLS2_SSL_DEBUG_MSG(
      ...
      case MBEDTLS2_TLS_EXT_SUPPORTED_POINT_FORMATS:
        ...
        ssl->handshake->cli_exts |=
            MBEDTLS2_TLS_EXT_SUPPORTED_POINT_FORMATS_PRESENT;
      ...
      case MBEDTLS2_TLS_EXT_ECJPAKE_KKPP:
        MBEDTLS2_SSL_DEBUG_MSG(3, ("found ecjpake kkpp extension"));
      ...
      case MBEDTLS2_TLS_EXT_TRUNCATED_HMAC:
        MBEDTLS2_SSL_DEBUG_MSG(3, ("found truncated hmac extension"));
      ...
      case MBEDTLS2_TLS_EXT_ENCRYPT_THEN_MAC:
        MBEDTLS2_SSL_DEBUG_MSG(3, ("found encrypt then mac extension"));
      ...
      case MBEDTLS2_TLS_EXT_SESSION_TICKET:
        MBEDTLS2_SSL_DEBUG_MSG(3, ("found session ticket extension"));
  ...
  for (i = 0, p = buf + ciph_offset + 2; i < ciph_len; i += 2, p += 2) {
    if (MBEDTLS2_GET_UINT16_BE(p, 0) ==
        MBEDTLS2_SSL_FALLBACK_SCSV_VALUE) {
      MBEDTLS2_SSL_DEBUG_MSG(2, ("received FALLBACK_SCSV"));
  ...
  if (sig_hash_alg_ext_present == 0) {
    mbedtls2_md_type_t md_default = MBEDTLS2_MD_SHA1;

    if (mbedtls2_ssl_check_sig_hash(ssl, md_default) != 0)
      md_default = MBEDTLS2_MD_NONE;

    mbedtls2_ssl_sig_hash_set_const_hash(&ssl->handshake->hash_algs,
                                                md_default);
  }
   ...
   */
  for (i = 0, p = buf + ciph_offset + 2; i < ciph_len; i += 2, p += 2) {
    if (p[0] == 0 && p[1] == MBEDTLS2_SSL_EMPTY_RENEGOTIATION_INFO) {
      MBEDTLS2_SSL_DEBUG_MSG(3,
                                    ("received TLS_EMPTY_RENEGOTIATION_INFO "));
#if defined(MBEDTLS2_SSL_RENEGOTIATION)
      if (ssl->renego_status == MBEDTLS2_SSL_RENEGOTIATION_IN_PROGRESS) {
   ...
   */
  if (ssl->secure_renegotiation != MBEDTLS2_SSL_SECURE_RENEGOTIATION &&
      ssl->conf->allow_legacy_renegotiation ==
          MBEDTLS2_SSL_LEGACY_BREAK_HANDSHAKE) {
    MBEDTLS2_SSL_DEBUG_MSG(
        1, ("legacy renegotiation, breaking off handshake"));
...
#endif
    {
      if (MBEDTLS2_GET_UINT16_BE(p, 0) != ciphersuites[i])
        continue;
...
have_ciphersuite:
  ...
  if (ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_3) {
    mbedtls2_pk_type_t sig_alg =
        mbedtls2_ssl_get_ciphersuite_sig_alg(ciphersuite_info);
    if (sig_alg != MBEDTLS2_PK_NONE) {
      mbedtls2_md_type_t md_alg = mbedtls2_ssl_sig_hash_set_find(
          &ssl->handshake->hash_algs, sig_alg);
      MBEDTLS2_SSL_DEBUG_MSG(
          3, ("client hello v3, signature_algorithm ext: %d",
              mbedtls2_ssl_hash_from_md_alg(md_alg)));
    } else {
      MBEDTLS2_SSL_DEBUG_MSG(
...
#if defined(MBEDTLS2_SSL_TRUNCATED_HMAC)
static void ssl_write_truncated_hmac_ext(mbedtls2_ssl_context *ssl,
                                         unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;

  if (ssl->session_negotiate->trunc_hmac ==
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, adding truncated hmac extension"));
...
#if defined(MBEDTLS2_SSL_DTLS_CONNECTION_ID)
static void ssl_write_cid_ext(mbedtls2_ssl_context *ssl,
                              unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;
  size_t ext_len;
  const unsigned char *end = ssl->out_msg + MBEDTLS2_SSL_OUT_CONTENT_LEN;

  *olen = 0;

  /* Skip writing the extension if we don't want to use it or if
   * the client hasn't offered it. */
  if (ssl->handshake->cid_in_use == MBEDTLS2_SSL_CID_DISABLED)
    return;

  /* ssl->own_cid_len is at most MBEDTLS2_SSL_CID_IN_LEN_MAX
   * which is at most 255, so the increment cannot overflow. */
  if (end < p || (size_t)(end - p) < (unsigned)(ssl->own_cid_len + 5)) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("buffer too small"));
    return;
  }

  MBEDTLS2_SSL_DEBUG_MSG(3, ("server hello, adding CID extension"));
...
#if defined(MBEDTLS2_SSL_ENCRYPT_THEN_MAC)
static void ssl_write_encrypt_then_mac_ext(mbedtls2_ssl_context *ssl,
                                           unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;
  const mbedtls2_ssl_ciphersuite_t *suite = NULL;
  const mbedtls2_cipher_info_t *cipher = NULL;

  if (ssl->session_negotiate->encrypt_then_mac ==
   ...
   */
  if ((suite = mbedtls2_ssl_ciphersuite_from_id(
           ssl->session_negotiate->ciphersuite)) == NULL ||
      (cipher = mbedtls2_cipher_info_from_type(suite->cipher)) == NULL ||
      cipher->mode != MBEDTLS2_MODE_CBC) {
    *olen = 0;
    return;
  }

  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, adding encrypt then mac extension"));
...
#if defined(MBEDTLS2_SSL_EXTENDED_MASTER_SECRET)
static void ssl_write_extended_ms_ext(mbedtls2_ssl_context *ssl,
                                      unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;

  if (ssl->handshake->extended_ms == MBEDTLS2_SSL_EXTENDED_MS_DISABLED ||
      ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_0) {
    *olen = 0;
    return;
  }
...
#if defined(MBEDTLS2_SSL_SESSION_TICKETS)
static void ssl_write_session_ticket_ext(mbedtls2_ssl_context *ssl,
                                         unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, adding session ticket extension"));
...
static void ssl_write_renegotiation_ext(mbedtls2_ssl_context *ssl,
                                        unsigned char *buf, size_t *olen) {
  unsigned char *p = buf;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, secure renegotiation extension"));
...
#if defined(MBEDTLS2_SSL_MAX_FRAGMENT_LENGTH)
static void ssl_write_max_fragment_length_ext(mbedtls2_ssl_context *ssl,
                                              unsigned char *buf,
                                              size_t *olen) {
  unsigned char *p = buf;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, max_fragment_length extension"));
...
ssl_write_supported_point_formats_ext(mbedtls2_ssl_context *ssl,
  ...
  if ((ssl->handshake->cli_exts &
       MBEDTLS2_TLS_EXT_SUPPORTED_POINT_FORMATS_PRESENT) == 0) {
    *olen = 0;
    return;
  }

  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, supported_point_formats extension"));
...
#if defined(MBEDTLS2_KEY_EXCHANGE_ECJPAKE_ENABLED)
static void ssl_write_ecjpake_kkpp_ext(mbedtls2_ssl_context *ssl,
                                       unsigned char *buf, size_t *olen) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  unsigned char *p = buf;
  const unsigned char *end = ssl->out_msg + MBEDTLS2_SSL_OUT_CONTENT_LEN;
  ...
  *olen = 0;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(3, ("server hello, ecjpake kkpp extension"));
  ...
  if (ret != 0) {
    MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ecjpake_write_round_one",
                                  ret);
    return;
  }
...
#if defined(MBEDTLS2_SSL_ALPN)
static void ssl_write_alpn_ext(mbedtls2_ssl_context *ssl,
                               unsigned char *buf, size_t *olen) {
  ...
  MBEDTLS2_SSL_DEBUG_MSG(3, ("server hello, adding alpn extension"));
   ...
   * 7 . 7+n  protocol name
   */
  MBEDTLS2_PUT_UINT16_BE(MBEDTLS2_TLS_EXT_ALPN, buf, 0);
...
static void ssl_write_use_srtp_ext(mbedtls2_ssl_context *ssl,
                                   unsigned char *buf, size_t *olen) {
  ...
  *olen = 0;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(3, ("server hello, adding use_srtp extension"));
  ...
  /* extension */
  MBEDTLS2_PUT_UINT16_BE(MBEDTLS2_TLS_EXT_USE_SRTP, buf, 0);
  /*
   * total length 5 and mki value: only one profile(2 bytes)
   *              and length(2 bytes) and srtp_mki  )
   */
  ext_len = 5 + mki_len;
  MBEDTLS2_PUT_UINT16_BE(ext_len, buf, 2);
  ...
  if (profile_value != MBEDTLS2_TLS_SRTP_UNSET) {
    MBEDTLS2_PUT_UINT16_BE(profile_value, buf, 6);
  } else {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("use_srtp extension invalid profile"));
    return;
  }
...
#if defined(MBEDTLS2_SSL_DTLS_HELLO_VERIFY)
static int ssl_write_hello_verify_request(mbedtls2_ssl_context *ssl) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  unsigned char *p = ssl->out_msg + 4;
  unsigned char *cookie_len_byte;

  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write hello verify request"));
  ...
  if (ssl->conf->f_cookie_write == NULL) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("inconsistent cookie callbacks"));
    return (MBEDTLS2_ERR_SSL_INTERNAL_ERROR);
  }
  ...
  if ((ret = ssl->conf->f_cookie_write(ssl->conf->p_cookie, &p,
                                       ...
                                       ssl->cli_id, ssl->cli_id_len)) != 0) {
    MBEDTLS2_SSL_DEBUG_RET(1, "f_cookie_write", ret);
    return (ret);
  }

  *cookie_len_byte = (unsigned char)(p - (cookie_len_byte + 1));

  MBEDTLS2_SSL_DEBUG_BUF(3, "cookie sent", cookie_len_byte + 1,
                                *cookie_len_byte);

  ssl->out_msglen = p - ssl->out_msg;
  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_HELLO_VERIFY_REQUEST;

  ssl->state = MBEDTLS2_SSL_SERVER_HELLO_VERIFY_REQUEST_SENT;
...
static void
ssl_handle_id_based_session_resumption(mbedtls2_ssl_context *ssl) {
  int ret;
  mbedtls2_ssl_session session_tmp;
  mbedtls2_ssl_session *const session = ssl->session_negotiate;
  ...
  mbedtls2_ssl_session_init(&session_tmp);

  session_tmp.id_len = session->id_len;
  memcpy(session_tmp.id, session->id, session->id_len);

  ret = ssl->conf->f_get_cache(ssl->conf->p_cache, &session_tmp);
  if (ret != 0)
    goto exit;
...
exit:
  ...
  mbedtls2_ssl_session_free(&session_tmp);
}

static int ssl_write_server_hello(mbedtls2_ssl_context *ssl) {
#if defined(MBEDTLS2_HAVE_TIME)
  mbedtls2_time_t t;
#endif
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write server hello"));

#if defined(MBEDTLS2_SSL_DTLS_HELLO_VERIFY)
  if (ssl->conf->transport == MBEDTLS2_SSL_TRANSPORT_DATAGRAM &&
      ssl->handshake->verify_cookie_len != 0) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("client hello was not authenticated"));
    MBEDTLS2_SSL_DEBUG_MSG(2, ("<= write server hello"));
  ...
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, current time: %" MBEDTLS2_PRINTF_LONGLONG,
          (long long)t));
#else
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "server hello, random bytes", buf + 6, 32);
  ...
  *p++ = (unsigned char)ssl->session_negotiate->id_len;
  ...
  MBEDTLS2_SSL_DEBUG_BUF(3, "server hello, session id", buf + 39, n);
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("%s session has been resumed", ssl->handshake->resume ? "a" : "no"));

  MBEDTLS2_PUT_UINT16_BE(ssl->session_negotiate->ciphersuite, p, 0);
  p += 2;
  *p++ = MBEDTLS2_BYTE_0(ssl->session_negotiate->compression);

  MBEDTLS2_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %s",
                                    mbedtls2_ssl_get_ciphersuite_name(
                                        ssl->session_negotiate->ciphersuite)));
  MBEDTLS2_SSL_DEBUG_MSG(
      3, ("server hello, compress alg.: 0x%02X",
...
#if defined(MBEDTLS2_ECDH_C) || defined(MBEDTLS2_ECDSA_C) ||     \
    defined(MBEDTLS2_KEY_EXCHANGE_ECJPAKE_ENABLED)
    if (mbedtls2_ssl_ciphersuite_uses_ec(
            mbedtls2_ssl_ciphersuite_from_id(
                ssl->session_negotiate->ciphersuite))) {
  ...
  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_SERVER_HELLO;

  ret = mbedtls2_ssl_write_handshake_msg(ssl);
...
static int ssl_write_certificate_request(mbedtls2_ssl_context *ssl) {
  const mbedtls2_ssl_ciphersuite_t *ciphersuite_info =
      ssl->handshake->ciphersuite_info;

  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write certificate request"));

  if (!mbedtls2_ssl_ciphersuite_cert_req_allowed(ciphersuite_info)) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("<= skip write certificate request"));
    ssl->state++;
    return (0);
  }
...
static int ssl_write_certificate_request(mbedtls2_ssl_context *ssl) {
  ...
  const unsigned char *const end =
      ssl->out_msg + MBEDTLS2_SSL_OUT_CONTENT_LEN;
...
#if defined(MBEDTLS2_SSL_SERVER_NAME_INDICATION)
  if (ssl->handshake->sni_authmode != MBEDTLS2_SSL_VERIFY_UNSET)
  ...
  if (!mbedtls2_ssl_ciphersuite_cert_req_allowed(ciphersuite_info) ||
      authmode == MBEDTLS2_SSL_VERIFY_NONE) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("<= skip write certificate request"));
    return (0);
  }
  ...
  p[1 + ct_len++] = MBEDTLS2_SSL_CERT_TYPE_RSA_SIGN;
#endif
#if defined(MBEDTLS2_ECDSA_C)
  p[1 + ct_len++] = MBEDTLS2_SSL_CERT_TYPE_ECDSA_SIGN;
#endif
  ...
  if (ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_3) {
    ...
    for (cur = ssl->conf->sig_hashes; *cur != MBEDTLS2_MD_NONE; cur++) {
      unsigned char hash = mbedtls2_ssl_hash_from_md_alg(*cur);

      if (MBEDTLS2_SSL_HASH_NONE == hash ||
          mbedtls2_ssl_set_calc_verify_md(ssl, hash))
        continue;

#if defined(MBEDTLS2_RSA_C)
      p[2 + sa_len++] = hash;
      p[2 + sa_len++] = MBEDTLS2_SSL_SIG_RSA;
#endif
#if defined(MBEDTLS2_ECDSA_C)
      p[2 + sa_len++] = hash;
      p[2 + sa_len++] = MBEDTLS2_SSL_SIG_ECDSA;
#endif
    }

    MBEDTLS2_PUT_UINT16_BE(sa_len, p, 0);
    sa_len += 2;
    p += sa_len;
  }
#endif /* MBEDTLS2_SSL_PROTO_TLS1_2 */
  ...
  if (ssl->conf->cert_req_ca_list ==
      MBEDTLS2_SSL_CERT_REQ_CA_LIST_ENABLED) {
    /* NOTE: If trusted certificates are provisioned
     *       via a CA callback (configured through
     *       `mbedtls2_ssl_conf_ca_cb()`, then the
    ...
    while (crt != NULL && crt->version != 0) {
      ...
      if (end < p || (size_t)(end - p) < 2 + (size_t)dn_size) {
        MBEDTLS2_SSL_DEBUG_MSG(1, ("skipping CAs: buffer too short"));
        break;
      }

      MBEDTLS2_PUT_UINT16_BE(dn_size, p, 0);
      p += 2;
      memcpy(p, crt->subject_raw.p, dn_size);
      p += dn_size;

      MBEDTLS2_SSL_DEBUG_BUF(3, "requested DN", p - dn_size, dn_size);
  ...
  ssl->out_msglen = p - buf;
  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_CERTIFICATE_REQUEST;
  MBEDTLS2_PUT_UINT16_BE(total_dn_size, ssl->out_msg,
                                4 + ct_len + sa_len);
...
static int ssl_get_ecdh_params_from_cert(mbedtls2_ssl_context *ssl) {
  ...
  if ((ret = mbedtls2_ecdh_get_params(
           &ssl->handshake->ecdh_ctx,
           mbedtls2_pk_ec(*mbedtls2_ssl_own_key(ssl)),
           MBEDTLS2_ECDH_OURS)) != 0) {
  ...
  return (0);
    ...
    defined(MBEDTLS2_SSL_ASYNC_PRIVATE)
static int ssl_resume_server_key_exchange(mbedtls2_ssl_context *ssl,
  ...
  size_t sig_max_len =
      (ssl->out_buf + MBEDTLS2_SSL_OUT_CONTENT_LEN - sig_start);
  int ret =
      ssl->conf->f_async_resume(ssl, sig_start, signature_len, sig_max_len);
  if (ret != MBEDTLS2_ERR_SSL_ASYNC_IN_PROGRESS) {
    ssl->handshake->async_in_progress = 0;
    mbedtls2_ssl_set_async_operation_data(ssl, NULL);
  }
  MBEDTLS2_SSL_DEBUG_RET(2, "ssl_resume_server_key_exchange", ret);
  return (ret);
}
#endif /* defined(MBEDTLS2_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED)  \
          && defined(MBEDTLS2_SSL_ASYNC_PRIVATE) */

/* Prepare the ServerKeyExchange message, up to and including
 * calculating the signature if any, but excluding formatting the
 * signature and sending the message. */
static int ssl_prepare_server_key_exchange(mbedtls2_ssl_context *ssl,
                                           size_t *signature_len) {
  const mbedtls2_ssl_ciphersuite_t *ciphersuite_info =
      ssl->handshake->ciphersuite_info;

#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_PFS_ENABLED)
#if defined(MBEDTLS2_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED)
  unsigned char *dig_signed = NULL;
#endif /* MBEDTLS2_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED */
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_PFS_ENABLED */
   ...
   */
#if defined(MBEDTLS2_KEY_EXCHANGE_ECJPAKE_ENABLED)
  if (ciphersuite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_ECJPAKE) {
    int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;

    ret = mbedtls2_ecjpake_write_round_two(
        &ssl->handshake->ecjpake_ctx, ssl->out_msg + ssl->out_msglen,
        MBEDTLS2_SSL_OUT_CONTENT_LEN - ssl->out_msglen, &len,
        ssl->conf->f_rng, ssl->conf->p_rng);
    if (ret != 0) {
  ...
  if (ciphersuite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_DHE_PSK ||
   ...
   * - DHE key exchanges
   */
#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_DHE_ENABLED)
  if (mbedtls2_ssl_ciphersuite_uses_dhe(ciphersuite_info)) {
    int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;

    if (ssl->conf->dhm_P.p == NULL || ssl->conf->dhm_G.p == NULL) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("no DH parameters set"));
      return (MBEDTLS2_ERR_SSL_BAD_INPUT_DATA);
    }
     ...
     * } ServerDHParams;
     */
    if ((ret = mbedtls2_dhm_set_group(&ssl->handshake->dhm_ctx,
                                             &ssl->conf->dhm_P,
                                             &ssl->conf->dhm_G)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_dhm_set_group", ret);
      return (ret);
    }

    if ((ret = mbedtls2_dhm_make_params(
             &ssl->handshake->dhm_ctx,
             (int)mbedtls2_mpi_size(&ssl->handshake->dhm_ctx.P),
             ssl->out_msg + ssl->out_msglen, &len, ssl->conf->f_rng,
             ssl->conf->p_rng)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_dhm_make_params", ret);
      return (ret);
    }
...
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_DHE_ENABLED */

  /*
   * - ECDHE key exchanges
   */
#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_ECDHE_ENABLED)
  if (mbedtls2_ssl_ciphersuite_uses_ecdhe(ciphersuite_info)) {
    /*
     ...
     * } ServerECDHParams;
     */
    const mbedtls2_ecp_curve_info **curve = NULL;
    const mbedtls2_ecp_group_id *gid;
    int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;

    /* Match our preference list against the offered curves */
    for (gid = ssl->conf->curve_list; *gid != MBEDTLS2_ECP_DP_NONE;
         gid++)
  ...
  curve_matching_done:
    if (curve == NULL || *curve == NULL) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("no matching curve for ECDHE"));
      return (MBEDTLS2_ERR_SSL_NO_CIPHER_CHOSEN);
    }

    MBEDTLS2_SSL_DEBUG_MSG(2, ("ECDHE curve: %s", (*curve)->name));

    if ((ret = mbedtls2_ecdh_setup(&ssl->handshake->ecdh_ctx,
                                          (*curve)->grp_id)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ecp_group_load", ret);
      return (ret);
    }

    if ((ret = mbedtls2_ecdh_make_params(
             &ssl->handshake->ecdh_ctx, &len, ssl->out_msg + ssl->out_msglen,
             MBEDTLS2_SSL_OUT_CONTENT_LEN - ssl->out_msglen,
             ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ecdh_make_params", ret);
      return (ret);
    }
...
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_ECDHE_ENABLED */
  ...
  if (mbedtls2_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
    ...
    if (ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_3) {
      ...
      if (sig_alg == MBEDTLS2_PK_NONE ||
          (md_alg = mbedtls2_ssl_sig_hash_set_find(
               &ssl->handshake->hash_algs, sig_alg)) ==
              MBEDTLS2_MD_NONE) {
        MBEDTLS2_SSL_DEBUG_MSG(1, ("should never happen"));
    ...
    MBEDTLS2_SSL_DEBUG_MSG(
        3, ("pick hash algorithm %u for signing", (unsigned)md_alg));
    ...
    if (md_alg == MBEDTLS2_MD_NONE) {
      hashlen = 36;
      ret = mbedtls2_ssl_get_key_exchange_md_ssl_tls(
          ssl, hash, dig_signed, dig_signed_len);
      if (ret != 0)
        return (ret);
    } else
#endif /* MBEDTLS2_SSL_PROTO_SSL3 || MBEDTLS2_SSL_PROTO_TLS1 ||  \
    ...
    if (ssl->minor_ver == MBEDTLS2_SSL_MINOR_VERSION_3) {
      ...
      ssl->out_msg[ssl->out_msglen++] =
          mbedtls2_ssl_hash_from_md_alg(md_alg);
      ssl->out_msg[ssl->out_msglen++] =
          mbedtls2_ssl_sig_from_pk_alg(sig_alg);
    }
#endif /* MBEDTLS2_SSL_PROTO_TLS1_2 */
    ...
    if (ssl->conf->f_async_sign_start != NULL) {
      ret = ssl->conf->f_async_sign_start(
          ssl, mbedtls2_ssl_own_cert(ssl), md_alg, hash, hashlen);
      switch (ret) {
      case MBEDTLS2_ERR_SSL_HW_ACCEL_FALLTHROUGH:
        /* act as if f_async_sign was null */
        break;
      case 0:
        ssl->handshake->async_in_progress = 1;
        return (ssl_resume_server_key_exchange(ssl, signature_len));
      case MBEDTLS2_ERR_SSL_ASYNC_IN_PROGRESS:
        ssl->handshake->async_in_progress = 1;
        return (MBEDTLS2_ERR_SSL_ASYNC_IN_PROGRESS);
      default:
        MBEDTLS2_SSL_DEBUG_RET(1, "f_async_sign_start", ret);
        return (ret);
      }
    }
    ...
    if (mbedtls2_ssl_own_key(ssl) == NULL) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("got no private key"));
     ...
     * ssl_write_server_key_exchange also takes care of incrementing
     * ssl->out_msglen. */
    if ((ret = mbedtls2_pk_sign(
             mbedtls2_ssl_own_key(ssl), md_alg, hash, hashlen,
 ...
 * machine. */
static int ssl_write_server_key_exchange(mbedtls2_ssl_context *ssl) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  size_t signature_len = 0;
#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_NON_PFS_ENABLED)
  const mbedtls2_ssl_ciphersuite_t *ciphersuite_info =
      ssl->handshake->ciphersuite_info;
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_NON_PFS_ENABLED */

  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write server key exchange"));

#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_NON_PFS_ENABLED)
  /* Extract static ECDH parameters and abort if ServerKeyExchange
   * is not needed. */
  if (mbedtls2_ssl_ciphersuite_no_pfs(ciphersuite_info)) {
    /* For suites involving ECDH, extract DH parameters
     * from certificate at this point. */
#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_ECDH_ENABLED)
    if (mbedtls2_ssl_ciphersuite_uses_ecdh(ciphersuite_info)) {
      ssl_get_ecdh_params_from_cert(ssl);
    }
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_ECDH_ENABLED */

    /* Key exchanges not involving ephemeral keys don't use
     * ServerKeyExchange, so end here. */
    MBEDTLS2_SSL_DEBUG_MSG(2, ("<= skip write server key exchange"));
    ssl->state++;
    return (0);
  }
#endif /* MBEDTLS2_KEY_EXCHANGE_SOME_NON_PFS_ENABLED */
  ...
  if (ssl->handshake->async_in_progress != 0) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("resuming signature operation"));
...
#if defined(MBEDTLS2_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED)
  if (signature_len != 0) {
    ssl->out_msg[ssl->out_msglen++] = MBEDTLS2_BYTE_1(signature_len);
    ssl->out_msg[ssl->out_msglen++] = MBEDTLS2_BYTE_0(signature_len);

    MBEDTLS2_SSL_DEBUG_BUF(
        3, "my signature", ssl->out_msg + ssl->out_msglen, signature_len);
  ...
  /* Add header and send. */
  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_SERVER_KEY_EXCHANGE;
...
static int ssl_write_server_hello_done(mbedtls2_ssl_context *ssl) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;

  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write server hello done"));

  ssl->out_msglen = 4;
  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_SERVER_HELLO_DONE;
...
static int ssl_parse_client_dh_public(mbedtls2_ssl_context *ssl,
                                      unsigned char **p,
   ...
   */
  if (*p + 2 > end) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client key exchange message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE);
  }
  ...
  *p += 2;
  ...
  if ((ret = mbedtls2_dhm_read_public(&ssl->handshake->dhm_ctx, *p,
                                             n)) != 0) {
    MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_dhm_read_public", ret);
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP);
  }
...
#if defined(MBEDTLS2_SSL_ASYNC_PRIVATE)
static int ssl_resume_decrypt_pms(mbedtls2_ssl_context *ssl,
  ...
  MBEDTLS2_SSL_DEBUG_RET(2, "ssl_decrypt_encrypted_pms", ret);
  return (ret);
}
#endif /* MBEDTLS2_SSL_ASYNC_PRIVATE */

static int ssl_decrypt_encrypted_pms(mbedtls2_ssl_context *ssl,
                                     const unsigned char *p,
                                     const unsigned char *end,
                                     unsigned char *peer_pms,
                                     size_t *peer_pmslen, size_t peer_pmssize) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_pk_context *private_key = mbedtls2_ssl_own_key(ssl);
  mbedtls2_pk_context *public_key =
      &mbedtls2_ssl_own_cert(ssl)->pk;
  size_t len = mbedtls2_pk_get_len(public_key);

#if defined(MBEDTLS2_SSL_ASYNC_PRIVATE)
  /* If we have already started decoding the message and there is an ongoing
   * decryption operation, resume signing. */
  if (ssl->handshake->async_in_progress != 0) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("resuming decryption operation"));
  ...
  if (ssl->minor_ver != MBEDTLS2_SSL_MINOR_VERSION_0) {
    ...
    if (*p++ != MBEDTLS2_BYTE_1(len) ||
        *p++ != MBEDTLS2_BYTE_0(len)) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client key exchange message"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE);
    }
  }
  ...
  if (ssl->conf->f_async_decrypt_start != NULL) {
    ret = ssl->conf->f_async_decrypt_start(
        ssl, mbedtls2_ssl_own_cert(ssl), p, len);
    switch (ret) {
    case MBEDTLS2_ERR_SSL_HW_ACCEL_FALLTHROUGH:
    ...
    case MBEDTLS2_ERR_SSL_ASYNC_IN_PROGRESS:
      ...
      return (MBEDTLS2_ERR_SSL_ASYNC_IN_PROGRESS);
    default:
      MBEDTLS2_SSL_DEBUG_RET(1, "f_async_decrypt_start", ret);
      return (ret);
    }
  }
#endif /* MBEDTLS2_SSL_ASYNC_PRIVATE */

  if (!mbedtls2_pk_can_do(private_key, MBEDTLS2_PK_RSA)) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("got no RSA private key"));
    return (MBEDTLS2_ERR_SSL_PRIVATE_KEY_REQUIRED);
  }

  ret = mbedtls2_pk_decrypt(private_key, p, len, peer_pms, peer_pmslen,
                                   peer_pmssize, ssl->conf->f_rng,
                                   ssl->conf->p_rng);
  return (ret);
}

static int ssl_parse_encrypted_pms(mbedtls2_ssl_context *ssl,
                                   const unsigned char *p,
                                   const unsigned char *end,
                                   size_t pms_offset) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  mbedtls2_ssl_write_version(ssl->handshake->max_major_ver,
  ...
  mask = mbedtls2_ct_uint_mask(diff);
...
#if defined(MBEDTLS2_KEY_EXCHANGE_SOME_PSK_ENABLED)
static int ssl_parse_client_psk_identity(mbedtls2_ssl_context *ssl,
                                         unsigned char **p,
                                         const unsigned char *end) {
  int ret = 0;
  uint16_t n;
  ...
  if (ssl_conf_has_psk_or_cb(ssl->conf) == 0) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("got no pre-shared key"));
    return (MBEDTLS2_ERR_SSL_PRIVATE_KEY_REQUIRED);
  }
  ...
  *p += 2;
  ...
  if (ssl->conf->f_psk != NULL) {
    if (ssl->conf->f_psk(ssl->conf->p_psk, ssl, *p, n) != 0)
      ret = MBEDTLS2_ERR_SSL_UNKNOWN_IDENTITY;
  } else {
    /* Identity is not a big secret since clients send it in the clear,
     * but treat it carefully anyway, just in case */
    if (n != ssl->conf->psk_identity_len ||
        mbedtls2_ct_memcmp(ssl->conf->psk_identity, *p, n) != 0) {
      ret = MBEDTLS2_ERR_SSL_UNKNOWN_IDENTITY;
    }
  }

  if (ret == MBEDTLS2_ERR_SSL_UNKNOWN_IDENTITY) {
    MBEDTLS2_SSL_DEBUG_BUF(3, "Unknown PSK identity", *p, n);
    mbedtls2_ssl_send_alert_message(
        ssl, MBEDTLS2_SSL_ALERT_LEVEL_FATAL,
        MBEDTLS2_SSL_ALERT_MSG_UNKNOWN_PSK_IDENTITY);
    return (MBEDTLS2_ERR_SSL_UNKNOWN_IDENTITY);
  }
...
static int ssl_parse_client_key_exchange(mbedtls2_ssl_context *ssl) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  const mbedtls2_ssl_ciphersuite_t *ciphersuite_info;
  unsigned char *p, *end;
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> parse client key exchange"));

#if defined(MBEDTLS2_SSL_ASYNC_PRIVATE) &&                              \
    (defined(MBEDTLS2_KEY_EXCHANGE_RSA_ENABLED) ||                      \
     defined(MBEDTLS2_KEY_EXCHANGE_RSA_PSK_ENABLED))
  if ((ciphersuite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_RSA_PSK ||
       ciphersuite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_RSA) &&
      (ssl->handshake->async_in_progress != 0)) {
  ...
  if (ssl->in_msg[0] != MBEDTLS2_SSL_HS_CLIENT_KEY_EXCHANGE) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client key exchange message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE);
  }
  ...
  if (ciphersuite_info->key_exchange == MBEDTLS2_KEY_EXCHANGE_DHE_RSA) {
    if ((ret = ssl_parse_client_dh_public(ssl, &p, end)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, ("ssl_parse_client_dh_public"), ret);
      return (ret);
    }

    if (p != end) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("bad client key exchange"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE);
    }
    ...
    if ((ret = mbedtls2_dhm_calc_secret(
      ...
      return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_CS);
    }
    ...
    if ((ret = mbedtls2_ecdh_read_public(&ssl->handshake->ecdh_ctx, p,
                                                end - p)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ecdh_read_public", ret);
      return (MBEDTLS2_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP);
    }
    ...
    if ((ret = ssl_parse_client_psk_identity(ssl, &p, end)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, ("ssl_parse_client_psk_identity"), ret);
      return (ret);
    }
    ...
    if (ssl->handshake->async_in_progress != 0) {
      ...
      MBEDTLS2_SSL_DEBUG_MSG(3, ("PSK identity already parsed"));
...
#endif /* MBEDTLS2_SSL_ASYNC_PRIVATE */
      if ((ret = ssl_parse_client_psk_identity(ssl, &p, end)) != 0) {
        MBEDTLS2_SSL_DEBUG_RET(1, ("ssl_parse_client_psk_identity"),
                                      ret);
        return (ret);
      }
    ...
    if ((ret = ssl_parse_encrypted_pms(ssl, p, end, 2)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, ("ssl_parse_encrypted_pms"), ret);
      return (ret);
    }
    ...
    if ((ret = ssl_parse_encrypted_pms(ssl, p, end, 0)) != 0) {
      MBEDTLS2_SSL_DEBUG_RET(1, ("ssl_parse_parse_encrypted_pms_secret"),
                                    ret);
      return (ret);
    }
  } else
...
static int ssl_parse_certificate_verify(mbedtls2_ssl_context *ssl) {
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> parse certificate verify"));

  if (!mbedtls2_ssl_ciphersuite_cert_req_allowed(ciphersuite_info)) {
    MBEDTLS2_SSL_DEBUG_MSG(2, ("<= skip parse certificate verify"));
    ssl->state++;
    return (0);
  }
...
static int ssl_parse_certificate_verify(mbedtls2_ssl_context *ssl) {
  ...
  ret = mbedtls2_ssl_read_record(ssl, 0 /* no checksum update */);
  if (0 != ret) {
    MBEDTLS2_SSL_DEBUG_RET(1, ("mbedtls2_ssl_read_record"), ret);
    return (ret);
  }
  ...
  if (ssl->in_msgtype != MBEDTLS2_SSL_MSG_HANDSHAKE ||
      ssl->in_msg[0] != MBEDTLS2_SSL_HS_CERTIFICATE_VERIFY) {
    MBEDTLS2_SSL_DEBUG_MSG(1, ("bad certificate verify message"));
    return (MBEDTLS2_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY);
  }
  ...
  if (ssl->minor_ver != MBEDTLS2_SSL_MINOR_VERSION_3) {
    ...
    if (mbedtls2_pk_can_do(peer_pk, MBEDTLS2_PK_ECDSA)) {
      hash_start += 16;
      hashlen -= 16;
      md_alg = MBEDTLS2_MD_SHA1;
    }
     ...
     * Hash
     */
    md_alg = mbedtls2_ssl_md_alg_from_hash(ssl->in_msg[i]);

    if (md_alg == MBEDTLS2_MD_NONE ||
        mbedtls2_ssl_set_calc_verify_md(ssl, ssl->in_msg[i])) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
                                        " for verify message"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY);
    }

#if !defined(MBEDTLS2_MD_SHA1)
    if (MBEDTLS2_MD_SHA1 == md_alg)
      hash_start += 16;
#endif
     ...
     * Signature
     */
    if ((pk_alg = mbedtls2_ssl_pk_alg_from_sig(ssl->in_msg[i])) ==
        MBEDTLS2_PK_NONE) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
                                        " for verify message"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY);
    ...
    if (!mbedtls2_pk_can_do(peer_pk, pk_alg)) {
      MBEDTLS2_SSL_DEBUG_MSG(1, ("sig_alg doesn't match cert key"));
      return (MBEDTLS2_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY);
    }
  ...
  if ((ret = mbedtls2_pk_verify(peer_pk, md_alg, hash_start, hashlen,
...
#if defined(MBEDTLS2_SSL_SESSION_TICKETS)
static int ssl_write_new_session_ticket(mbedtls2_ssl_context *ssl) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  size_t tlen;
  uint32_t lifetime;

  MBEDTLS2_SSL_DEBUG_MSG(2, ("=> write new session ticket"));

  ssl->out_msgtype = MBEDTLS2_SSL_MSG_HANDSHAKE;
  ssl->out_msg[0] = MBEDTLS2_SSL_HS_NEW_SESSION_TICKET;
  ...
  if ((ret = ssl->conf->f_ticket_write(
           ...
           ssl->out_msg + MBEDTLS2_SSL_OUT_CONTENT_LEN, &tlen,
           &lifetime)) != 0) {
    MBEDTLS2_SSL_DEBUG_RET(1, "mbedtls2_ssl_ticket_write", ret);
    tlen = 0;
  }

  MBEDTLS2_PUT_UINT32_BE(lifetime, ssl->out_msg, 4);
  MBEDTLS2_PUT_UINT16_BE(tlen, ssl->out_msg, 8);
 ...
 */
int mbedtls2_ssl_handshake_server_step(
  ...
  MBEDTLS2_SSL_DEBUG_MSG(2, ("server state: %d", ssl->state));
...
#if defined(MBEDTLS2_SSL_PROTO_DTLS)
  case MBEDTLS2_SSL_SERVER_HELLO_VERIFY_REQUEST_SENT:
