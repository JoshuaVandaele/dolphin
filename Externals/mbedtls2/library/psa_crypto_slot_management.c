...
typedef struct {
  psa_key_slot_t key_slots[MBEDTLS2_PSA_KEY_SLOT_COUNT];
  unsigned key_slots_initialized : 1;
} psa_global_data_t;

static psa_global_data_t global_data;

int psa_is_valid_key_id(mbedtls2_svc_key_id_t key, int vendor_ok) {
  psa_key_id_t key_id = MBEDTLS2_SVC_KEY_ID_GET_KEY_ID(key);
 ...
 */
static psa_status_t
psa_get_and_lock_key_slot_in_memory(mbedtls2_svc_key_id_t key,
  ...
  if (psa_key_id_is_volatile(key_id)) {
     ...
     */
    status = mbedtls2_svc_key_id_equal(key, slot->attr.id)
                 ? PSA_SUCCESS
    ...
    for (slot_idx = 0; slot_idx < MBEDTLS2_PSA_KEY_SLOT_COUNT;
         slot_idx++) {
      slot = &global_data.key_slots[slot_idx];
      if (mbedtls2_svc_key_id_equal(key, slot->attr.id))
        break;
    }
    status = (slot_idx < MBEDTLS2_PSA_KEY_SLOT_COUNT)
                 ? PSA_SUCCESS
                 : PSA_ERROR_DOES_NOT_EXIST;
  }
...
static psa_status_t psa_load_builtin_key_into_slot(psa_key_slot_t *slot) {
  ...
  if (!psa_key_id_is_builtin(
          MBEDTLS2_SVC_KEY_ID_GET_KEY_ID(slot->attr.id))) {
    return (PSA_ERROR_DOES_NOT_EXIST);
  }

  /* Check the platform function to see whether this key actually exists */
  status = mbedtls2_psa_platform_get_builtin_key(
  ...
  status = psa_driver_wrapper_get_builtin_key(slot_number, &attributes, NULL, 0,
                                              NULL);
...
psa_status_t psa_get_and_lock_key_slot(mbedtls2_svc_key_id_t key,
                                       psa_key_slot_t **p_slot) {
  psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
  ...
  *p_slot = NULL;
...
#if defined(MBEDTLS2_PSA_CRYPTO_STORAGE_C) ||                           \
    defined(MBEDTLS2_PSA_CRYPTO_BUILTIN_KEYS)
  ...
  return (status);
#else  /* MBEDTLS2_PSA_CRYPTO_STORAGE_C ||                              \
...
psa_status_t psa_unlock_key_slot(psa_key_slot_t *slot) {
   ...
   * available call MBEDTLS2_PARAM_FAILED that may terminate execution
   * (if called as part of the execution of a unit test suite this will stop the
   * test suite execution).
   */
#ifdef MBEDTLS2_CHECK_PARAMS
  MBEDTLS2_PARAM_FAILED(slot->lock_count > 0);
#endif
...
psa_status_t psa_validate_key_location(psa_key_lifetime_t lifetime,
  ...
  if (psa_key_lifetime_is_external(lifetime)) {
    ...
    return (PSA_SUCCESS);
#else  /* MBEDTLS2_PSA_CRYPTO_DRIVERS */
...
psa_status_t psa_purge_key(mbedtls2_svc_key_id_t key) {
  psa_status_t status;
  psa_key_slot_t *slot;
...
void mbedtls2_psa_get_stats(mbedtls2_psa_stats_t *stats) {
  ...
  for (slot_idx = 0; slot_idx < MBEDTLS2_PSA_KEY_SLOT_COUNT;
    ...
    if (PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime))
      ...
      psa_key_id_t id = MBEDTLS2_SVC_KEY_ID_GET_KEY_ID(slot->attr.id);
