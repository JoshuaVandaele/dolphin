...
void mbedtls2_md2_free(mbedtls2_md2_context *ctx) {
  ...
  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_md2_context));
}
...
void mbedtls2_md2_process(mbedtls2_md2_context *ctx) {
  mbedtls2_internal_md2_process(ctx);
}
#endif
...
int mbedtls2_md2_update_ret(mbedtls2_md2_context *ctx,
  ...
  while (ilen > 0) {
    ...
    if (ctx->left == 16) {
      ctx->left = 0;
      if ((ret = mbedtls2_internal_md2_process(ctx)) != 0)
        return (ret);
    }
  }
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md2_update(mbedtls2_md2_context *ctx,
                                const unsigned char *input, size_t ilen) {
  mbedtls2_md2_update_ret(ctx, input, ilen);
}
#endif
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md2_finish(mbedtls2_md2_context *ctx,
                                unsigned char output[16]) {
  mbedtls2_md2_finish_ret(ctx, output);
}
#endif
...
int mbedtls2_md2_ret(const unsigned char *input, size_t ilen,
  ...
  if ((ret = mbedtls2_md2_starts_ret(&ctx)) != 0)
    goto exit;

  if ((ret = mbedtls2_md2_update_ret(&ctx, input, ilen)) != 0)
    goto exit;

  if ((ret = mbedtls2_md2_finish_ret(&ctx, output)) != 0)
    goto exit;
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md2(const unsigned char *input, size_t ilen,
                         unsigned char output[16]) {
  mbedtls2_md2_ret(input, ilen, output);
}
#endif
...
int mbedtls2_md2_self_test(int verbose) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    ret = mbedtls2_md2_ret(md2_test_str[i], md2_test_strlen[i], md2sum);
    if (ret != 0)
      goto fail;
