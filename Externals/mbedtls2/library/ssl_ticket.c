...
void mbedtls2_ssl_ticket_init(mbedtls2_ssl_ticket_context *ctx) {
  memset(ctx, 0, sizeof(mbedtls2_ssl_ticket_context));
 ...
 */
static int ssl_ticket_gen_key(mbedtls2_ssl_ticket_context *ctx,
                              unsigned char index) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  unsigned char buf[MAX_KEY_BYTES];
  mbedtls2_ssl_ticket_key *key = ctx->keys + index;

#if defined(MBEDTLS2_HAVE_TIME)
  key->generation_time = (uint32_t)mbedtls2_time(NULL);
#endif
  ...
  ret = mbedtls2_cipher_setkey(
      &key->ctx, buf, mbedtls2_cipher_get_key_bitlen(&key->ctx),
      MBEDTLS2_ENCRYPT);
 ...
 */
static int ssl_ticket_update_keys(mbedtls2_ssl_ticket_context *ctx) {
#if !defined(MBEDTLS2_HAVE_TIME)
  ((void)ctx);
#else
  if (ctx->ticket_lifetime != 0) {
    uint32_t current_time = (uint32_t)mbedtls2_time(NULL);
...
int mbedtls2_ssl_ticket_setup(
  ...
  cipher_info = mbedtls2_cipher_info_from_type(cipher);
  if (cipher_info == NULL)
  ...
  ret = mbedtls2_cipher_setup_psa(&ctx->keys[0].ctx, cipher_info,
                                         TICKET_AUTH_TAG_BYTES);
  if (ret != 0 && ret != MBEDTLS2_ERR_CIPHER_FEATURE_UNAVAILABLE)
    return (ret);
  /* We don't yet expect to support all ciphers through PSA,
   * so allow fallback to ordinary mbedtls2_cipher_setup(). */
  if (ret == MBEDTLS2_ERR_CIPHER_FEATURE_UNAVAILABLE)
#endif /* MBEDTLS2_USE_PSA_CRYPTO */
    if ((ret = mbedtls2_cipher_setup(&ctx->keys[0].ctx, cipher_info)) !=
        0)
      return (ret);

#if defined(MBEDTLS2_USE_PSA_CRYPTO)
  ret = mbedtls2_cipher_setup_psa(&ctx->keys[1].ctx, cipher_info,
                                         TICKET_AUTH_TAG_BYTES);
  if (ret != 0 && ret != MBEDTLS2_ERR_CIPHER_FEATURE_UNAVAILABLE)
    return (ret);
  if (ret == MBEDTLS2_ERR_CIPHER_FEATURE_UNAVAILABLE)
#endif /* MBEDTLS2_USE_PSA_CRYPTO */
    if ((ret = mbedtls2_cipher_setup(&ctx->keys[1].ctx, cipher_info)) !=
        0)
      return (ret);
...
int mbedtls2_ssl_ticket_write(void *p_ticket,
                                     const mbedtls2_ssl_session *session,
                                     unsigned char *start,
                                     const unsigned char *end, size_t *tlen,
                                     uint32_t *ticket_lifetime) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ssl_ticket_context *ctx = p_ticket;
  mbedtls2_ssl_ticket_key *key;
  ...
  *tlen = 0;
  ...
  MBEDTLS2_SSL_CHK_BUF_PTR(start, end, TICKET_MIN_LEN);
  ...
  *ticket_lifetime = ctx->ticket_lifetime;
  ...
  /* Dump session state */
  if ((ret = mbedtls2_ssl_session_save(session, state, end - state,
                                              &clear_len)) != 0 ||
      (unsigned long)clear_len > 65535) {
    goto cleanup;
  }
  MBEDTLS2_PUT_UINT16_BE(clear_len, state_len_bytes, 0);
 ...
 */
static mbedtls2_ssl_ticket_key *
ssl_ticket_select_key(mbedtls2_ssl_ticket_context *ctx,
...
/*
 * Load session ticket (see mbedtls2_ssl_ticket_write for structure)
 */
int mbedtls2_ssl_ticket_parse(void *p_ticket,
  ...
  if (len != TICKET_MIN_LEN + enc_len) {
    ret = MBEDTLS2_ERR_SSL_BAD_INPUT_DATA;
    goto cleanup;
  }
  ...
  if ((key = ssl_ticket_select_key(ctx, key_name)) == NULL) {
    ...
    ret = MBEDTLS2_ERR_SSL_SESSION_TICKET_EXPIRED;
    goto cleanup;
  }
  ...
  if ((ret = mbedtls2_ssl_session_load(session, ticket, clear_len)) != 0)
    goto cleanup;

#if defined(MBEDTLS2_HAVE_TIME)
  {
    /* Check for expiration */
    mbedtls2_time_t current_time = mbedtls2_time(NULL);
...
void mbedtls2_ssl_ticket_free(mbedtls2_ssl_ticket_context *ctx) {
  mbedtls2_cipher_free(&ctx->keys[0].ctx);
  mbedtls2_cipher_free(&ctx->keys[1].ctx);
  ...
  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_ssl_ticket_context));
}
