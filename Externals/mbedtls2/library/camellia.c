...
#define CAMELLIA_VALIDATE_RET(cond)                                            \
  MBEDTLS2_INTERNAL_VALIDATE_RET(                                       \
      cond, MBEDTLS2_ERR_CAMELLIA_BAD_INPUT_DATA)
#define CAMELLIA_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)
...
#else /* MBEDTLS2_CAMELLIA_SMALL_MEMORY */
...
static void camellia_feistel(const uint32_t x[2], const uint32_t k[2],
  ...
  I0 = ((uint32_t)SBOX1(MBEDTLS2_BYTE_3(I0)) << 24) |
       ((uint32_t)SBOX2(MBEDTLS2_BYTE_2(I0)) << 16) |
       ((uint32_t)SBOX3(MBEDTLS2_BYTE_1(I0)) << 8) |
       ((uint32_t)SBOX4(MBEDTLS2_BYTE_0(I0)));
  I1 = ((uint32_t)SBOX2(MBEDTLS2_BYTE_3(I1)) << 24) |
       ((uint32_t)SBOX3(MBEDTLS2_BYTE_2(I1)) << 16) |
       ((uint32_t)SBOX4(MBEDTLS2_BYTE_1(I1)) << 8) |
       ((uint32_t)SBOX1(MBEDTLS2_BYTE_0(I1)));
...
void mbedtls2_camellia_init(mbedtls2_camellia_context *ctx) {
  CAMELLIA_VALIDATE(ctx != NULL);
  memset(ctx, 0, sizeof(mbedtls2_camellia_context));
}

void mbedtls2_camellia_free(mbedtls2_camellia_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_camellia_context));
}

/*
 * Camellia key schedule (encryption)
 */
int mbedtls2_camellia_setkey_enc(mbedtls2_camellia_context *ctx,
  ...
  case 256:
    ...
    break;
  default:
    return (MBEDTLS2_ERR_CAMELLIA_BAD_INPUT_DATA);
  }
   ...
   */
  for (i = 0; i < 6; i++) {
    SIGMA[i][0] = MBEDTLS2_GET_UINT32_BE(SIGMA_CHARS[i], 0);
    SIGMA[i][1] = MBEDTLS2_GET_UINT32_BE(SIGMA_CHARS[i], 4);
  }
  ...
  /* Store KL, KR */
  for (i = 0; i < 8; i++)
    KC[i] = MBEDTLS2_GET_UINT32_BE(t, i * 4);
 ...
 */
int mbedtls2_camellia_setkey_dec(mbedtls2_camellia_context *ctx,
                                        const unsigned char *key,
                                        unsigned int keybits) {
  int idx, ret;
  size_t i;
  mbedtls2_camellia_context cty;
  uint32_t *RK;
  uint32_t *SK;
  CAMELLIA_VALIDATE_RET(ctx != NULL);
  CAMELLIA_VALIDATE_RET(key != NULL);

  mbedtls2_camellia_init(&cty);

  /* Also checks keybits */
  if ((ret = mbedtls2_camellia_setkey_enc(&cty, key, keybits)) != 0)
    goto exit;
 ...
 */
int mbedtls2_camellia_crypt_ecb(mbedtls2_camellia_context *ctx,
                                       int mode, const unsigned char input[16],
                                       unsigned char output[16]) {
  int NR;
  uint32_t *RK, X[4];
  CAMELLIA_VALIDATE_RET(ctx != NULL);
  CAMELLIA_VALIDATE_RET(mode == MBEDTLS2_CAMELLIA_ENCRYPT ||
                        mode == MBEDTLS2_CAMELLIA_DECRYPT);
  ...
  X[0] = MBEDTLS2_GET_UINT32_BE(input, 0);
  X[1] = MBEDTLS2_GET_UINT32_BE(input, 4);
  X[2] = MBEDTLS2_GET_UINT32_BE(input, 8);
  X[3] = MBEDTLS2_GET_UINT32_BE(input, 12);
  ...
  MBEDTLS2_PUT_UINT32_BE(X[2], output, 0);
  MBEDTLS2_PUT_UINT32_BE(X[3], output, 4);
  MBEDTLS2_PUT_UINT32_BE(X[0], output, 8);
  MBEDTLS2_PUT_UINT32_BE(X[1], output, 12);
 ...
 */
int mbedtls2_camellia_crypt_cbc(mbedtls2_camellia_context *ctx,
  ...
  if (length % 16)
    return (MBEDTLS2_ERR_CAMELLIA_INVALID_INPUT_LENGTH);

  if (mode == MBEDTLS2_CAMELLIA_DECRYPT) {
    while (length > 0) {
      memcpy(temp, input, 16);
      mbedtls2_camellia_crypt_ecb(ctx, mode, input, output);
    ...
    while (length > 0) {
      ...
      mbedtls2_camellia_crypt_ecb(ctx, mode, output, output);
      memcpy(iv, output, 16);
 ...
 */
int mbedtls2_camellia_crypt_cfb128(mbedtls2_camellia_context *ctx,
                                          int mode, size_t length,
  ...
  if (mode == MBEDTLS2_CAMELLIA_DECRYPT) {
    while (length--) {
      if (n == 0)
        mbedtls2_camellia_crypt_ecb(
            ctx, MBEDTLS2_CAMELLIA_ENCRYPT, iv, iv);
 ...
 */
int mbedtls2_camellia_crypt_ctr(mbedtls2_camellia_context *ctx,
  ...
  while (length--) {
    if (n == 0) {
      mbedtls2_camellia_crypt_ecb(ctx, MBEDTLS2_CAMELLIA_ENCRYPT,
 ...
 * Checkup routine
 */
int mbedtls2_camellia_self_test(int verbose) {
  int i, j, u, v;
  ...
  for (j = 0; j < 6; j++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  CAMELLIA-ECB-%3d (%s): ", 128 + u * 64,
                             (v == MBEDTLS2_CAMELLIA_DECRYPT) ? "dec"
                                                                     : "enc");

    for (i = 0; i < CAMELLIA_TESTS_ECB; i++) {
      memcpy(key, camellia_test_ecb_key[u][i], 16 + 8 * u);

      if (v == MBEDTLS2_CAMELLIA_DECRYPT) {
        mbedtls2_camellia_setkey_dec(&ctx, key, 128 + u * 64);
        memcpy(src, camellia_test_ecb_cipher[u][i], 16);
        memcpy(dst, camellia_test_ecb_plain[i], 16);
      } else { /* MBEDTLS2_CAMELLIA_ENCRYPT */
        mbedtls2_camellia_setkey_enc(&ctx, key, 128 + u * 64);
        memcpy(src, camellia_test_ecb_plain[i], 16);
        memcpy(dst, camellia_test_ecb_cipher[u][i], 16);
      }

      mbedtls2_camellia_crypt_ecb(&ctx, v, src, buf);
  ...
  for (j = 0; j < 6; j++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  CAMELLIA-CBC-%3d (%s): ", 128 + u * 64,
                             (v == MBEDTLS2_CAMELLIA_DECRYPT) ? "dec"
                                                                     : "enc");
    ...
    for (i = 0; i < CAMELLIA_TESTS_CBC; i++) {
      ...
      mbedtls2_camellia_crypt_cbc(&ctx, v, 16, iv, src, buf);
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (verbose != 0)
      mbedtls2_printf("  CAMELLIA-CTR-128 (%s): ",
                             (v == MBEDTLS2_CAMELLIA_DECRYPT) ? "dec"
                                                                     : "enc");
    ...
    offset = 0;
    mbedtls2_camellia_setkey_enc(&ctx, key, 128);

    if (v == MBEDTLS2_CAMELLIA_DECRYPT) {
      len = camellia_test_ctr_len[u];
      memcpy(buf, camellia_test_ctr_ct[u], len);

      mbedtls2_camellia_crypt_ctr(&ctx, len, &offset, nonce_counter,
