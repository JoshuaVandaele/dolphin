...
#define ECJPAKE_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)

/*
 * Convert a mbedtls2_ecjpake_role to identifier string
 */
...
void mbedtls2_ecjpake_init(mbedtls2_ecjpake_context *ctx) {
  ...
  mbedtls2_ecp_point_init(&ctx->Xm1);
  mbedtls2_ecp_point_init(&ctx->Xm2);
  mbedtls2_ecp_point_init(&ctx->Xp1);
  mbedtls2_ecp_point_init(&ctx->Xp2);
  mbedtls2_ecp_point_init(&ctx->Xp);

  mbedtls2_mpi_init(&ctx->xm1);
  mbedtls2_mpi_init(&ctx->xm2);
  mbedtls2_mpi_init(&ctx->s);
}
...
void mbedtls2_ecjpake_free(mbedtls2_ecjpake_context *ctx) {
  ...
  mbedtls2_ecp_point_free(&ctx->Xm1);
  mbedtls2_ecp_point_free(&ctx->Xm2);
  mbedtls2_ecp_point_free(&ctx->Xp1);
  mbedtls2_ecp_point_free(&ctx->Xp2);
  mbedtls2_ecp_point_free(&ctx->Xp);

  mbedtls2_mpi_free(&ctx->xm1);
  mbedtls2_mpi_free(&ctx->xm2);
  mbedtls2_mpi_free(&ctx->s);
}
...
int mbedtls2_ecjpake_setup(mbedtls2_ecjpake_context *ctx,
  ...
  ECJPAKE_VALIDATE_RET(role == MBEDTLS2_ECJPAKE_CLIENT ||
                       role == MBEDTLS2_ECJPAKE_SERVER);
  ECJPAKE_VALIDATE_RET(secret != NULL || len == 0);

  ctx->role = role;

  if ((ctx->md_info = mbedtls2_md_info_from_type(hash)) == NULL)
    return (MBEDTLS2_ERR_MD_FEATURE_UNAVAILABLE);

  MBEDTLS2_MPI_CHK(mbedtls2_ecp_group_load(&ctx->grp, curve));

  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_read_binary(&ctx->s, secret, len));

cleanup:
  if (ret != 0)
    mbedtls2_ecjpake_free(ctx);
...
int mbedtls2_ecjpake_check(const mbedtls2_ecjpake_context *ctx) {
  ...
  if (ctx->md_info == NULL || ctx->grp.id == MBEDTLS2_ECP_DP_NONE ||
      ctx->s.p == NULL) {
...
static int ecjpake_write_len_point(unsigned char **p, const unsigned char *end,
  ...
  ret = mbedtls2_ecp_point_write_binary(grp, P, pf, &len, *p + 4,
                                               end - (*p + 4));
  if (ret != 0)
    return (ret);

  MBEDTLS2_PUT_UINT32_BE(len, *p, 0);
 ...
 */
#define ECJPAKE_HASH_BUF_LEN (3 * (4 + MBEDTLS2_ECP_MAX_PT_LEN) + 4 + 6)

/*
 * Compute hash for ZKP (7.4.2.2.2.1)
 */
static int ecjpake_hash(const mbedtls2_md_info_t *md_info,
                        const mbedtls2_ecp_group *grp, const int pf,
                        const mbedtls2_ecp_point *G,
                        const mbedtls2_ecp_point *V,
                        const mbedtls2_ecp_point *X, const char *id,
                        mbedtls2_mpi *h) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  MBEDTLS2_MPI_CHK(ecjpake_write_len_point(&p, end, grp, pf, G));
  MBEDTLS2_MPI_CHK(ecjpake_write_len_point(&p, end, grp, pf, V));
  MBEDTLS2_MPI_CHK(ecjpake_write_len_point(&p, end, grp, pf, X));

  if (end - p < 4)
    return (MBEDTLS2_ERR_ECP_BUFFER_TOO_SMALL);

  MBEDTLS2_PUT_UINT32_BE(id_len, p, 0);
  p += 4;
  ...
  /* Compute hash */
  MBEDTLS2_MPI_CHK(mbedtls2_md(md_info, buf, p - buf, hash));

  /* Turn it into an integer mod n */
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_read_binary(
      h, hash, mbedtls2_md_get_size(md_info)));
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mod_mpi(h, h, &grp->N));
 ...
 */
static int ecjpake_zkp_read(const mbedtls2_md_info_t *md_info,
                            const mbedtls2_ecp_group *grp, const int pf,
                            const mbedtls2_ecp_point *G,
                            const mbedtls2_ecp_point *X, const char *id,
                            const unsigned char **p, const unsigned char *end) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ecp_point V, VV;
  mbedtls2_mpi r, h;
  size_t r_len;

  mbedtls2_ecp_point_init(&V);
  mbedtls2_ecp_point_init(&VV);
  mbedtls2_mpi_init(&r);
  mbedtls2_mpi_init(&h);
  ...
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_tls_read_point(grp, &V, p, end - *p));
  ...
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_read_binary(&r, *p, r_len));
  *p += r_len;

  /*
   * Verification
   */
  MBEDTLS2_MPI_CHK(ecjpake_hash(md_info, grp, pf, G, &V, X, id, &h));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_muladd(
      (mbedtls2_ecp_group *)grp, &VV, &h, X, &r, G));

  if (mbedtls2_ecp_point_cmp(&VV, &V) != 0) {
    ret = MBEDTLS2_ERR_ECP_VERIFY_FAILED;
    goto cleanup;
  }

cleanup:
  mbedtls2_ecp_point_free(&V);
  mbedtls2_ecp_point_free(&VV);
  mbedtls2_mpi_free(&r);
  mbedtls2_mpi_free(&h);
 ...
 */
static int ecjpake_zkp_write(const mbedtls2_md_info_t *md_info,
                             const mbedtls2_ecp_group *grp, const int pf,
                             const mbedtls2_ecp_point *G,
                             const mbedtls2_mpi *x,
                             const mbedtls2_ecp_point *X, const char *id,
                             unsigned char **p, const unsigned char *end,
                             int (*f_rng)(void *, unsigned char *, size_t),
                             void *p_rng) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ecp_point V;
  mbedtls2_mpi v;
  mbedtls2_mpi h; /* later recycled to hold r */
  size_t len;
  ...
  /* Compute signature */
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_gen_keypair_base(
      (mbedtls2_ecp_group *)grp, G, &v, &V, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(ecjpake_hash(md_info, grp, pf, G, &V, X, id, &h));
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mul_mpi(&h, &h, x)); /* x*h */
  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_sub_mpi(&h, &v, &h)); /* v - x*h */
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mod_mpi(&h, &h, &grp->N)); /* r */

  /* Write it out */
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_tls_write_point(grp, &V, pf, &len, *p, end - *p));
  *p += len;

  len = mbedtls2_mpi_size(&h); /* actually r */
  if (end < *p || (size_t)(end - *p) < 1 + len || len > 255) {
    ret = MBEDTLS2_ERR_ECP_BUFFER_TOO_SMALL;
    goto cleanup;
  }

  *(*p)++ = MBEDTLS2_BYTE_0(len);
  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_write_binary(&h, *p, len)); /* r */
  *p += len;
 ...
 */
static int ecjpake_kkp_read(const mbedtls2_md_info_t *md_info,
                            const mbedtls2_ecp_group *grp, const int pf,
                            const mbedtls2_ecp_point *G,
                            mbedtls2_ecp_point *X, const char *id,
                            const unsigned char **p, const unsigned char *end) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
   ...
   */
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_tls_read_point(grp, X, p, end - *p));
  if (mbedtls2_ecp_is_zero(X)) {
    ret = MBEDTLS2_ERR_ECP_INVALID_KEY;
    goto cleanup;
  }

  MBEDTLS2_MPI_CHK(ecjpake_zkp_read(md_info, grp, pf, G, X, id, p, end));
 ...
 */
static int ecjpake_kkp_write(const mbedtls2_md_info_t *md_info,
                             const mbedtls2_ecp_group *grp, const int pf,
                             const mbedtls2_ecp_point *G,
                             mbedtls2_mpi *x,
                             mbedtls2_ecp_point *X, const char *id,
  ...
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_gen_keypair_base(
      (mbedtls2_ecp_group *)grp, G, x, X, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_tls_write_point(grp, X, pf, &len, *p, end - *p));
  *p += len;
 ...
 */
static int ecjpake_kkpp_read(const mbedtls2_md_info_t *md_info,
                             const mbedtls2_ecp_group *grp, const int pf,
                             const mbedtls2_ecp_point *G,
                             mbedtls2_ecp_point *Xa,
                             mbedtls2_ecp_point *Xb, const char *id,
   ...
   */
  MBEDTLS2_MPI_CHK(
      ecjpake_kkp_read(md_info, grp, pf, G, Xa, id, &p, end));
  MBEDTLS2_MPI_CHK(
      ecjpake_kkp_read(md_info, grp, pf, G, Xb, id, &p, end));
 ...
 */
static int
ecjpake_kkpp_write(const mbedtls2_md_info_t *md_info,
                   const mbedtls2_ecp_group *grp, const int pf,
                   const mbedtls2_ecp_point *G, mbedtls2_mpi *xm1,
                   mbedtls2_ecp_point *Xa, mbedtls2_mpi *xm2,
                   mbedtls2_ecp_point *Xb, const char *id,
  ...
  MBEDTLS2_MPI_CHK(ecjpake_kkp_write(md_info, grp, pf, G, xm1, Xa, id,
                                            &p, end, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(ecjpake_kkp_write(md_info, grp, pf, G, xm2, Xb, id,
                                            &p, end, f_rng, p_rng));
 ...
 */
int mbedtls2_ecjpake_write_round_one(
    mbedtls2_ecjpake_context *ctx, unsigned char *buf, size_t len,
 ...
 */
static int ecjpake_ecp_add3(mbedtls2_ecp_group *grp,
                            mbedtls2_ecp_point *R,
                            const mbedtls2_ecp_point *A,
                            const mbedtls2_ecp_point *B,
                            const mbedtls2_ecp_point *C) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_mpi one;

  mbedtls2_mpi_init(&one);

  MBEDTLS2_MPI_CHK(mbedtls2_mpi_lset(&one, 1));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_muladd(grp, R, &one, A, &one, B));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_muladd(grp, R, &one, R, &one, C));

cleanup:
  mbedtls2_mpi_free(&one);
...
int mbedtls2_ecjpake_read_round_two(mbedtls2_ecjpake_context *ctx,
  ...
  mbedtls2_ecp_point G; /* C: GB, S: GA */

  ECJPAKE_VALIDATE_RET(ctx != NULL);
  ECJPAKE_VALIDATE_RET(buf != NULL);

  mbedtls2_ecp_group_init(&grp);
  mbedtls2_ecp_point_init(&G);
   ...
   */
  if (ctx->role == MBEDTLS2_ECJPAKE_CLIENT) {
    MBEDTLS2_MPI_CHK(mbedtls2_ecp_tls_read_group(&grp, &p, len));
  ...
  MBEDTLS2_MPI_CHK(ecjpake_kkp_read(ctx->md_info, &ctx->grp,
                                           ctx->point_format, &G, &ctx->Xp,
                                           ID_PEER, &p, end));
 ...
 */
static int ecjpake_mul_secret(mbedtls2_mpi *R, int sign,
                              const mbedtls2_mpi *X,
                              const mbedtls2_mpi *S,
                              const mbedtls2_mpi *N,
                              int (*f_rng)(void *, unsigned char *, size_t),
                              void *p_rng) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_mpi b; /* Blinding value, then s + N * blinding */

  mbedtls2_mpi_init(&b);

  /* b = s + rnd-128-bit * N */
  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_fill_random(&b, 16, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mul_mpi(&b, &b, N));
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_add_mpi(&b, &b, S));

  /* R = sign * X * b mod N */
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mul_mpi(R, X, &b));
  R->s *= sign;
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_mod_mpi(R, R, N));
 ...
 */
int mbedtls2_ecjpake_write_round_two(
    mbedtls2_ecjpake_context *ctx, unsigned char *buf, size_t len,
    size_t *olen, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ecp_point G;  /* C: GA, S: GB */
  mbedtls2_ecp_point Xm; /* C: Xc, S: Xs */
  mbedtls2_mpi xm;       /* C: xc, S: xs */
  unsigned char *p = buf;
  const unsigned char *end = buf + len;
  size_t ec_len;
  ...
  mbedtls2_ecp_point_init(&G);
  mbedtls2_ecp_point_init(&Xm);
  mbedtls2_mpi_init(&xm);
  ...
  MBEDTLS2_MPI_CHK(ecjpake_mul_secret(&xm, 1, &ctx->xm2, &ctx->s,
                                             &ctx->grp.N, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_mul(&ctx->grp, &Xm, &xm, &G, f_rng, p_rng));
   ...
   * } Client/ServerECJPAKEParams;
   */
  if (ctx->role == MBEDTLS2_ECJPAKE_SERVER) {
    if (end < p) {
      ret = MBEDTLS2_ERR_ECP_BUFFER_TOO_SMALL;
      goto cleanup;
    }
    MBEDTLS2_MPI_CHK(
        mbedtls2_ecp_tls_write_group(&ctx->grp, &ec_len, p, end - p));
    p += ec_len;
  }
  ...
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_tls_write_point(
      &ctx->grp, &Xm, ctx->point_format, &ec_len, p, end - p));
  p += ec_len;

  MBEDTLS2_MPI_CHK(ecjpake_zkp_write(ctx->md_info, &ctx->grp,
                                            ctx->point_format, &G, &xm, &Xm,
...
cleanup:
  mbedtls2_ecp_point_free(&G);
  mbedtls2_ecp_point_free(&Xm);
  mbedtls2_mpi_free(&xm);
 ...
 */
int mbedtls2_ecjpake_derive_secret(
    mbedtls2_ecjpake_context *ctx, unsigned char *buf, size_t len,
    size_t *olen, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ecp_point K;
  mbedtls2_mpi m_xm2_s, one;
  unsigned char kx[MBEDTLS2_ECP_MAX_BYTES];
  size_t x_bytes;
  ...
  *olen = mbedtls2_md_get_size(ctx->md_info);
  if (len < *olen)
    return (MBEDTLS2_ERR_ECP_BUFFER_TOO_SMALL);

  mbedtls2_ecp_point_init(&K);
  mbedtls2_mpi_init(&m_xm2_s);
  mbedtls2_mpi_init(&one);
   ...
   */
  MBEDTLS2_MPI_CHK(ecjpake_mul_secret(&m_xm2_s, -1, &ctx->xm2, &ctx->s,
                                             &ctx->grp.N, f_rng, p_rng));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_muladd(
      &ctx->grp, &K, &one, &ctx->Xp, &m_xm2_s, &ctx->Xp2));
  MBEDTLS2_MPI_CHK(
      mbedtls2_ecp_mul(&ctx->grp, &K, &ctx->xm2, &K, f_rng, p_rng));

  /* PMS = SHA-256( K.X ) */
  x_bytes = (ctx->grp.pbits + 7) / 8;
  MBEDTLS2_MPI_CHK(mbedtls2_mpi_write_binary(&K.X, kx, x_bytes));
  MBEDTLS2_MPI_CHK(mbedtls2_md(ctx->md_info, kx, x_bytes, buf));

cleanup:
  mbedtls2_ecp_point_free(&K);
  mbedtls2_mpi_free(&m_xm2_s);
  mbedtls2_mpi_free(&one);
...
static int ecjpake_test_load(mbedtls2_ecjpake_context *ctx,
                             const unsigned char *xm1, size_t len1,
                             const unsigned char *xm2, size_t len2) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;

  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_read_binary(&ctx->xm1, xm1, len1));
  MBEDTLS2_MPI_CHK(
      mbedtls2_mpi_read_binary(&ctx->xm2, xm2, len2));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_mul(
      &ctx->grp, &ctx->Xm1, &ctx->xm1, &ctx->grp.G, NULL, NULL));
  MBEDTLS2_MPI_CHK(mbedtls2_ecp_mul(
      &ctx->grp, &ctx->Xm2, &ctx->xm2, &ctx->grp.G, NULL, NULL));
...
int mbedtls2_ecjpake_self_test(int verbose) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_ecjpake_context cli;
  mbedtls2_ecjpake_context srv;
  unsigned char buf[512], pms[32];
  size_t len, pmslen;

  mbedtls2_ecjpake_init(&cli);
  mbedtls2_ecjpake_init(&srv);

  if (verbose != 0)
    mbedtls2_printf("  ECJPAKE test #0 (setup): ");

  TEST_ASSERT(mbedtls2_ecjpake_setup(
                  &cli, MBEDTLS2_ECJPAKE_CLIENT,
                  MBEDTLS2_MD_SHA256, MBEDTLS2_ECP_DP_SECP256R1,
                  ecjpake_test_password, sizeof(ecjpake_test_password)) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_setup(
                  &srv, MBEDTLS2_ECJPAKE_SERVER,
                  MBEDTLS2_MD_SHA256, MBEDTLS2_ECP_DP_SECP256R1,
                  ecjpake_test_password, sizeof(ecjpake_test_password)) == 0);
  ...
  if (verbose != 0)
    mbedtls2_printf("  ECJPAKE test #1 (random handshake): ");

  TEST_ASSERT(mbedtls2_ecjpake_write_round_one(
                  &cli, buf, sizeof(buf), &len, ecjpake_lgc, NULL) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_one(&srv, buf, len) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_write_round_one(
                  &srv, buf, sizeof(buf), &len, ecjpake_lgc, NULL) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_one(&cli, buf, len) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_write_round_two(
                  &srv, buf, sizeof(buf), &len, ecjpake_lgc, NULL) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_two(&cli, buf, len) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_derive_secret(
                  &cli, pms, sizeof(pms), &pmslen, ecjpake_lgc, NULL) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_write_round_two(
                  &cli, buf, sizeof(buf), &len, ecjpake_lgc, NULL) == 0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_two(&srv, buf, len) == 0);
  ...
  if (verbose != 0)
    mbedtls2_printf("  ECJPAKE test #2 (reference handshake): ");

  /* Simulate generation of round one */
  MBEDTLS2_MPI_CHK(
      ecjpake_test_load(&cli, ecjpake_test_x1, sizeof(ecjpake_test_x1),
                        ecjpake_test_x2, sizeof(ecjpake_test_x2)));

  MBEDTLS2_MPI_CHK(
      ecjpake_test_load(&srv, ecjpake_test_x3, sizeof(ecjpake_test_x3),
                        ecjpake_test_x4, sizeof(ecjpake_test_x4)));

  /* Read round one */
  TEST_ASSERT(mbedtls2_ecjpake_read_round_one(
                  &srv, ecjpake_test_cli_one, sizeof(ecjpake_test_cli_one)) ==
              0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_one(
                  &cli, ecjpake_test_srv_one, sizeof(ecjpake_test_srv_one)) ==
              0);

  /* Skip generation of round two, read round two */
  TEST_ASSERT(mbedtls2_ecjpake_read_round_two(
                  &cli, ecjpake_test_srv_two, sizeof(ecjpake_test_srv_two)) ==
              0);

  TEST_ASSERT(mbedtls2_ecjpake_read_round_two(
                  &srv, ecjpake_test_cli_two, sizeof(ecjpake_test_cli_two)) ==
              0);
...
cleanup:
  mbedtls2_ecjpake_free(&cli);
  mbedtls2_ecjpake_free(&srv);
...
#endif /* MBEDTLS2_ECP_DP_SECP256R1_ENABLED && MBEDTLS2_SHA256_C \
        */
