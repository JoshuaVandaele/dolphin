...
#error                                                                         \
    "Platform entropy sources only work on Unix and Windows, see MBEDTLS2_NO_PLATFORM_ENTROPY in config.h"
#endif
...
int mbedtls2_hardclock_poll(void *data, unsigned char *output,
                                   size_t len, size_t *olen) {
  unsigned long timer = mbedtls2_timing_hardclock();
  ((void)data);
  *olen = 0;
...
int mbedtls2_havege_poll(void *data, unsigned char *output, size_t len,
                                size_t *olen) {
  mbedtls2_havege_state *hs = (mbedtls2_havege_state *)data;
  *olen = 0;

  if (mbedtls2_havege_random(hs, output, len) != 0)
    return (MBEDTLS2_ERR_ENTROPY_SOURCE_FAILED);
...
int mbedtls2_nv_seed_poll(void *data, unsigned char *output, size_t len,
  ...
  size_t use_len = MBEDTLS2_ENTROPY_BLOCK_SIZE;
  ((void)data);

  memset(buf, 0, MBEDTLS2_ENTROPY_BLOCK_SIZE);

  if (mbedtls2_nv_seed_read(buf, MBEDTLS2_ENTROPY_BLOCK_SIZE) < 0)
    return (MBEDTLS2_ERR_ENTROPY_SOURCE_FAILED);
