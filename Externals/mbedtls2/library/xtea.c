...
void mbedtls2_xtea_init(mbedtls2_xtea_context *ctx) {
  memset(ctx, 0, sizeof(mbedtls2_xtea_context));
}
...
void mbedtls2_xtea_free(mbedtls2_xtea_context *ctx) {
  ...
  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_xtea_context));
}
...
void mbedtls2_xtea_setup(mbedtls2_xtea_context *ctx,
  ...
  for (i = 0; i < 4; i++) {
    ctx->k[i] = MBEDTLS2_GET_UINT32_BE(key, i << 2);
  }
}
...
int mbedtls2_xtea_crypt_ecb(mbedtls2_xtea_context *ctx, int mode,
  ...
  v0 = MBEDTLS2_GET_UINT32_BE(input, 0);
  v1 = MBEDTLS2_GET_UINT32_BE(input, 4);

  if (mode == MBEDTLS2_XTEA_ENCRYPT) {
  ...
  MBEDTLS2_PUT_UINT32_BE(v0, output, 0);
  MBEDTLS2_PUT_UINT32_BE(v1, output, 4);
...
int mbedtls2_xtea_crypt_cbc(mbedtls2_xtea_context *ctx, int mode,
  ...
  if (length % 8)
    return (MBEDTLS2_ERR_XTEA_INVALID_INPUT_LENGTH);

  if (mode == MBEDTLS2_XTEA_DECRYPT) {
    while (length > 0) {
      memcpy(temp, input, 8);
      mbedtls2_xtea_crypt_ecb(ctx, mode, input, output);
    ...
    while (length > 0) {
      ...
      mbedtls2_xtea_crypt_ecb(ctx, mode, output, output);
      memcpy(iv, output, 8);
...
int mbedtls2_xtea_self_test(int verbose) {
  ...
  for (i = 0; i < 6; i++) {
    if (verbose != 0)
      mbedtls2_printf("  XTEA test #%d: ", i + 1);

    memcpy(buf, xtea_test_pt[i], 8);

    mbedtls2_xtea_setup(&ctx, xtea_test_key[i]);
    mbedtls2_xtea_crypt_ecb(&ctx, MBEDTLS2_XTEA_ENCRYPT, buf,
                                   buf);
