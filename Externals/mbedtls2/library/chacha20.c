...
#define CHACHA20_VALIDATE_RET(cond)                                            \
  MBEDTLS2_INTERNAL_VALIDATE_RET(                                       \
      cond, MBEDTLS2_ERR_CHACHA20_BAD_INPUT_DATA)
#define CHACHA20_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)
...
static void chacha20_block(const uint32_t initial_state[16],
  ...
  for (i = 0U; i < 16; i++) {
    ...
    MBEDTLS2_PUT_UINT32_LE(working_state[i], keystream, offset);
  }

  mbedtls2_platform_zeroize(working_state, sizeof(working_state));
}

void mbedtls2_chacha20_init(mbedtls2_chacha20_context *ctx) {
  CHACHA20_VALIDATE(ctx != NULL);

  mbedtls2_platform_zeroize(ctx->state, sizeof(ctx->state));
  mbedtls2_platform_zeroize(ctx->keystream8, sizeof(ctx->keystream8));
...
void mbedtls2_chacha20_free(mbedtls2_chacha20_context *ctx) {
  if (ctx != NULL) {
    mbedtls2_platform_zeroize(ctx,
                                     sizeof(mbedtls2_chacha20_context));
  }
}
...
int mbedtls2_chacha20_setkey(mbedtls2_chacha20_context *ctx,
  ...
  /* Set key */
  ctx->state[4] = MBEDTLS2_GET_UINT32_LE(key, 0);
  ctx->state[5] = MBEDTLS2_GET_UINT32_LE(key, 4);
  ctx->state[6] = MBEDTLS2_GET_UINT32_LE(key, 8);
  ctx->state[7] = MBEDTLS2_GET_UINT32_LE(key, 12);
  ctx->state[8] = MBEDTLS2_GET_UINT32_LE(key, 16);
  ctx->state[9] = MBEDTLS2_GET_UINT32_LE(key, 20);
  ctx->state[10] = MBEDTLS2_GET_UINT32_LE(key, 24);
  ctx->state[11] = MBEDTLS2_GET_UINT32_LE(key, 28);
...
int mbedtls2_chacha20_starts(mbedtls2_chacha20_context *ctx,
  ...
  /* Nonce */
  ctx->state[13] = MBEDTLS2_GET_UINT32_LE(nonce, 0);
  ctx->state[14] = MBEDTLS2_GET_UINT32_LE(nonce, 4);
  ctx->state[15] = MBEDTLS2_GET_UINT32_LE(nonce, 8);
...
int mbedtls2_chacha20_crypt(const unsigned char key[32],
  ...
  ret = mbedtls2_chacha20_setkey(&ctx, key);
  if (ret != 0)
    goto cleanup;

  ret = mbedtls2_chacha20_starts(&ctx, nonce, counter);
  if (ret != 0)
    goto cleanup;

  ret = mbedtls2_chacha20_update(&ctx, data_len, input, output);

cleanup:
  mbedtls2_chacha20_free(&ctx);
  return (ret);
}
...
int mbedtls2_chacha20_self_test(int verbose) {
  ...
  for (i = 0U; i < 2U; i++) {
    if (verbose != 0)
      mbedtls2_printf("  ChaCha20 test %u ", i);

    ret = mbedtls2_chacha20_crypt(test_keys[i], test_nonces[i],
