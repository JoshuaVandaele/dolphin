 ...
 * 0xFFFFFF52. */
static psa_storage_uid_t
psa_its_identifier_of_slot(mbedtls2_svc_key_id_t key) {
#if defined(MBEDTLS2_PSA_CRYPTO_KEY_ID_ENCODES_OWNER)
  /* Encode the owner in the upper 32 bits. This means that if
   * owner values are nonzero (as they are on a PSA platform),
   * no key file will ever have a value less than 0x100000000, so
   * the whole range 0..0xffffffff is available for non-key files. */
  uint32_t unsigned_owner_id = MBEDTLS2_SVC_KEY_ID_GET_OWNER_ID(key);
  return (((uint64_t)unsigned_owner_id << 32) |
          MBEDTLS2_SVC_KEY_ID_GET_KEY_ID(key));
#else
 ...
 */
static psa_status_t
psa_crypto_storage_load(const mbedtls2_svc_key_id_t key, uint8_t *data,
                        size_t data_size) {
  psa_status_t status;
  ...
  status =
      psa_its_get(data_identifier, 0, (uint32_t)data_size, data, &data_length);
...
int psa_is_key_present_in_storage(const mbedtls2_svc_key_id_t key) {
  psa_status_t ret;
  psa_storage_uid_t data_identifier = psa_its_identifier_of_slot(key);
  struct psa_storage_info_t data_identifier_info;
 ...
 */
static psa_status_t
psa_crypto_storage_store(const mbedtls2_svc_key_id_t key,
...
psa_status_t
psa_destroy_persistent_key(const mbedtls2_svc_key_id_t key) {
  psa_status_t ret;
  psa_storage_uid_t data_identifier = psa_its_identifier_of_slot(key);
  struct psa_storage_info_t data_identifier_info;
 ...
 */
static psa_status_t
psa_crypto_storage_get_data_length(const mbedtls2_svc_key_id_t key,
                                   size_t *data_length) {
  psa_status_t status;
  psa_storage_uid_t data_identifier = psa_its_identifier_of_slot(key);
  struct psa_storage_info_t data_identifier_info;
...
void psa_format_key_data_for_storage(const uint8_t *data,
  ...
  MBEDTLS2_PUT_UINT32_LE(0, storage_format->version, 0);
  MBEDTLS2_PUT_UINT32_LE(attr->lifetime, storage_format->lifetime, 0);
  MBEDTLS2_PUT_UINT16_LE((uint16_t)attr->type, storage_format->type, 0);
  MBEDTLS2_PUT_UINT16_LE((uint16_t)attr->bits, storage_format->bits, 0);
  MBEDTLS2_PUT_UINT32_LE(attr->policy.usage, storage_format->policy, 0);
  MBEDTLS2_PUT_UINT32_LE(attr->policy.alg, storage_format->policy,
                                sizeof(uint32_t));
  MBEDTLS2_PUT_UINT32_LE(attr->policy.alg2, storage_format->policy,
                                2 * sizeof(uint32_t));
  MBEDTLS2_PUT_UINT32_LE(data_length, storage_format->data_len, 0);
...
psa_status_t psa_parse_key_data_from_storage(const uint8_t *storage_data,
  ...
  version = MBEDTLS2_GET_UINT32_LE(storage_format->version, 0);
  if (version != 0)
    return (PSA_ERROR_DATA_INVALID);

  *key_data_length = MBEDTLS2_GET_UINT32_LE(storage_format->data_len, 0);
  ...
  if (*key_data_length == 0) {
    ...
    *key_data = mbedtls2_calloc(1, *key_data_length);
    if (*key_data == NULL)
      return (PSA_ERROR_INSUFFICIENT_MEMORY);
    memcpy(*key_data, storage_format->key_data, *key_data_length);
  }

  attr->lifetime = MBEDTLS2_GET_UINT32_LE(storage_format->lifetime, 0);
  attr->type = MBEDTLS2_GET_UINT16_LE(storage_format->type, 0);
  attr->bits = MBEDTLS2_GET_UINT16_LE(storage_format->bits, 0);
  attr->policy.usage = MBEDTLS2_GET_UINT32_LE(storage_format->policy, 0);
  attr->policy.alg =
      MBEDTLS2_GET_UINT32_LE(storage_format->policy, sizeof(uint32_t));
  attr->policy.alg2 = MBEDTLS2_GET_UINT32_LE(storage_format->policy,
                                                    2 * sizeof(uint32_t));
...
psa_status_t psa_save_persistent_key(const psa_core_key_attributes_t *attr,
  ...
  storage_data = mbedtls2_calloc(1, storage_data_length);
...
void psa_free_persistent_key_data(uint8_t *key_data, size_t key_data_length) {
  if (key_data != NULL) {
    mbedtls2_platform_zeroize(key_data, key_data_length);
  }
  mbedtls2_free(key_data);
}
...
psa_status_t psa_load_persistent_key(psa_core_key_attributes_t *attr,
  ...
  mbedtls2_svc_key_id_t key = attr->id;

  status = psa_crypto_storage_get_data_length(key, &storage_data_length);
  if (status != PSA_SUCCESS)
    return (status);

  loaded_data = mbedtls2_calloc(1, storage_data_length);
...
psa_status_t
mbedtls2_psa_storage_inject_entropy(const unsigned char *seed,
                                           size_t seed_size) {
  psa_status_t status;
  struct psa_storage_info_t p_info;
