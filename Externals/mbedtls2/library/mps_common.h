...
#ifndef MBEDTLS2_MPS_COMMON_H
#define MBEDTLS2_MPS_COMMON_H
 ...
 *  with error #MBEDTLS2_ERR_MPS_OPERATION_UNEXPECTED.
 ...
 */
#define MBEDTLS2_MPS_STATE_VALIDATION
 ...
 */
#define MBEDTLS2_MPS_ENABLE_ASSERTIONS

/*! This flag controls whether tracing for MPS should be enabled. */
//#define MBEDTLS2_MPS_ENABLE_TRACE

#if defined(MBEDTLS2_MPS_STATE_VALIDATION)

#define MBEDTLS2_MPS_STATE_VALIDATE_RAW( cond, string )                         \
    do                                                                         \
    {                                                                          \
        if( !(cond) )                                                          \
        {                                                                      \
            MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_ERROR, string );         \
            MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_OPERATION_UNEXPECTED );  \
        }                                                                      \
    } while( 0 )

#else /* MBEDTLS2_MPS_STATE_VALIDATION */
...
#endif /* MBEDTLS2_MPS_STATE_VALIDATION */

#if defined(MBEDTLS2_MPS_ENABLE_ASSERTIONS)

#define MBEDTLS2_MPS_ASSERT_RAW( cond, string )                          \
    do                                                                  \
    {                                                                   \
        if( !(cond) )                                                   \
        {                                                               \
            MBEDTLS2_MPS_TRACE( MBEDTLS2_MPS_TRACE_TYPE_ERROR, string );  \
            MBEDTLS2_MPS_TRACE_RETURN( MBEDTLS2_ERR_MPS_INTERNAL_ERROR ); \
        }                                                               \
    } while( 0 )

#else /* MBEDTLS2_MPS_ENABLE_ASSERTIONS */

#define MBEDTLS2_MPS_ASSERT_RAW( cond, string ) do {} while( 0 )

#endif /* MBEDTLS2_MPS_ENABLE_ASSERTIONS */
 ...
 *           than ::mbedtls2_mps_size_t here because of
 *           potential truncation during conversion.
 *
 * \warning  Handshake messages in TLS may be up to 2^24 ~ 16Mb in size.
 *           If mbedtls2_mps_[opt_]stored_size_t is smaller than that, the
 *           maximum handshake message is restricted accordingly.
 *
 * For now, we use the default type of size_t throughout, and the use of
 * smaller types or different types for ::mbedtls2_mps_size_t and
 * ::mbedtls2_mps_stored_size_t is not yet supported.
 *
 */
typedef size_t mbedtls2_mps_stored_size_t;
#define MBEDTLS2_MPS_STORED_SIZE_MAX  ( (mbedtls2_mps_stored_size_t) -1 )

/** \brief The type of buffer sizes and offsets used in the MPS API
 *         and implementation.
 *
 *         This must be at least as wide as ::mbedtls2_stored_size_t but
 ...
 *         so almost 10%.
 */
typedef size_t mbedtls2_mps_size_t;
#define MBEDTLS2_MPS_SIZE_MAX  ( (mbedtls2_mps_size_t) -1 )

#if MBEDTLS2_MPS_STORED_SIZE_MAX > MBEDTLS2_MPS_SIZE_MAX
#error "Misconfiguration of mbedtls2_mps_size_t and mbedtls2_mps_stored_size_t."
#endif

/* \} SECTION: Common types */


#endif /* MBEDTLS2_MPS_COMMON_H */
