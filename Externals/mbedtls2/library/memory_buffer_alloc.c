...
#include "mbedtls2/memory_buffer_alloc.h"

/* No need for the header guard as MBEDTLS2_MEMORY_BUFFER_ALLOC_C
   is dependent upon MBEDTLS2_PLATFORM_C */
...
static void *buffer_alloc_calloc(size_t n, size_t size) {
  ...
  if (n == 0 || size == 0 || len / n != size)
    return (NULL);
  else if (len > (size_t)-MBEDTLS2_MEMORY_ALIGN_MULTIPLE)
    return (NULL);

  if (len % MBEDTLS2_MEMORY_ALIGN_MULTIPLE) {
    len -= len % MBEDTLS2_MEMORY_ALIGN_MULTIPLE;
    len += MBEDTLS2_MEMORY_ALIGN_MULTIPLE;
  }
  ...
  //
  if (cur->size - len <
      sizeof(memory_header) + MBEDTLS2_MEMORY_ALIGN_MULTIPLE) {
    cur->alloc = 1;
    ...
    if ((heap.verify & MBEDTLS2_MEMORY_VERIFY_ALLOC) &&
        verify_chain() != 0)
      mbedtls2_exit(1);
...
static void buffer_alloc_free(void *ptr) {
...
#if defined(MBEDTLS2_MEMORY_DEBUG)
    mbedtls2_fprintf(stderr,
                            "FATAL: mbedtls2_free() outside of managed "
                            "space\n");
#endif
    mbedtls2_exit(1);
  }
...
#if defined(MBEDTLS2_MEMORY_DEBUG)
    mbedtls2_fprintf(stderr,
                            "FATAL: mbedtls2_free() on unallocated "
                            "data\n");
#endif
    mbedtls2_exit(1);
  }
  ...
  if ((heap.verify & MBEDTLS2_MEMORY_VERIFY_FREE) && verify_chain() != 0)
    mbedtls2_exit(1);
}
...
int mbedtls2_memory_buffer_alloc_verify(void) { return verify_chain(); }

#if defined(MBEDTLS2_MEMORY_DEBUG)
void mbedtls2_memory_buffer_alloc_status(void) {
  mbedtls2_fprintf(
      stderr,
...
void mbedtls2_memory_buffer_alloc_max_get(size_t *max_used,
                                                 size_t *max_blocks) {
...
void mbedtls2_memory_buffer_alloc_cur_get(size_t *cur_used,
                                                 size_t *cur_blocks) {
...
#else
  mbedtls2_platform_set_calloc_free(buffer_alloc_calloc,
                                           buffer_alloc_free);
#endif

  if (len < sizeof(memory_header) + MBEDTLS2_MEMORY_ALIGN_MULTIPLE)
    return;
  else if ((size_t)buf % MBEDTLS2_MEMORY_ALIGN_MULTIPLE) {
    /* Adjust len first since buf is used in the computation */
    len -= MBEDTLS2_MEMORY_ALIGN_MULTIPLE -
           (size_t)buf % MBEDTLS2_MEMORY_ALIGN_MULTIPLE;
    buf += MBEDTLS2_MEMORY_ALIGN_MULTIPLE -
           (size_t)buf % MBEDTLS2_MEMORY_ALIGN_MULTIPLE;
  }
...
void mbedtls2_memory_buffer_alloc_free(void) {
#if defined(MBEDTLS2_THREADING_C)
  mbedtls2_mutex_free(&heap.mutex);
#endif
  mbedtls2_platform_zeroize(&heap, sizeof(buffer_alloc_ctx));
}
...
static int check_pointer(void *p) {
  ...
  if ((size_t)p % MBEDTLS2_MEMORY_ALIGN_MULTIPLE != 0)
    return (-1);
