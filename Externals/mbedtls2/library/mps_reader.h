 ...
 *     received through calls to mbedtls2_mps_reader_get(),
 ...
 *     mbedtls2_mps_reader_reclaim().
 *     The marker is set via mbedtls2_mps_reader_commit()
 ...
 * done via successful calls to mbedtls2_mps_reader_feed(), while
 * transitioning from Attached to either Unset or Accumulating (depending
 * on what has been processed) is done via mbedtls2_mps_reader_reclaim().
 *
...
#ifndef MBEDTLS2_READER_H
#define MBEDTLS2_READER_H
...
struct mbedtls2_mps_reader;
typedef struct mbedtls2_mps_reader mbedtls2_mps_reader;
...
struct mbedtls2_mps_reader
{
    unsigned char *frag;  /*!< The fragment of incoming data managed by
                           ...
                           *   through mbedtls2_mps_reader_feed(). The reader
    ...
    mbedtls2_mps_stored_size_t frag_len;
                          /*!< The length of the current fragment.
                           *   Must be 0 if \c frag == \c NULL.             */
    mbedtls2_mps_stored_size_t commit;
    ...
    mbedtls2_mps_stored_size_t end;
                          /*!< The offset of the end of the last chunk
                           *   passed to the user through a call to
                           *   mbedtls2_mps_reader_get(), relative to the first
    ...
    unsigned char *acc;   /*!< The accumulator is used to gather incoming
                           *   data if a read-request via mbedtls2_mps_reader_get()
                           *   cannot be served from the current fragment.   */
    mbedtls2_mps_stored_size_t acc_len;
                           /*!< The total size of the accumulator.           */
    mbedtls2_mps_stored_size_t acc_available;
    ...
    union
    {
        mbedtls2_mps_stored_size_t acc_remaining;
        ...
        mbedtls2_mps_stored_size_t frag_offset;
 ...
 *                  in case get requests through mbedtls2_mps_reader_get()
 *                  exceed the buffer provided by mbedtls2_mps_reader_feed().
 ...
 * \return          A negative \c MBEDTLS2_ERR_READER_XXX error code on failure.
 */
int mbedtls2_mps_reader_init( mbedtls2_mps_reader *reader,
                             unsigned char *acc,
 ...
 * \return          A negative \c MBEDTLS2_ERR_READER_XXX error code on failure.
 */
int mbedtls2_mps_reader_free( mbedtls2_mps_reader *reader );
 ...
 *                  of \p buf until mbedtls2_mps_reader_reclaim()
 *                  is called. It is the responsibility of the caller
 *                  to ensure that the \p buf persists and is not changed
 *                  between successful calls to mbedtls2_mps_reader_feed()
 *                  and mbedtls2_mps_reader_reclaim().
 * \return          \c MBEDTLS2_ERR_MPS_READER_NEED_MORE if more input data is
 *                  required to fulfill a previous request to mbedtls2_mps_reader_get().
 *                  In this case, the reader remains in producing mode and
 *                  takes no ownership of the provided buffer (an internal copy
 *                  is made instead).
 * \return          Another negative \c MBEDTLS2_ERR_READER_XXX error code on
 *                  different kinds of failures.
 */
int mbedtls2_mps_reader_feed( mbedtls2_mps_reader *reader,
                             unsigned char *buf,
 ...
 *                  mbedtls2_mps_reader_get() has exceeded the bounds of the
 *                  input buffer.
 *
 * \return          \c 0 on success.
 * \return          A negative \c MBEDTLS2_ERR_READER_XXX error code on failure.
 */
int mbedtls2_mps_reader_reclaim( mbedtls2_mps_reader *reader,
                                int *paused );
 ...
 *                  until the next call to mbedtls2_mps_reader_reclaim().
 * \return          #MBEDTLS2_ERR_MPS_READER_OUT_OF_DATA if there is not enough
 ...
 *                  mbedtls2_mps_reader_reclaim() and mbedtls2_mps_reader_feed().
 ...
 *                  is requested after a mbedtls2_mps_reader_reclaim() and
 *                  mbedtls2_mps_reader_feed() cycle.
 * \return          Another negative \c MBEDTLS2_ERR_READER_XXX error
 ...
 *                  afterwards.
 */
int mbedtls2_mps_reader_get( mbedtls2_mps_reader *reader,
                            mbedtls2_mps_size_t desired,
                            unsigned char **buffer,
                            mbedtls2_mps_size_t *buflen );

/**
 * \brief         Mark data obtained from mbedtls2_mps_reader_get() as processed.
 *
 *                This call indicates that all data received from prior calls to
 *                mbedtls2_mps_reader_get() has been or will have been
 *                processed when mbedtls2_mps_reader_reclaim() is called,
 *                and thus need not be backed up.
 *
 *                This function has no user observable effect until
 *                mbedtls2_mps_reader_reclaim() is called. In particular,
 *                buffers received from mbedtls2_mps_reader_get() remain
 *                valid until mbedtls2_mps_reader_reclaim() is called.
 *
 ...
 *
 */
int mbedtls2_mps_reader_commit( mbedtls2_mps_reader *reader );

#endif /* MBEDTLS2_READER_H */
