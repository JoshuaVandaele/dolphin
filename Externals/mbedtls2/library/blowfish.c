...
#define BLOWFISH_VALIDATE_RET(cond)                                            \
  MBEDTLS2_INTERNAL_VALIDATE_RET(                                       \
      cond, MBEDTLS2_ERR_BLOWFISH_BAD_INPUT_DATA)
#define BLOWFISH_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)

static const uint32_t P[MBEDTLS2_BLOWFISH_ROUNDS + 2] = {
...
static uint32_t F(mbedtls2_blowfish_context *ctx, uint32_t x) {
...
static void blowfish_enc(mbedtls2_blowfish_context *ctx, uint32_t *xl,
                         uint32_t *xr) {
  uint32_t Xl, Xr, temp;
  short i;
  ...
  for (i = 0; i < MBEDTLS2_BLOWFISH_ROUNDS; ++i) {
    Xl = Xl ^ ctx->P[i];
    Xr = F(ctx, Xl) ^ Xr;
  ...
  Xr = Xr ^ ctx->P[MBEDTLS2_BLOWFISH_ROUNDS];
  Xl = Xl ^ ctx->P[MBEDTLS2_BLOWFISH_ROUNDS + 1];

  *xl = Xl;
  *xr = Xr;
}

static void blowfish_dec(mbedtls2_blowfish_context *ctx, uint32_t *xl,
                         uint32_t *xr) {
  uint32_t Xl, Xr, temp;
  short i;
  ...
  for (i = MBEDTLS2_BLOWFISH_ROUNDS + 1; i > 1; --i) {
    Xl = Xl ^ ctx->P[i];
    Xr = F(ctx, Xl) ^ Xr;
...
void mbedtls2_blowfish_init(mbedtls2_blowfish_context *ctx) {
  ...
  memset(ctx, 0, sizeof(mbedtls2_blowfish_context));
}
...
void mbedtls2_blowfish_free(mbedtls2_blowfish_context *ctx) {
  ...
  mbedtls2_platform_zeroize(ctx,
                                   sizeof(mbedtls2_blowfish_context));
}
...
int mbedtls2_blowfish_setkey(mbedtls2_blowfish_context *ctx,
  ...
  if (keybits < MBEDTLS2_BLOWFISH_MIN_KEY_BITS ||
      keybits > MBEDTLS2_BLOWFISH_MAX_KEY_BITS || keybits % 8 != 0) {
    return (MBEDTLS2_ERR_BLOWFISH_BAD_INPUT_DATA);
  }
  ...
  j = 0;
  for (i = 0; i < MBEDTLS2_BLOWFISH_ROUNDS + 2; ++i) {
    data = 0x00000000;
    for (k = 0; k < 4; ++k) {
      data = (data << 8) | key[j++];
      if (j >= keybits)
  ...
  for (i = 0; i < MBEDTLS2_BLOWFISH_ROUNDS + 2; i += 2) {
...
int mbedtls2_blowfish_crypt_ecb(
  ...
  uint32_t X0, X1;
  BLOWFISH_VALIDATE_RET(ctx != NULL);
  BLOWFISH_VALIDATE_RET(mode == MBEDTLS2_BLOWFISH_ENCRYPT ||
                        mode == MBEDTLS2_BLOWFISH_DECRYPT);
  BLOWFISH_VALIDATE_RET(input != NULL);
  BLOWFISH_VALIDATE_RET(output != NULL);

  X0 = MBEDTLS2_GET_UINT32_BE(input, 0);
  X1 = MBEDTLS2_GET_UINT32_BE(input, 4);

  if (mode == MBEDTLS2_BLOWFISH_DECRYPT) {
    blowfish_dec(ctx, &X0, &X1);
  } else /* MBEDTLS2_BLOWFISH_ENCRYPT */
  {
    blowfish_enc(ctx, &X0, &X1);
  }

  MBEDTLS2_PUT_UINT32_BE(X0, output, 0);
  MBEDTLS2_PUT_UINT32_BE(X1, output, 4);
...
int mbedtls2_blowfish_crypt_cbc(
  ...
  int i;
  unsigned char temp[MBEDTLS2_BLOWFISH_BLOCKSIZE];
  BLOWFISH_VALIDATE_RET(ctx != NULL);
  BLOWFISH_VALIDATE_RET(mode == MBEDTLS2_BLOWFISH_ENCRYPT ||
                        mode == MBEDTLS2_BLOWFISH_DECRYPT);
  ...
  if (length % MBEDTLS2_BLOWFISH_BLOCKSIZE)
    return (MBEDTLS2_ERR_BLOWFISH_INVALID_INPUT_LENGTH);

  if (mode == MBEDTLS2_BLOWFISH_DECRYPT) {
    while (length > 0) {
      memcpy(temp, input, MBEDTLS2_BLOWFISH_BLOCKSIZE);
      mbedtls2_blowfish_crypt_ecb(ctx, mode, input, output);

      for (i = 0; i < MBEDTLS2_BLOWFISH_BLOCKSIZE; i++)
        output[i] = (unsigned char)(output[i] ^ iv[i]);

      memcpy(iv, temp, MBEDTLS2_BLOWFISH_BLOCKSIZE);

      input += MBEDTLS2_BLOWFISH_BLOCKSIZE;
      output += MBEDTLS2_BLOWFISH_BLOCKSIZE;
      length -= MBEDTLS2_BLOWFISH_BLOCKSIZE;
    }
  } else {
    while (length > 0) {
      for (i = 0; i < MBEDTLS2_BLOWFISH_BLOCKSIZE; i++)
      ...
      mbedtls2_blowfish_crypt_ecb(ctx, mode, output, output);
      memcpy(iv, output, MBEDTLS2_BLOWFISH_BLOCKSIZE);
...
int mbedtls2_blowfish_crypt_cfb64(
  ...
  if (mode == MBEDTLS2_BLOWFISH_DECRYPT) {
    while (length--) {
      if (n == 0)
        mbedtls2_blowfish_crypt_ecb(
            ctx, MBEDTLS2_BLOWFISH_ENCRYPT, iv, iv);

      c = *input++;
      *output++ = (unsigned char)(c ^ iv[n]);
      iv[n] = (unsigned char)c;

      n = (n + 1) % MBEDTLS2_BLOWFISH_BLOCKSIZE;
    }
  } else {
    while (length--) {
      if (n == 0)
...
int mbedtls2_blowfish_crypt_ctr(
  ...
  while (length--) {
    if (n == 0) {
      mbedtls2_blowfish_crypt_ecb(ctx, MBEDTLS2_BLOWFISH_ENCRYPT,
                                         nonce_counter, stream_block);

      for (i = MBEDTLS2_BLOWFISH_BLOCKSIZE; i > 0; i--)
