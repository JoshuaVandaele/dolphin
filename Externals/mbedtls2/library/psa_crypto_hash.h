 ...
 * mbedtls2_psa_hash_setup(), the operation will need to be reset by a
 * call to mbedtls2_psa_hash_abort(). The core may call
 * mbedtls2_psa_hash_abort() at any time after the operation has been
 * initialized.
 *
 * After a successful call to mbedtls2_psa_hash_setup(), the core must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to mbedtls2_psa_hash_finish() or
 * mbedtls2_psa_hash_verify().
 * - A call to mbedtls2_psa_hash_abort().
 *
 ...
 * to calling mbedtls2_psa_hash_setup() on \p target_operation with the
 * same algorithm that \p source_operation was set up for, then
 * mbedtls2_psa_hash_update() on \p target_operation with the same input
 * that that was passed to \p source_operation. After this function returns, the
 ...
 *       drivers.
 *
 * The application must call mbedtls2_psa_hash_setup() before calling
 * this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling mbedtls2_psa_hash_abort().
 *
 ...
 *
 * The application must call mbedtls2_psa_hash_setup() before calling
 * this function. This function calculates the hash of the message formed by
 * concatenating the inputs passed to preceding calls to
 * mbedtls2_psa_hash_update().
 *
 ...
 * mbedtls2_psa_hash_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized by one of the methods described in #psa_hash_operation_t.
 *
 * In particular, calling mbedtls2_psa_hash_abort() after the operation
 * has been terminated by a call to mbedtls2_psa_hash_abort(),
 * mbedtls2_psa_hash_finish() or mbedtls2_psa_hash_verify() is
 ...
 */
psa_status_t
mbedtls2_psa_hash_abort(mbedtls2_psa_hash_operation_t *operation);
