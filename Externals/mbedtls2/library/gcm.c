...
#if defined(MBEDTLS2_SELF_TEST) && defined(MBEDTLS2_AES_C)
...
#endif /* MBEDTLS2_SELF_TEST && MBEDTLS2_AES_C */

#if !defined(MBEDTLS2_GCM_ALT)

/* Parameter validation macros */
#define GCM_VALIDATE_RET(cond)                                                 \
  MBEDTLS2_INTERNAL_VALIDATE_RET(cond, MBEDTLS2_ERR_GCM_BAD_INPUT)
#define GCM_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)

/*
 * Initialize a context
 */
void mbedtls2_gcm_init(mbedtls2_gcm_context *ctx) {
  GCM_VALIDATE(ctx != NULL);
  memset(ctx, 0, sizeof(mbedtls2_gcm_context));
}
 ...
 */
static int gcm_gen_table(mbedtls2_gcm_context *ctx) {
  int ret, i, j;
  uint64_t hi, lo;
  uint64_t vl, vh;
  unsigned char h[16];
  ...
  memset(h, 0, 16);
  if ((ret = mbedtls2_cipher_update(&ctx->cipher_ctx, h, 16, h,
                                           &olen)) != 0)
    return (ret);

  /* pack h as two 64-bits ints, big-endian */
  hi = MBEDTLS2_GET_UINT32_BE(h, 0);
  lo = MBEDTLS2_GET_UINT32_BE(h, 4);
  vh = (uint64_t)hi << 32 | lo;

  hi = MBEDTLS2_GET_UINT32_BE(h, 8);
  lo = MBEDTLS2_GET_UINT32_BE(h, 12);
  ...
  if (mbedtls2_aesni_has_support(MBEDTLS2_AESNI_CLMUL))
    return (0);
#endif
...
int mbedtls2_gcm_setkey(mbedtls2_gcm_context *ctx,
                               mbedtls2_cipher_id_t cipher,
  ...
  cipher_info = mbedtls2_cipher_info_from_values(
      cipher, keybits, MBEDTLS2_MODE_ECB);
  if (cipher_info == NULL)
    return (MBEDTLS2_ERR_GCM_BAD_INPUT);

  if (cipher_info->block_size != 16)
    return (MBEDTLS2_ERR_GCM_BAD_INPUT);

  mbedtls2_cipher_free(&ctx->cipher_ctx);

  if ((ret = mbedtls2_cipher_setup(&ctx->cipher_ctx, cipher_info)) != 0)
    return (ret);

  if ((ret = mbedtls2_cipher_setkey(&ctx->cipher_ctx, key, keybits,
                                           MBEDTLS2_ENCRYPT)) != 0) {
    return (ret);
  }
 ...
 */
static void gcm_mult(mbedtls2_gcm_context *ctx,
  ...
  if (mbedtls2_aesni_has_support(MBEDTLS2_AESNI_CLMUL)) {
    ...
    MBEDTLS2_PUT_UINT32_BE(ctx->HH[8] >> 32, h, 0);
    MBEDTLS2_PUT_UINT32_BE(ctx->HH[8], h, 4);
    MBEDTLS2_PUT_UINT32_BE(ctx->HL[8] >> 32, h, 8);
    MBEDTLS2_PUT_UINT32_BE(ctx->HL[8], h, 12);

    mbedtls2_aesni_gcm_mult(output, x, h);
    return;
  }
#endif /* MBEDTLS2_AESNI_C && MBEDTLS2_HAVE_X86_64 */
  ...
  MBEDTLS2_PUT_UINT32_BE(zh >> 32, output, 0);
  MBEDTLS2_PUT_UINT32_BE(zh, output, 4);
  MBEDTLS2_PUT_UINT32_BE(zl >> 32, output, 8);
  MBEDTLS2_PUT_UINT32_BE(zl, output, 12);
}

int mbedtls2_gcm_starts(mbedtls2_gcm_context *ctx, int mode,
  ...
  if (iv_len == 12) {
    ...
    MBEDTLS2_PUT_UINT64_BE(iv_bits, work_buf, 8);
  ...
  if ((ret = mbedtls2_cipher_update(&ctx->cipher_ctx, ctx->y, 16,
...
int mbedtls2_gcm_update(mbedtls2_gcm_context *ctx, size_t length,
  ...
  while (length > 0) {
    ...
    if ((ret = mbedtls2_cipher_update(&ctx->cipher_ctx, ctx->y, 16, ectr,
                                             &olen)) != 0) {
      return (ret);
    }

    for (i = 0; i < use_len; i++) {
      if (ctx->mode == MBEDTLS2_GCM_DECRYPT)
        ctx->buf[i] ^= p[i];
      out_p[i] = ectr[i] ^ p[i];
      if (ctx->mode == MBEDTLS2_GCM_ENCRYPT)
        ctx->buf[i] ^= out_p[i];
    }
...
int mbedtls2_gcm_finish(mbedtls2_gcm_context *ctx,
  ...
  if (orig_len || orig_add_len) {
    ...
    MBEDTLS2_PUT_UINT32_BE((orig_add_len >> 32), work_buf, 0);
    MBEDTLS2_PUT_UINT32_BE((orig_add_len), work_buf, 4);
    MBEDTLS2_PUT_UINT32_BE((orig_len >> 32), work_buf, 8);
    MBEDTLS2_PUT_UINT32_BE((orig_len), work_buf, 12);
...
int mbedtls2_gcm_crypt_and_tag(mbedtls2_gcm_context *ctx,
                                      int mode, size_t length,
                                      const unsigned char *iv, size_t iv_len,
                                      const unsigned char *add, size_t add_len,
                                      const unsigned char *input,
                                      unsigned char *output, size_t tag_len,
                                      unsigned char *tag) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  if ((ret = mbedtls2_gcm_starts(ctx, mode, iv, iv_len, add, add_len)) !=
      0)
    return (ret);

  if ((ret = mbedtls2_gcm_update(ctx, length, input, output)) != 0)
    return (ret);

  if ((ret = mbedtls2_gcm_finish(ctx, tag, tag_len)) != 0)
    return (ret);

  return (0);
}

int mbedtls2_gcm_auth_decrypt(mbedtls2_gcm_context *ctx,
                                     size_t length, const unsigned char *iv,
                                     size_t iv_len, const unsigned char *add,
                                     size_t add_len, const unsigned char *tag,
                                     size_t tag_len, const unsigned char *input,
                                     unsigned char *output) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  ...
  if ((ret = mbedtls2_gcm_crypt_and_tag(
           ctx, MBEDTLS2_GCM_DECRYPT, length, iv, iv_len, add, add_len,
  ...
  if (diff != 0) {
    mbedtls2_platform_zeroize(output, length);
    return (MBEDTLS2_ERR_GCM_AUTH_FAILED);
  }

  return (0);
}

void mbedtls2_gcm_free(mbedtls2_gcm_context *ctx) {
  if (ctx == NULL)
    return;
  mbedtls2_cipher_free(&ctx->cipher_ctx);
  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_gcm_context));
}
...
int mbedtls2_gcm_self_test(int verbose) {
  mbedtls2_gcm_context ctx;
  unsigned char buf[64];
  unsigned char tag_buf[16];
  int i, j, ret;
  mbedtls2_cipher_id_t cipher = MBEDTLS2_CIPHER_ID_AES;

  for (j = 0; j < 3; j++) {
    int key_len = 128 + 64 * j;

    for (i = 0; i < MAX_TESTS; i++) {
      mbedtls2_gcm_init(&ctx);

      if (verbose != 0)
        mbedtls2_printf("  AES-GCM-%3d #%d (%s): ", key_len, i, "enc");

      ret = mbedtls2_gcm_setkey(
      ...
      ret = mbedtls2_gcm_crypt_and_tag(
          &ctx, MBEDTLS2_GCM_ENCRYPT, pt_len_test_data[i],
...
#endif /* defined(MBEDTLS2_GCM_ALT) */
      if (ret != 0)
        goto exit;
      ...
      if (verbose != 0)
        mbedtls2_printf("  AES-GCM-%3d #%d (%s): ", key_len, i, "dec");
      ...
      ret = mbedtls2_gcm_crypt_and_tag(
          &ctx, MBEDTLS2_GCM_DECRYPT, pt_len_test_data[i],
          iv_test_data[iv_index_test_data[i]], iv_len_test_data[i],
          additional_test_data[add_index_test_data[i]], add_len_test_data[i],
      ...
      if (verbose != 0)
        mbedtls2_printf("  AES-GCM-%3d #%d split (%s): ", key_len, i,
                               "enc");
      ...
      ret = mbedtls2_gcm_starts(
          &ctx, MBEDTLS2_GCM_ENCRYPT,
          iv_test_data[iv_index_test_data[i]], iv_len_test_data[i],
          additional_test_data[add_index_test_data[i]], add_len_test_data[i]);
      if (ret != 0)
        goto exit;
      ...
      if (pt_len_test_data[i] > 32) {
        ...
        ret = mbedtls2_gcm_update(
            &ctx, 32, pt_test_data[pt_index_test_data[i]], buf);
        if (ret != 0)
          goto exit;

        ret = mbedtls2_gcm_update(
            &ctx, rest_len, pt_test_data[pt_index_test_data[i]] + 32, buf + 32);
        if (ret != 0)
          goto exit;
      } else {
        ret = mbedtls2_gcm_update(&ctx, pt_len_test_data[i],
      ...
      ret = mbedtls2_gcm_finish(&ctx, tag_buf, 16);
      if (ret != 0)
        goto exit;
      ...
      if (pt_len_test_data[i] > 32) {
        size_t rest_len = pt_len_test_data[i] - 32;
        ret =
            mbedtls2_gcm_update(&ctx, 32, ct_test_data[j * 6 + i], buf);
        if (ret != 0)
          goto exit;

        ret = mbedtls2_gcm_update(
            &ctx, rest_len, ct_test_data[j * 6 + i] + 32, buf + 32);
        if (ret != 0)
          goto exit;
      } else {
        ret = mbedtls2_gcm_update(&ctx, pt_len_test_data[i],
                                         ct_test_data[j * 6 + i], buf);
        if (ret != 0)
          goto exit;
      }
