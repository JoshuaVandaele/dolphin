...
void mbedtls2_md4_init(mbedtls2_md4_context *ctx) {
  memset(ctx, 0, sizeof(mbedtls2_md4_context));
}

void mbedtls2_md4_free(mbedtls2_md4_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_md4_context));
}

void mbedtls2_md4_clone(mbedtls2_md4_context *dst,
                               const mbedtls2_md4_context *src) {
  *dst = *src;
}
 ...
 * MD4 context setup
 */
int mbedtls2_md4_starts_ret(mbedtls2_md4_context *ctx) {
  ctx->total[0] = 0;
  ctx->total[1] = 0;
...
void mbedtls2_md4_starts(mbedtls2_md4_context *ctx) {
  mbedtls2_md4_starts_ret(ctx);
}
#endif

#if !defined(MBEDTLS2_MD4_PROCESS_ALT)
int mbedtls2_internal_md4_process(mbedtls2_md4_context *ctx,
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md4_process(mbedtls2_md4_context *ctx,
                                 const unsigned char data[64]) {
  mbedtls2_internal_md4_process(ctx, data);
}
#endif
 ...
 * MD4 process buffer
 */
int mbedtls2_md4_update_ret(mbedtls2_md4_context *ctx,
  ...
  if (left && ilen >= fill) {
    ...
    if ((ret = mbedtls2_internal_md4_process(ctx, ctx->buffer)) != 0)
      return (ret);
  ...
  while (ilen >= 64) {
    if ((ret = mbedtls2_internal_md4_process(ctx, input)) != 0)
      return (ret);
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md4_update(mbedtls2_md4_context *ctx,
                                const unsigned char *input, size_t ilen) {
  mbedtls2_md4_update_ret(ctx, input, ilen);
}
#endif
 ...
 * MD4 final digest
 */
int mbedtls2_md4_finish_ret(mbedtls2_md4_context *ctx,
  ...
  ret = mbedtls2_md4_update_ret(ctx, (unsigned char *)md4_padding, padn);
  if (ret != 0)
    return (ret);

  if ((ret = mbedtls2_md4_update_ret(ctx, msglen, 8)) != 0)
    return (ret);
...
#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md4_finish(mbedtls2_md4_context *ctx,
                                unsigned char output[16]) {
  mbedtls2_md4_finish_ret(ctx, output);
}
#endif
 ...
 */
int mbedtls2_md4_ret(const unsigned char *input, size_t ilen,
                            unsigned char output[16]) {
  int ret = MBEDTLS2_ERR_ERROR_CORRUPTION_DETECTED;
  mbedtls2_md4_context ctx;

  mbedtls2_md4_init(&ctx);

  if ((ret = mbedtls2_md4_starts_ret(&ctx)) != 0)
    goto exit;

  if ((ret = mbedtls2_md4_update_ret(&ctx, input, ilen)) != 0)
    goto exit;

  if ((ret = mbedtls2_md4_finish_ret(&ctx, output)) != 0)
    goto exit;

exit:
  mbedtls2_md4_free(&ctx);

  return (ret);
}

#if !defined(MBEDTLS2_DEPRECATED_REMOVED)
void mbedtls2_md4(const unsigned char *input, size_t ilen,
                         unsigned char output[16]) {
  mbedtls2_md4_ret(input, ilen, output);
}
#endif
 ...
 * Checkup routine
 */
int mbedtls2_md4_self_test(int verbose) {
  int i, ret = 0;
  unsigned char md4sum[16];

  for (i = 0; i < 7; i++) {
    if (verbose != 0)
      mbedtls2_printf("  MD4 test #%d: ", i + 1);

    ret = mbedtls2_md4_ret(md4_test_str[i], md4_test_strlen[i], md4sum);
    if (ret != 0)
      goto fail;
