...
#define ARIA_VALIDATE_RET(cond)                                                \
  MBEDTLS2_INTERNAL_VALIDATE_RET(                                       \
      cond, MBEDTLS2_ERR_ARIA_BAD_INPUT_DATA)
#define ARIA_VALIDATE(cond) MBEDTLS2_INTERNAL_VALIDATE(cond)
...
static inline void aria_sl(uint32_t *a, uint32_t *b, uint32_t *c, uint32_t *d,
  ...
  *a = ((uint32_t)sa[MBEDTLS2_BYTE_0(*a)]) ^
       (((uint32_t)sb[MBEDTLS2_BYTE_1(*a)]) << 8) ^
       (((uint32_t)sc[MBEDTLS2_BYTE_2(*a)]) << 16) ^
       (((uint32_t)sd[MBEDTLS2_BYTE_3(*a)]) << 24);
  *b = ((uint32_t)sa[MBEDTLS2_BYTE_0(*b)]) ^
       (((uint32_t)sb[MBEDTLS2_BYTE_1(*b)]) << 8) ^
       (((uint32_t)sc[MBEDTLS2_BYTE_2(*b)]) << 16) ^
       (((uint32_t)sd[MBEDTLS2_BYTE_3(*b)]) << 24);
  *c = ((uint32_t)sa[MBEDTLS2_BYTE_0(*c)]) ^
       (((uint32_t)sb[MBEDTLS2_BYTE_1(*c)]) << 8) ^
       (((uint32_t)sc[MBEDTLS2_BYTE_2(*c)]) << 16) ^
       (((uint32_t)sd[MBEDTLS2_BYTE_3(*c)]) << 24);
  *d = ((uint32_t)sa[MBEDTLS2_BYTE_0(*d)]) ^
       (((uint32_t)sb[MBEDTLS2_BYTE_1(*d)]) << 8) ^
       (((uint32_t)sc[MBEDTLS2_BYTE_2(*d)]) << 16) ^
       (((uint32_t)sd[MBEDTLS2_BYTE_3(*d)]) << 24);
}
 ...
 * MBEDTLS2_GET_UINT32_LE / MBEDTLS2_PUT_UINT32_LE ) so we need to
 * reverse bytes here.
 */
 ...
 * Set encryption key
 */
int mbedtls2_aria_setkey_enc(mbedtls2_aria_context *ctx,
  ...
  if (keybits != 128 && keybits != 192 && keybits != 256)
    return (MBEDTLS2_ERR_ARIA_BAD_INPUT_DATA);

  /* Copy key to W0 (and potential remainder to W1) */
  w[0][0] = MBEDTLS2_GET_UINT32_LE(key, 0);
  w[0][1] = MBEDTLS2_GET_UINT32_LE(key, 4);
  w[0][2] = MBEDTLS2_GET_UINT32_LE(key, 8);
  w[0][3] = MBEDTLS2_GET_UINT32_LE(key, 12);

  memset(w[1], 0, 16);
  if (keybits >= 192) {
    w[1][0] = MBEDTLS2_GET_UINT32_LE(key, 16); // 192 bit key
    w[1][1] = MBEDTLS2_GET_UINT32_LE(key, 20);
  }
  if (keybits == 256) {
    w[1][2] = MBEDTLS2_GET_UINT32_LE(key, 24); // 256 bit key
    w[1][3] = MBEDTLS2_GET_UINT32_LE(key, 28);
  }
  ...
  mbedtls2_platform_zeroize(w, sizeof(w));
 ...
 * Set decryption key
 */
int mbedtls2_aria_setkey_dec(mbedtls2_aria_context *ctx,
                                    const unsigned char *key,
                                    unsigned int keybits) {
  int i, j, k, ret;
  ARIA_VALIDATE_RET(ctx != NULL);
  ...
  ret = mbedtls2_aria_setkey_enc(ctx, key, keybits);
  if (ret != 0)
    return (ret);
 ...
 * Encrypt a block
 */
int mbedtls2_aria_crypt_ecb(
    mbedtls2_aria_context *ctx,
    const unsigned char input[MBEDTLS2_ARIA_BLOCKSIZE],
    unsigned char output[MBEDTLS2_ARIA_BLOCKSIZE]) {
  int i;
  ...
  a = MBEDTLS2_GET_UINT32_LE(input, 0);
  b = MBEDTLS2_GET_UINT32_LE(input, 4);
  c = MBEDTLS2_GET_UINT32_LE(input, 8);
  d = MBEDTLS2_GET_UINT32_LE(input, 12);
  ...
  MBEDTLS2_PUT_UINT32_LE(a, output, 0);
  MBEDTLS2_PUT_UINT32_LE(b, output, 4);
  MBEDTLS2_PUT_UINT32_LE(c, output, 8);
  MBEDTLS2_PUT_UINT32_LE(d, output, 12);

  return (0);
}

/* Initialize context */
void mbedtls2_aria_init(mbedtls2_aria_context *ctx) {
  ARIA_VALIDATE(ctx != NULL);
  memset(ctx, 0, sizeof(mbedtls2_aria_context));
}

/* Clear context */
void mbedtls2_aria_free(mbedtls2_aria_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls2_platform_zeroize(ctx, sizeof(mbedtls2_aria_context));
}
 ...
 */
int mbedtls2_aria_crypt_cbc(
    mbedtls2_aria_context *ctx, int mode, size_t length,
    unsigned char iv[MBEDTLS2_ARIA_BLOCKSIZE],
    const unsigned char *input, unsigned char *output) {
  int i;
  unsigned char temp[MBEDTLS2_ARIA_BLOCKSIZE];

  ARIA_VALIDATE_RET(ctx != NULL);
  ARIA_VALIDATE_RET(mode == MBEDTLS2_ARIA_ENCRYPT ||
                    mode == MBEDTLS2_ARIA_DECRYPT);
  ARIA_VALIDATE_RET(length == 0 || input != NULL);
  ARIA_VALIDATE_RET(length == 0 || output != NULL);
  ARIA_VALIDATE_RET(iv != NULL);

  if (length % MBEDTLS2_ARIA_BLOCKSIZE)
    return (MBEDTLS2_ERR_ARIA_INVALID_INPUT_LENGTH);

  if (mode == MBEDTLS2_ARIA_DECRYPT) {
    while (length > 0) {
      memcpy(temp, input, MBEDTLS2_ARIA_BLOCKSIZE);
      mbedtls2_aria_crypt_ecb(ctx, input, output);

      for (i = 0; i < MBEDTLS2_ARIA_BLOCKSIZE; i++)
        output[i] = (unsigned char)(output[i] ^ iv[i]);

      memcpy(iv, temp, MBEDTLS2_ARIA_BLOCKSIZE);

      input += MBEDTLS2_ARIA_BLOCKSIZE;
      output += MBEDTLS2_ARIA_BLOCKSIZE;
      length -= MBEDTLS2_ARIA_BLOCKSIZE;
    }
  } else {
    while (length > 0) {
      for (i = 0; i < MBEDTLS2_ARIA_BLOCKSIZE; i++)
      ...
      mbedtls2_aria_crypt_ecb(ctx, output, output);
      memcpy(iv, output, MBEDTLS2_ARIA_BLOCKSIZE);
 ...
 */
int mbedtls2_aria_crypt_cfb128(
    mbedtls2_aria_context *ctx, int mode, size_t length, size_t *iv_off,
    unsigned char iv[MBEDTLS2_ARIA_BLOCKSIZE],
    const unsigned char *input, unsigned char *output) {
  unsigned char c;
  size_t n;
  ...
  if (n >= MBEDTLS2_ARIA_BLOCKSIZE)
    return (MBEDTLS2_ERR_ARIA_BAD_INPUT_DATA);

  if (mode == MBEDTLS2_ARIA_DECRYPT) {
    while (length--) {
      if (n == 0)
        mbedtls2_aria_crypt_ecb(ctx, iv, iv);
 ...
 */
int mbedtls2_aria_crypt_ctr(
    mbedtls2_aria_context *ctx, size_t length, size_t *nc_off,
    unsigned char nonce_counter[MBEDTLS2_ARIA_BLOCKSIZE],
    unsigned char stream_block[MBEDTLS2_ARIA_BLOCKSIZE],
    const unsigned char *input, unsigned char *output) {
  int c, i;
  size_t n;
  ...
  while (length--) {
    if (n == 0) {
      mbedtls2_aria_crypt_ecb(ctx, nonce_counter, stream_block);

      for (i = MBEDTLS2_ARIA_BLOCKSIZE; i > 0; i--)
...
static const uint8_t
    aria_test1_ecb_pt[MBEDTLS2_ARIA_BLOCKSIZE] = // plaintext
    {
...
static const uint8_t
    aria_test1_ecb_ct[3][MBEDTLS2_ARIA_BLOCKSIZE] = // ciphertext
 ...
 */
     ...
     defined(MBEDTLS2_CIPHER_MODE_CTR))
...
#if (defined(MBEDTLS2_CIPHER_MODE_CBC) ||                               \
     defined(MBEDTLS2_CIPHER_MODE_CFB))
static const uint8_t aria_test2_iv[MBEDTLS2_ARIA_BLOCKSIZE] = {
 ...
 * Checkup routine
 */
int mbedtls2_aria_self_test(int verbose) {
  int i;
  uint8_t blk[MBEDTLS2_ARIA_BLOCKSIZE];
  mbedtls2_aria_context ctx;
  int ret = 1;
  ...
  size_t j;
     ...
     defined(MBEDTLS2_CIPHER_MODE_CFB) ||                               \
     defined(MBEDTLS2_CIPHER_MODE_CTR))
  uint8_t buf[48], iv[MBEDTLS2_ARIA_BLOCKSIZE];
#endif
  ...
  for (i = 0; i < 3; i++) {
    /* test ECB encryption */
    if (verbose)
      mbedtls2_printf("  ARIA-ECB-%d (enc): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test1_ecb_key, 128 + 64 * i);
    mbedtls2_aria_crypt_ecb(&ctx, aria_test1_ecb_pt, blk);
    if (memcmp(blk, aria_test1_ecb_ct[i], MBEDTLS2_ARIA_BLOCKSIZE) != 0)
      ARIA_SELF_TEST_IF_FAIL;

    /* test ECB decryption */
    if (verbose)
      mbedtls2_printf("  ARIA-ECB-%d (dec): ", 128 + 64 * i);
    mbedtls2_aria_setkey_dec(&ctx, aria_test1_ecb_key, 128 + 64 * i);
    mbedtls2_aria_crypt_ecb(&ctx, aria_test1_ecb_ct[i], blk);
    if (memcmp(blk, aria_test1_ecb_pt, MBEDTLS2_ARIA_BLOCKSIZE) != 0)
      ARIA_SELF_TEST_IF_FAIL;
  }
  if (verbose)
    mbedtls2_printf("\n");
  ...
  for (i = 0; i < 3; i++) {
    /* Test CBC encryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CBC-%d (enc): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
    memcpy(iv, aria_test2_iv, MBEDTLS2_ARIA_BLOCKSIZE);
    memset(buf, 0x55, sizeof(buf));
    mbedtls2_aria_crypt_cbc(&ctx, MBEDTLS2_ARIA_ENCRYPT, 48, iv,
                                   aria_test2_pt, buf);
    ...
    /* Test CBC decryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CBC-%d (dec): ", 128 + 64 * i);
    mbedtls2_aria_setkey_dec(&ctx, aria_test2_key, 128 + 64 * i);
    memcpy(iv, aria_test2_iv, MBEDTLS2_ARIA_BLOCKSIZE);
    memset(buf, 0xAA, sizeof(buf));
    mbedtls2_aria_crypt_cbc(&ctx, MBEDTLS2_ARIA_DECRYPT, 48, iv,
  ...
  for (i = 0; i < 3; i++) {
    /* Test CFB encryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CFB-%d (enc): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
    memcpy(iv, aria_test2_iv, MBEDTLS2_ARIA_BLOCKSIZE);
    memset(buf, 0x55, sizeof(buf));
    j = 0;
    mbedtls2_aria_crypt_cfb128(&ctx, MBEDTLS2_ARIA_ENCRYPT, 48,
                                      &j, iv, aria_test2_pt, buf);
    if (memcmp(buf, aria_test2_cfb_ct[i], 48) != 0)
      ARIA_SELF_TEST_IF_FAIL;

    /* Test CFB decryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CFB-%d (dec): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
    memcpy(iv, aria_test2_iv, MBEDTLS2_ARIA_BLOCKSIZE);
    memset(buf, 0xAA, sizeof(buf));
    j = 0;
    mbedtls2_aria_crypt_cfb128(&ctx, MBEDTLS2_ARIA_DECRYPT, 48,
                                      &j, iv, aria_test2_cfb_ct[i], buf);
    if (memcmp(buf, aria_test2_pt, 48) != 0)
      ARIA_SELF_TEST_IF_FAIL;
  }
  if (verbose)
  ...
  for (i = 0; i < 3; i++) {
    /* Test CTR encryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CTR-%d (enc): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
    memset(iv, 0, MBEDTLS2_ARIA_BLOCKSIZE); // IV = 0
    memset(buf, 0x55, sizeof(buf));
    j = 0;
    mbedtls2_aria_crypt_ctr(&ctx, 48, &j, iv, blk, aria_test2_pt, buf);
    if (memcmp(buf, aria_test2_ctr_ct[i], 48) != 0)
      ARIA_SELF_TEST_IF_FAIL;

    /* Test CTR decryption */
    if (verbose)
      mbedtls2_printf("  ARIA-CTR-%d (dec): ", 128 + 64 * i);
    mbedtls2_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
    memset(iv, 0, MBEDTLS2_ARIA_BLOCKSIZE); // IV = 0
    memset(buf, 0xAA, sizeof(buf));
    j = 0;
    mbedtls2_aria_crypt_ctr(&ctx, 48, &j, iv, blk, aria_test2_ctr_ct[i],
                                   buf);
    if (memcmp(buf, aria_test2_pt, 48) != 0)
      ARIA_SELF_TEST_IF_FAIL;
  }
