...
TEST_F(FileSystemTest, CreateFile)
{
  ...
  constexpr u8 ARBITRARY_ATTRIBUTE = 0xE1;

  ASSERT_EQ(m_fs->CreateFile(Uid{0}, Gid{0}, path, ARBITRARY_ATTRIBUTE, MODES), ResultCode::Success);
  ...
  EXPECT_EQ(stats->attribute, ARBITRARY_ATTRIBUTE);
...
TEST_F(FileSystemTest, CreateDirectory)
{
  ...
  constexpr u8 ARBITRARY_ATTRIBUTE = 0x20;

  ASSERT_EQ(m_fs->CreateDirectory(Uid{0}, Gid{0}, path, ARBITRARY_ATTRIBUTE, MODES),
            ResultCode::Success);
