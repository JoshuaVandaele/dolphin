...
void NDKCameraInterface::OnImageAvailable(void* ctx, AImageReader* reader)
{
  ...
  std::lock_guard lock(self->m_close_mutex);
  if (!self->m_is_open)
    return;
...
std::expected<void, CameraError> NDKCameraInterface::Open()
{
  ...
  if (jboolean result =
          env->CallStaticBooleanMethod(IDCache::GetPermissionHandlerClass(),
                                       IDCache::GetPermissionHandlerHasCameraPermission(), nullptr);
      result == JNI_FALSE)
  {
    env->CallStaticVoidMethod(IDCache::GetPermissionHandlerClass(),
                              IDCache::GetPermissionHandlerRequestCameraPermission(), nullptr);
  }
