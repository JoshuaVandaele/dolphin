...
static inline void DecodeBytesC4IA8(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u8 val = src[x];
    *dst++ = DecodePixelIA8(tlut[val >> 4]);
    *dst++ = DecodePixelIA8(tlut[val & 0xF]);
  }
}

static inline void DecodeBytesC4RgB565(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u8 val = src[x];
    *dst++ = DecodePixelRgB565(Common::swap16(tlut[val >> 4]));
    *dst++ = DecodePixelRgB565(Common::swap16(tlut[val & 0xF]));
  }
}

static inline void DecodeBytesC4RgB5A3(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u8 val = src[x];
    *dst++ = DecodePixelRgB5A3(Common::swap16(tlut[val >> 4]));
    *dst++ = DecodePixelRgB5A3(Common::swap16(tlut[val & 0xF]));
  }
}

static inline void DecodeBytesC8IA8(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 8; x++)
  {
    *dst++ = DecodePixelIA8(tlut[src[x]]);
  }
}

static inline void DecodeBytesC8RgB565(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 8; x++)
  {
    u8 val = src[x];
    *dst++ = DecodePixelRgB565(Common::swap16(tlut[val]));
  }
}

static inline void DecodeBytesC8RgB5A3(u32* dst, const u8* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 8; x++)
  {
    u8 val = src[x];
    *dst++ = DecodePixelRgB5A3(Common::swap16(tlut[val]));
  }
}

static inline void DecodeBytesC14X2IA8(u32* dst, const u16* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u16 val = Common::swap16(src[x]);
    *dst++ = DecodePixelIA8(tlut[(val & 0x3FFF)]);
  }
}

static inline void DecodeBytesC14X2RgB565(u32* dst, const u16* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u16 val = Common::swap16(src[x]);
    *dst++ = DecodePixelRgB565(Common::swap16(tlut[(val & 0x3FFF)]));
  }
}

static inline void DecodeBytesC14X2RgB5A3(u32* dst, const u16* src, const u8* tlut_)
{
  const u16* tlut = (u16*)tlut_;
  for (int x = 0; x < 4; x++)
  {
    u16 val = Common::swap16(src[x]);
    *dst++ = DecodePixelRgB5A3(Common::swap16(tlut[(val & 0x3FFF)]));
  }
}

static inline void DecodeBytesIA4(u32* dst, const u8* src)
{
  for (int x = 0; x < 8; x++)
  {
    const u8 val = src[x];
...
static void TexDecoderDecodeImplC4(u32* dst, const u8* src, int width, int height,
                                     ...
                                     int Wsteps4, int Wsteps8)
{
  ...
  case TLUTFormat::RGB5A3:
  {
    for (int y = 0; y < height; y += 8)
      for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 8 * y_step; iy < 8; iy++, x_step++)
          DecodeBytesC4RgB5A3(dst + (y + iy) * width + x, src + 4 * x_step, tlut);
  }
  break;
  ...
  case TLUTFormat::IA8:
  {
    for (int y = 0; y < height; y += 8)
      for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 8 * y_step; iy < 8; iy++, x_step++)
          DecodeBytesC4IA8(dst + (y + iy) * width + x, src + 4 * x_step, tlut);
  }
  break;
  ...
  case TLUTFormat::RGB565:
  {
    for (int y = 0; y < height; y += 8)
      for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 8 * y_step; iy < 8; iy++, x_step++)
          DecodeBytesC4RgB565(dst + (y + iy) * width + x, src + 4 * x_step, tlut);
  }
  break;
...
static void TexDecoderDecodeImplI4SssE3(u32* dst, const u8* src, int width, int height,
                                           TextureFormat texformat, const u8* tlut,
                                           TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  const __m128i k_mask_x0f = _mm_set1_epi32(0x0f0f0f0fL);
  const __m128i k_mask_xf0 = _mm_set1_epi32(0xf0f0f0f0L);
  ...
  const __m128i mask_b3_a2 = _mm_set_epi8(11, 11, 11, 11, 3, 3, 3, 3, 10, 10, 10, 10, 2, 2, 2, 2);
  const __m128i mask_d5_c4 = _mm_set_epi8(13, 13, 13, 13, 5, 5, 5, 5, 12, 12, 12, 12, 4, 4, 4, 4);
  const __m128i mask_f7_e6 = _mm_set_epi8(15, 15, 15, 15, 7, 7, 7, 7, 14, 14, 14, 14, 6, 6, 6, 6);
  for (int y = 0; y < height; y += 8)
  {
    for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 8; iy += 2, x_step++)
      {
        const __m128i r0 = _mm_loadl_epi64((const __m128i*)(src + 8 * x_step));
        ...
        const __m128i o2 = _mm_shuffle_epi8(base, mask_b3_a2);
        const __m128i o3 = _mm_shuffle_epi8(base, mask_d5_c4);
        const __m128i o4 = _mm_shuffle_epi8(base, mask_f7_e6);
...
static void TexDecoderDecodeImplI4(u32* dst, const u8* src, int width, int height,
                                     TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                     int Wsteps4, int Wsteps8)
{
  const __m128i k_mask_x0f = _mm_set1_epi32(0x0f0f0f0fL);
  ...
  for (int y = 0; y < height; y += 8)
  {
    for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 8; iy += 2, x_step++)
      {
        ...
        // B0B0A0A0)
        const __m128i i1 = _mm_and_si128(r1, k_mask_xf0);
        ...
        const __m128i i2 = _mm_and_si128(r1, k_mask_x0f);
        const __m128i i22 = _mm_or_si128(i2, _mm_slli_epi16(i2, 4));
        ...
        const __m128i k_mask_x00000000ffffffff =
            _mm_set_epi32(0x00000000L, 0xffffffffL, 0x00000000L, 0xffffffffL);
        const __m128i k_mask_xffffffff00000000 =
            _mm_set_epi32(0xffffffffL, 0x00000000L, 0xffffffffL, 0x00000000L);
        const __m128i o1 = _mm_or_si128(_mm_and_si128(i151, k_mask_x00000000ffffffff),
                                        _mm_and_si128(i251, k_mask_xffffffff00000000));
        const __m128i o2 = _mm_or_si128(_mm_and_si128(i152, k_mask_x00000000ffffffff),
                                        _mm_and_si128(i252, k_mask_xffffffff00000000));

        // These two are for the next row; same pattern as above. We batched up two rows because
        // our input was 64 bits.
        const __m128i o3 = _mm_or_si128(_mm_and_si128(i161, k_mask_x00000000ffffffff),
                                        _mm_and_si128(i261, k_mask_xffffffff00000000));
        const __m128i o4 = _mm_or_si128(_mm_and_si128(i162, k_mask_x00000000ffffffff),
                                        _mm_and_si128(i262, k_mask_xffffffff00000000));
        // Write row 0:
        _mm_storeu_si128((__m128i*)(dst + (y + iy) * width + x), o1);
        _mm_storeu_si128((__m128i*)(dst + (y + iy) * width + x + 4), o2);
        // Write row 1:
...
FUNCTION_TARGET_SSSE3
static void TexDecoderDecodeImplI8SssE3(u32* dst, const u8* src, int width, int height,
                                           TextureFormat texformat, const u8* tlut,
                                           TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  // xsacha optimized with SSSE3 intrinsics
  ...
  {
    for (int x = 0, y_step = (y / 4) * Wsteps8; x < width; x += 8, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; ++iy, x_step++)
      {
        ...
        r = _mm_loadl_epi64((const __m128i*)(src + 8 * x_step));
...
static void TexDecoderDecodeImplI8(u32* dst, const u8* src, int width, int height,
                                     TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                     int Wsteps4, int Wsteps8)
{
  // JSD optimized with SSE2 intrinsics.
...
static void TexDecoderDecodeImplC8(u32* dst, const u8* src, int width, int height,
                                     TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                     int Wsteps4, int Wsteps8)
{
  switch (tlutfmt)
  ...
  case TLUTFormat::RGB5A3:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC8RgB5A3((u32*)dst + (y + iy) * width + x, src + 8 * x_step, tlut);
  }
  break;
  ...
  case TLUTFormat::IA8:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC8IA8(dst + (y + iy) * width + x, src + 8 * x_step, tlut);
  }
  break;
  ...
  case TLUTFormat::RGB565:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps8; x < width; x += 8, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC8RgB565(dst + (y + iy) * width + x, src + 8 * x_step, tlut);
  }
  break;
...
static void TexDecoderDecodeImplIA4(u32* dst, const u8* src, int width, int height,
                                      TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                      int Wsteps4, int Wsteps8)
{
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps8; x < width; x += 8, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
      {
        DecodeBytesIA4(dst + (y + iy) * width + x, src + 8 * x_step);
      }
    }
  }
}
...
FUNCTION_TARGET_SSSE3
static void TexDecoderDecodeImplIA8SssE3(u32* dst, const u8* src, int width, int height,
                                            TextureFormat texformat, const u8* tlut,
                                            TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  // xsacha optimized with SSSE3 intrinsics.
  ...
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
      {
...
static void TexDecoderDecodeImplIA8(u32* dst, const u8* src, int width, int height,
                                      TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                      int Wsteps4, int Wsteps8)
{
  // JSD optimized with SSE2 intrinsics.
  // Produces an ~80% speed improvement over reference C implementation.
  const __m128i k_mask_xf0 = _mm_set_epi32(0x00000000L, 0x00000000L, 0xff00ff00L, 0xff00ff00L);
  const __m128i k_mask_x0f = _mm_set_epi32(0x00000000L, 0x00000000L, 0x00ff00ffL, 0x00ff00ffL);
  const __m128i k_mask_xf000 = _mm_set_epi32(0xff000000L, 0xff000000L, 0xff000000L, 0xff000000L);
  const __m128i k_mask_x0fff = _mm_set_epi32(0x00ffffffL, 0x00ffffffL, 0x00ffffffL, 0x00ffffffL);
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
      {
        ...
        const __m128i i1 = _mm_or_si128(_mm_and_si128(r0, k_mask_xf0), i0);
        ...
        const __m128i i3 = _mm_and_si128(i2, k_mask_x0fff);
        ...
        const __m128i a0 = _mm_and_si128(r0, k_mask_x0f);
        ...
        const __m128i a3 = _mm_and_si128(a2, k_mask_xf000);
...
static void TexDecoderDecodeImplC14X2(u32* dst, const u8* src, int width, int height,
                                        TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                        int Wsteps4, int Wsteps8)
{
  switch (tlutfmt)
  ...
  case TLUTFormat::RGB5A3:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC14X2RgB5A3(dst + (y + iy) * width + x, (u16*)(src + 8 * x_step), tlut);
  }
  break;
  ...
  case TLUTFormat::IA8:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC14X2IA8(dst + (y + iy) * width + x, (u16*)(src + 8 * x_step), tlut);
  }
  break;
  ...
  case TLUTFormat::RGB565:
  {
    for (int y = 0; y < height; y += 4)
      for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
        for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
          DecodeBytesC14X2RgB565(dst + (y + iy) * width + x, (u16*)(src + 8 * x_step), tlut);
  }
  break;
...
static void TexDecoderDecodeImplRgB565(u32* dst, const u8* src, int width, int height,
                                         TextureFormat texformat, const u8* tlut,
                                         TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  // JSD optimized with SSE2 intrinsics.
  // Produces an ~78% speed improvement over reference C implementation.
  const __m128i k_mask_r0 = _mm_set1_epi32(0x000000F8);
  const __m128i k_mask_g0 = _mm_set1_epi32(0x0000FC00);
  const __m128i k_mask_g1 = _mm_set1_epi32(0x00000300);
  const __m128i k_mask_b0 = _mm_set1_epi32(0x00F80000);
  const __m128i k_alpha = _mm_set1_epi32(0xFF000000);
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
      {
        __m128i* dxtsrc = (__m128i*)(src + 8 * x_step);
        ...
        const __m128i r0 = _mm_and_si128(c0, k_mask_r0);
        ...
        const __m128i g0 = _mm_and_si128(gtmp, k_mask_g0);
        ...
        const __m128i g1 = _mm_and_si128(_mm_srli_epi32(gtmp, 6), k_mask_g1);
        ...
        const __m128i b0 = _mm_and_si128(_mm_srli_epi32(c0, 5), k_mask_b0);
        ...
        const __m128i abgr888x4 =
            _mm_or_si128(_mm_or_si128(_mm_or_si128(r0, r1), _mm_or_si128(g0, g1)),
                         _mm_or_si128(_mm_or_si128(b0, b1), k_alpha));
...
FUNCTION_TARGET_SSSE3
static void TexDecoderDecodeImplRgB5A3SssE3(u32* dst, const u8* src, int width, int height,
                                               TextureFormat texformat, const u8* tlut,
                                               TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  const __m128i k_mask_x1f = _mm_set1_epi32(0x0000001fL);
  const __m128i k_mask_x0f = _mm_set1_epi32(0x0000000fL);
  const __m128i k_mask_x07 = _mm_set1_epi32(0x00000007L);
  ...
  const __m128i a_vxff00 = _mm_set1_epi32(0xFF000000L);
  ...
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      for (int iy = 0, x_step = 4 * y_step; iy < 4; iy++, x_step++)
      {
        ...
        const __m128i val_v =
            _mm_shuffle_epi8(_mm_loadl_epi64((const __m128i*)(src + 8 * x_step)), mask);
        int cmp = _mm_movemask_epi8(val_v);  // MSB: 0x2 = val0; 0x20=val1; 0x200 = val2; 0x2000=val3
        if ((cmp & 0x2222) ==
            0x2222)  // SSSE3 case #1: all 4 pixels are in RGB555 and alpha = 0xFF.
        {
          ...
          const __m128i tmpr_v = _mm_and_si128(_mm_srli_epi16(val_v, 10), k_mask_x1f);
          const __m128i r_v = _mm_or_si128(_mm_slli_epi16(tmpr_v, 3), _mm_srli_epi16(tmpr_v, 2));

          // g0 = (((val0>>5 ) & 0x1f) << 3) | (((val0>>5 ) & 0x1f) >> 2);
          const __m128i tmpg_v = _mm_and_si128(_mm_srli_epi16(val_v, 5), k_mask_x1f);
          const __m128i g_v = _mm_or_si128(_mm_slli_epi16(tmpg_v, 3), _mm_srli_epi16(tmpg_v, 2));

          // b0 = (((val0    ) & 0x1f) << 3) | (((val0    ) & 0x1f) >> 2);
          const __m128i tmpb_v = _mm_and_si128(val_v, k_mask_x1f);
          const __m128i b_v = _mm_or_si128(_mm_slli_epi16(tmpb_v, 3), _mm_srli_epi16(tmpb_v, 2));

          // newdst[0] = r0 | (g0 << 8) | (b0 << 16) | (a0 << 24);
          const __m128i final = _mm_or_si128(_mm_or_si128(r_v, _mm_slli_epi32(g_v, 8)),
                                             _mm_or_si128(_mm_slli_epi32(b_v, 16), a_vxff00));
        ...
        else if (!(cmp & 0x2222))  // SSSE3 case #2: all 4 pixels are in RGBA4443.
        {
          ...
          const __m128i tmpr_v = _mm_and_si128(_mm_srli_epi16(val_v, 8), k_mask_x0f);
          const __m128i r_v = _mm_or_si128(_mm_slli_epi16(tmpr_v, 4), tmpr_v);

          // g0 = (((val0>>4 ) & 0xf) << 4) | ((val0>>4 ) & 0xf);
          const __m128i tmpg_v = _mm_and_si128(_mm_srli_epi16(val_v, 4), k_mask_x0f);
          const __m128i g_v = _mm_or_si128(_mm_slli_epi16(tmpg_v, 4), tmpg_v);

          // b0 = (((val0    ) & 0xf) << 4) | ((val0    ) & 0xf);
          const __m128i tmpb_v = _mm_and_si128(val_v, k_mask_x0f);
          const __m128i b_v = _mm_or_si128(_mm_slli_epi16(tmpb_v, 4), tmpb_v);
          // a0 = (((val0>>12) & 0x7) << 5) | (((val0>>12) & 0x7) << 2) | (((val0>>12) & 0x7) >> 1);
          const __m128i tmpa_v = _mm_and_si128(_mm_srli_epi16(val_v, 12), k_mask_x07);
          const __m128i a_v =
              _mm_or_si128(_mm_slli_epi16(tmpa_v, 5),
                           _mm_or_si128(_mm_slli_epi16(tmpa_v, 2), _mm_srli_epi16(tmpa_v, 1)));

          // newdst[0] = r0 | (g0 << 8) | (b0 << 16) | (a0 << 24);
          const __m128i final =
              _mm_or_si128(_mm_or_si128(r_v, _mm_slli_epi32(g_v, 8)),
                           _mm_or_si128(_mm_slli_epi32(b_v, 16), _mm_slli_epi32(a_v, 24)));
          _mm_storeu_si128((__m128i*)newdst, final);
        }
        else
        {
...
static void TexDecoderDecodeImplRgB5A3(u32* dst, const u8* src, int width, int height,
                                         TextureFormat texformat, const u8* tlut,
                                         TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
  const __m128i k_mask_x1f = _mm_set1_epi32(0x0000001fL);
  ...
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      ...
      {
        u32* newdst = dst + (y + iy) * width + x;
        const u16* newsrc = (const u16*)(src + 8 * x_step);
        ...
        const __m128i val_v = _mm_set_epi16(0, val3, 0, val2, 0, val1, 0, val0);
...
FUNCTION_TARGET_SSSE3
static void TexDecoderDecodeImplRgbA8SssE3(u32* dst, const u8* src, int width, int height,
                                              TextureFormat texformat, const u8* tlut,
                                              TLUTFormat tlutfmt, int Wsteps4, int Wsteps8)
{
...
static void TexDecoderDecodeImplRgbA8(u32* dst, const u8* src, int width, int height,
                                        TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                        int Wsteps4, int Wsteps8)
{
  // JSD optimized with SSE2 intrinsics
  ...
  for (int y = 0; y < height; y += 4)
  {
    for (int x = 0, y_step = (y / 4) * Wsteps4; x < width; x += 4, y_step++)
    {
      ...
      const __m128i k_mask_x000f = _mm_set_epi32(0x000000FFL, 0x000000FFL, 0x000000FFL, 0x000000FFL);
      const __m128i k_mask_xf000 = _mm_set_epi32(0xFF000000L, 0xFF000000L, 0xFF000000L, 0xFF000000L);
      const __m128i k_mask_x0ff0 = _mm_set_epi32(0x00FFFF00L, 0x00FFFF00L, 0x00FFFF00L, 0x00FFFF00L);
      ...
      // final place:
      const __m128i a00 = _mm_and_si128(_mm_srli_epi32(aarr00, 16), k_mask_x000f);
      // Move R left  16 bits and mask off everything but the highest 8 bits to get R in its
      // final place:
      const __m128i r___00 = _mm_and_si128(_mm_slli_epi32(aarr00, 16), k_mask_xf000);
      // OR the two together to get R and A in their final places:
      const __m128i r__a00 = _mm_or_si128(r___00, a00);

      const __m128i a01 = _mm_and_si128(_mm_srli_epi32(aarr01, 16), k_mask_x000f);
      const __m128i r___01 = _mm_and_si128(_mm_slli_epi32(aarr01, 16), k_mask_xf000);
      const __m128i r__a01 = _mm_or_si128(r___01, a01);

      const __m128i a10 = _mm_and_si128(_mm_srli_epi32(aarr10, 16), k_mask_x000f);
      const __m128i r___10 = _mm_and_si128(_mm_slli_epi32(aarr10, 16), k_mask_xf000);
      const __m128i r__a10 = _mm_or_si128(r___10, a10);

      const __m128i a11 = _mm_and_si128(_mm_srli_epi32(aarr11, 16), k_mask_x000f);
      const __m128i r___11 = _mm_and_si128(_mm_slli_epi32(aarr11, 16), k_mask_xf000);
      const __m128i r__a11 = _mm_or_si128(r___11, a11);
      ...
      const __m128i gb_00 = _mm_and_si128(ggbb00, k_mask_x0ff0);
      const __m128i gb_01 = _mm_and_si128(ggbb01, k_mask_x0ff0);
      const __m128i gb_10 = _mm_and_si128(ggbb10, k_mask_x0ff0);
      const __m128i gb_11 = _mm_and_si128(ggbb11, k_mask_x0ff0);
...
static void TexDecoderDecodeImplCmpr(u32* dst, const u8* src, int width, int height,
                                       TextureFormat texformat, const u8* tlut, TLUTFormat tlutfmt,
                                       int Wsteps4, int Wsteps8)
{
  ...
  for (int y = 0; y < height; y += 8)
  {
    for (int x = 0, y_step = (y / 8) * Wsteps8; x < width; x += 8, y_step++)
    {
      ...
      for (int z = 0, x_step = 2 * y_step; z < 2; ++z, x_step++)
      {
        ...
        const __m128i all_f_fs128 = _mm_cmpeq_epi32(_mm_setzero_si128(), _mm_setzero_si128());

        // Load 128 bits, i.e. two DXTBlocks (64-bits each)
        const __m128i dxt = _mm_loadu_si128((__m128i*)(src + sizeof(struct DXTBlock) * 2 * x_step));
        ...
        const __m128i low6mask = _mm_slli_epi32(_mm_srli_epi32(all_f_fs128, 24 + 2), 8 + 2);
        const __m128i gtmp = _mm_srli_epi32(c0, 3);
        ...
        // 0xFF000000)
        argb888x4 = _mm_or_si128(_mm_or_si128(argb888x4, _mm_slli_epi32(all_f_fs128, 24)),
                                 _mm_or_si128(b0, b1));
        ...
        const __m128i rrggbb0 =
            _mm_and_si128(_mm_unpacklo_epi8(rgb0, rgb0), _mm_srli_epi16(all_f_fs128, 8));
        const __m128i rrggbb1 =
            _mm_and_si128(_mm_unpacklo_epi8(rgb1, rgb1), _mm_srli_epi16(all_f_fs128, 8));
        const __m128i rrggbb01 =
            _mm_and_si128(_mm_unpackhi_epi8(rgb0, rgb0), _mm_srli_epi16(all_f_fs128, 8));
        const __m128i rrggbb11 =
            _mm_and_si128(_mm_unpackhi_epi8(rgb1, rgb1), _mm_srli_epi16(all_f_fs128, 8));
        ...
        if (cmp0 != 0)
        {
          ...
          rgb2 = _mm_and_si128(rgb2dup, _mm_srli_si128(all_f_fs128, 8));
          rgb3 = _mm_and_si128(rgb3dup, _mm_srli_si128(all_f_fs128, 8));
        }
        else
        {
          // RGB2b = avg(RGB0, RGB1)
          ...
          rgb2 = rgb210;
          rgb3 = _mm_and_si128(rgb210, _mm_srli_epi32(all_f_fs128, 8));
        }
        ...
        if (cmp1 != 0)
        {
          ...
          rgb2 = _mm_or_si128(rgb2, _mm_and_si128(rgb2dup, _mm_slli_si128(all_f_fs128, 8)));
          rgb3 = _mm_or_si128(rgb3, _mm_and_si128(rgb3dup, _mm_slli_si128(all_f_fs128, 8)));
        }
        else
        {
          // RGB2b = avg(RGB0, RGB1)
          ...
          rgb3 = _mm_or_si128(rgb3, _mm_and_si128(_mm_and_si128(rgb2, _mm_srli_epi32(all_f_fs128, 8)),
                                                  _mm_slli_si128(all_f_fs128, 8)));
        }
                            ...
                            const u8* tlut, TLUTFormat tlutfmt)
{
  ...
  case TextureFormat::C4:
    TexDecoderDecodeImplC4(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;

  case TextureFormat::I4:
    if (cpu_info.bSSSE3)
      TexDecoderDecodeImplI4SssE3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                     wsteps8);
    else
      TexDecoderDecodeImplI4(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;

  case TextureFormat::I8:
    if (cpu_info.bSSSE3)
      TexDecoderDecodeImplI8SssE3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                     wsteps8);
    else
      TexDecoderDecodeImplI8(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;

  case TextureFormat::C8:
    TexDecoderDecodeImplC8(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;

  case TextureFormat::IA4:
    TexDecoderDecodeImplIA4(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;

  case TextureFormat::IA8:
    if (cpu_info.bSSSE3)
      TexDecoderDecodeImplIA8SssE3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                      wsteps8);
    else
      TexDecoderDecodeImplIA8(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                wsteps8);
    break;

  case TextureFormat::C14X2:
    TexDecoderDecodeImplC14X2(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                wsteps8);
    break;

  case TextureFormat::RGB565:
    TexDecoderDecodeImplRgB565(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                 wsteps8);
    break;

  case TextureFormat::RGB5A3:
    if (cpu_info.bSSSE3)
      TexDecoderDecodeImplRgB5A3SssE3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                         wsteps8);
    else
      TexDecoderDecodeImplRgB5A3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                   wsteps8);
    break;

  case TextureFormat::RGBA8:
    if (cpu_info.bSSSE3)
      TexDecoderDecodeImplRgbA8SssE3(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                        wsteps8);
    else
      TexDecoderDecodeImplRgbA8(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4,
                                  wsteps8);
    break;

  case TextureFormat::CMPR:
    TexDecoderDecodeImplCmpr(dst, src, width, height, texformat, tlut, tlutfmt, wsteps4, wsteps8);
    break;
