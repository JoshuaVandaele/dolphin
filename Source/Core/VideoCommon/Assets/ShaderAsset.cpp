                                  ...
                                  std::vector<ShaderProperty>* shader_properties)
{
  ...
  for (const auto& property_data : properties_data)
  {
    ...
    property.name = code_name_it->get<std::string>();

    if (auto default_it = property_data.find("default"); default_it != property_data.end())
    {
      if (!ParseShaderValue(asset_id, *default_it, property.name, type, &property.default_value))
      {
        return false;
      }
    }
                                       ...
                                       const nlohmann::json& json, RasterSurfaceShaderData* data)
{
  const auto parse_properties = [&](const char* name, std::string_view source,
    ...
    return ParseShaderProperties(asset_id, values_data, shader_properties);
  };
...
{
  const auto add_property = [](nlohmann::json* json_properties, const ShaderProperty& property) {
    nlohmann::json json_property;

    json_property["code_name"] = property.name;
