...
void DecryptARCode(std::vector<std::string> vCodes, std::vector<AREntry>* ops)
{
  ...
  const u32 ret = Alphatobin(u_codes.data(), vCodes, (int)vCodes.size());
  if (ret)
  {
  ...
  else if (!Batchdecrypt(u_codes.data(), (u16)vCodes.size() << 1))
  {
    ...
    {
      ops->emplace_back(u_codes[i], u_codes[i + 1]);
